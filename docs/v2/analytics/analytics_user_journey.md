...
- Call-site CSV created: [docs/v2/analytics_event_callsites.csv](../analytics_event_callsites.csv) — full mapping of detected server-side event write call sites (file, line, table, event_type, actor, description).
- Reconciliation report: [docs/v2/analytics_event_reconciliation.md](../analytics_event_reconciliation.md) — compares server-emitted events against the client `VALID_EVENT_TYPES` allowlist and lists recommended actions.
