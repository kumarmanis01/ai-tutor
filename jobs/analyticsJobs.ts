...
export async function runAnalyticsJobs(): Promise<{
  success: boolean;
  durationMs: number;
  error?: string;
  skipped?: boolean;
  ...
  try {
    ...
    if ((lock as any).skipped) {
      ...
      logAuditEvent(prisma, {
        ...
        action: null,
        details: {
          legacyAction: 'ANALYTICS_JOB_RUN',
          status: 'SKIPPED',
      ...
      if (typeof agg.runForAllCourses === 'function') {
        ...
        try {
          (await import('@/lib/logger')).logger?.warn?.(
            'analyticsAggregator.runForAllCourses not found; skipping aggregation step'
          );
        } catch {}
      }
    } catch (e) {
    ...
    try {
      ...
      if (typeof signals.generateSignalsForAllCourses === 'function') {
        ...
        try {
          (await import('@/lib/logger')).logger?.warn?.(
            'generateSignalsForAllCourses not found; skipping signals generation'
          );
        } catch {}
      }
      ...
      try {
        ...
        const newSignals = await (db as any).analyticsSignal.findMany({
          where: { createdAt: { gte: new Date(started) } },
        });
        for (const sig of newSignals) {
          try {
            ...
            try {
              const logger = (await import('@/lib/logger')).logger;
              logger?.warn?.('Insight engine failed for signal', {
                signalId: sig.id,
                err: (inner as Error)?.message ?? String(inner),
              });
            } catch {}
          }
        }
        ...
        try {
          const logger = (await import('@/lib/logger')).logger;
          logger?.warn?.('Failed to persist content suggestions', {
  ...
  try {
    logAuditEvent(prisma, {
      ...
      details: {
        ...
        durationMs,
        error: accumulatedError ?? null,
      },
    });
  } catch (e) {
    try {
      const logger = (await import('@/lib/logger')).logger;
      logger?.warn?.('Failed to enqueue audit log for analytics job', {
  ...
  try {
    ...
    try {
      const logger = (await import('@/lib/logger')).logger;
      logger?.warn?.('Failed to increment analytics job metric', {
        error: (e as Error)?.message ?? String(e),
      });
    } catch {}
  }
  ...
  if (lockAcquired) {
    try {
      await releaseJobLock('analytics_jobs');
    } catch {}
  }
