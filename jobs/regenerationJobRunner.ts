...
export async function runRegenerationJobs() {
  ...
  const tryLock: any = await (prisma as any)
    .$queryRaw`SELECT pg_try_advisory_lock(${lockKey}) as acquired`;
  let acquired = Array.isArray(tryLock)
    ? (tryLock[0]?.acquired ?? false)
  ...
  try {
    ...
    const pending = await (prisma as any).regenerationJob.findFirst({
    ...
    let claim = await (prisma as any).regenerationJob.updateMany({
    ...
    if (!claim?.count || claim.count === 0) {
      ...
      try {
        await (prisma as any).regenerationJob.update({
          where: { id: pending.id },
          data: { status: 'RUNNING' },
        });
        claim = { count: 1 };
    ...
    logAuditEvent(prisma as any, {
      ...
      targetId: job.id,
      action: null,
      details: { legacyAction: AuditEvents.REGEN_JOB_STARTED },
    });
    ...
    try {
      ...
      const updated = await (prisma as any).regenerationJob.updateMany({
      ...
      if ((updated?.count ?? 0) > 0) {
        logAuditEvent(prisma as any, {
          ...
          targetId: job.id,
          action: null,
          details: { legacyAction: AuditEvents.REGEN_JOB_COMPLETED, outputRef },
        });
      }
      ...
      await (prisma as any).regenerationJob
        .updateMany({
          where: { id: job.id, status: 'RUNNING' },
      ...
      logAuditEvent(prisma as any, {
        ...
        targetId: job.id,
        action: null,
        details: { legacyAction: AuditEvents.REGEN_JOB_FAILED, errorJson },
      });
      return { processed: 0 };
    }
  } finally {
