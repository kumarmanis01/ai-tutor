...
export async function POST(req: Request) {
  try {
    ...
    logger.info('[verify-access-token] incoming body keys', {
      className: 'api.msg91.verify-access-token',
      methodName: 'POST',
    ...
    if (!accessToken) {
      return NextResponse.json(
        { error: 'missing_access_token', message: 'accessToken is required in request body' },
        { status: 400 }
      );
    }
    ...
    if (!MSG91_AUTH_KEY) {
      return NextResponse.json(
        { error: 'server_misconfigured', message: 'MSG91_AUTH_KEY is not set in environment' },
        { status: 500 }
      );
    }
    ...
    logger.info('[verify-access-token] calling provider', {
      className: 'api.msg91.verify-access-token',
      methodName: 'POST',
      url,
    });
    ...
    logger.info('[verify-access-token] provider response', {
      className: 'api.msg91.verify-access-token',
      methodName: 'POST',
      status: resp.status,
      ok: resp.ok,
    ...
    if (!resp.ok) {
      logger.warn('[verify-access-token] provider returned error', {
        className: 'api.msg91.verify-access-token',
        methodName: 'POST',
        status: resp.status,
        body: json,
    ...
    try {
      ...
      logger.info('[verify-access-token] extracted identifier', {
        className: 'api.msg91.verify-access-token',
        methodName: 'POST',
        found,
      });
      ...
      if (found) {
        ...
        try {
          ...
          logger.info('[verify-access-token] upserted user', {
            className: 'api.msg91.verify-access-token',
            methodName: 'POST',
            id: user.id,
            phone: user.phone,
          ...
          logger.info('[verify-access-token] setting cookie', {
            className: 'api.msg91.verify-access-token',
            methodName: 'POST',
            cookieSnippet: cookieValue.slice(0, 80),
          });
          res.headers.set('Set-Cookie', cookieValue);
          return res;
        } catch (e) {
          logger.warn('prisma upsert in verify-access-token failed', {
            className: 'api.msg91.verify-access-token',
            methodName: 'POST',
            error: e,
          });
      ...
      logger.warn('extracting identifier from provider response failed', {
        className: 'api.msg91.verify-access-token',
        methodName: 'POST',
        error: e,
      });
    ...
    logger.error('MSG91 verify-access-token error', {
      className: 'api.msg91.verify-access-token',
      methodName: 'POST',
      error: err,
    });
    return NextResponse.json(
      { error: formatErrorForResponse(err), message: 'Failed to verify access token' },
      { status: 500 }
    );
  }
}
