...
export async function POST(req: Request) {
  ...
  const pausedSetting = await prisma.systemSetting.findUnique({
    where: { key: 'HYDRATION_PAUSED' },
  });
  if (isSystemSettingEnabled(pausedSetting?.value)) {
    return NextResponse.json(
      {
        error: 'hydration_paused',
        message:
          'Hydration is globally paused (HYDRATION_PAUSED=true). Resume hydration in System Settings before enqueueing jobs.',
      },
      { status: 409 }
    );
  }
