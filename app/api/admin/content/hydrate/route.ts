...
export async function POST(req: Request) {
  ...
  if (!subjectId || !board || grade == null) {
    return NextResponse.json(
      { error: 'missing_fields', required: ['subjectId', 'board', 'grade'] },
      { status: 400 }
    );
  }
  ...
  if (!result.created) {
    ...
    if (result.reason === 'job_already_queued') {
      return NextResponse.json(
        {
          error: 'conflict',
          message: 'A hydration job is already pending or running for this subject.',
        },
        { status: 409 }
      );
    }
    if (result.reason === 'hydration_paused') {
      return NextResponse.json(
        { error: 'hydration_paused', message: 'Hydration is currently paused.' },
        { status: 503 }
      );
    }
