...
export async function POST(req: NextRequest) {
  ...
  try {
      ...
      users = [
        {
          ...
          subjects: (u.subjects || []) as string[],
        },
      ];
    } else {
      users = await prisma.user
        .findMany({ where: { board, grade, language } })
        .then((arr) =>
          arr.map((u) => ({
            id: u.id,
            board: u.board,
    ...
    for (const u of users) {
      const subjSet =
        subjects && Array.isArray(subjects) && subjects.length ? subjects : u.subjects || [];
      const where: any = {
        active: true,
        board: u.board || board,
        grade: u.grade || grade,
    ...
    return NextResponse.json(
      { error: 'seed_failed', message: String(e?.message || e) },
      { status: 500 }
    );
  }
}
