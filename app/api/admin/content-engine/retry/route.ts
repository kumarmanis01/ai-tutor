...
export async function POST(req: Request) {
  ...
  try {
    ...
    if ((old.attempts ?? 0) >= (old.maxAttempts ?? 3))
    ...
    if (!retryable.includes(old.jobType))
      return NextResponse.json({ error: 'Job type not retryable' }, { status: 400 });
    ...
    try {
      ...
      await prisma.outbox.create({
        data: {
          ...
          payload: { type: payloadType, payload: { jobId: created.id } },
          meta: { hydrationJobId: created.id, retriedFrom: old.id },
        },
      });
    } catch (e) {
    ...
    try {
      await prisma.jobExecutionLog.create({
        data: {
          ...
          meta: { retriedFrom: old.id, retryBy: session.user.id },
        },
      });
    } catch (e) {
      logger.warn('retry: failed to write jobExecutionLog', {
        error: String(e),
        jobId: created.id,
      });
    }
