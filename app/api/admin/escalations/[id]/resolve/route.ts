...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!resolutionType || !VALID_RESOLUTION_TYPES.has(resolutionType)) {
    return NextResponse.json(
      {
        error:
          'resolutionType must be one of: chunk_updated, misconception_added, prompt_fix, cached_answer',
      },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    try {
      ...
      logger.warn('[admin/escalations/resolve] notify_enqueue_failed', {
        escalationId: id,
    ...
    logger.warn('[admin/escalations/resolve] notify_import_failed', {
      escalationId: id,
      err: String((e as any)?.message ?? e),
    });
  }
