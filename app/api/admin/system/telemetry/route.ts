...
export async function GET(req: Request) {
  try {
    ...
    let bucket =
      bucketParam === 'minute' || bucketParam === 'hour'
        ? bucketParam
        : rangeMs <= MAX_MINUTE_RANGE_MS
          ? 'minute'
          : 'hour';
    ...
    if (bucket === 'minute') {
      ...
      for (const r of rows) {
        ...
        if (!series[seriesKey])
          series[seriesKey] = { key: r.key, dimensions: r.dimensions ?? null, points: [] };
        series[seriesKey].points.push({ ts: r.timestamp.toISOString(), value: r.value });
      }

      return NextResponse.json({
        range: { from: from.toISOString(), to: to.toISOString(), bucket: 'minute' },
        series,
        meta: responseMeta,
      });
    }
    ...
    for (const r of rows) {
      ...
      if (!series[seriesKey])
        series[seriesKey] = { key: r.key, dimensions: r.dimensions ?? null, points: [] };
      series[seriesKey].points.push({
        ts: new Date(r.bucket_ts).toISOString(),
        value: Number(r.value),
      });
    }

    return NextResponse.json({
      range: { from: from.toISOString(), to: to.toISOString(), bucket: 'hour' },
      series,
      meta: responseMeta,
    });
  } catch (err: any) {
