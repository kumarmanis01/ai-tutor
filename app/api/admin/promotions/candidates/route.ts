...
export async function POST(req: Request) {
  try {
    ...
    if (!scope || !scopeRefId || !regenerationJobId || !outputRef)
      return NextResponse.json({ error: 'missing_fields' }, { status: 400 });

    const store = makePromotionStore(prisma as any);
    try {
      const created = await store.createPromotionCandidate({
        scope,
        scopeRefId,
        regenerationJobId,
        outputRef,
      ...
      try {
        logAuditEvent(prisma as any, {
          action: 'PROMOTION_CANDIDATE_CREATED',
          entityId: created.id,
          actorId: session.user.id,
          metadata: { scope, scopeRefId },
        });
      } catch {}
