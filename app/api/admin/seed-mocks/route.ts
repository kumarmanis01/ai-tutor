...
export async function POST(req: NextRequest) {
  ...
  if (dryRun) {
    try {
      ...
      return NextResponse.json({ ok: true, dryRun: true, createdCount: res.created.length, preview: res.created.slice(0, 50) })
    } catch (err: unknown) {
  ...
  try {
    const enqueueRes = await enqueueSeedMocks({ minPer, subjectId, operatorId: (session?.user as any)?.id, dryRun: false })

    // create an audit row for traceability
    try {
      await prisma.auditLog.create({ data: { adminId: (session?.user as any)?.id, targetEntity: 'SeedMocks', targetId: subjectId ?? 'ALL', action: 'CONTENT_HYDRATE', details: { minPer, queuedBy: (session?.user as any)?.id, jobId: enqueueRes.jobId } } })
    } catch {}
