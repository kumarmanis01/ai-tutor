  ...
  try {
    ...
    const jobs = await Promise.all(
      execs.map(async (ex) => {
        ...
        const hydration = hydrationJobId
          ? await prisma.hydrationJob.findUnique({ where: { id: hydrationJobId } })
          : null;
        const latestLog = await prisma.jobExecutionLog.findFirst({
          where: { jobId: ex.id },
    ...
    return new Response(JSON.stringify({ error: formatErrorForResponse(err) }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
