...
export async function PATCH(
  ...
  if (typeof isAvailable !== 'boolean') {
    return NextResponse.json({ error: 'isAvailable_required', message: 'isAvailable must be a boolean' }, { status: 400 })
  }
  ...
  if (subject.isAvailable === isAvailable) {
    return NextResponse.json(
      { error: 'no_change', message: `Subject is already ${isAvailable ? 'available' : 'unavailable'}` },
      { status: 409 },
    )
  }
