...
export async function POST(req: Request) {
  ...
  if (!validPlanId) {
    const res = NextResponse.json(
      { code: 'INVALID_PLAN', message: 'Invalid plan' },
      { status: 400 }
    );
    logger.logAPI(req, res, { className: 'CreateSubscriptionAPI', methodName: 'POST' }, start);
    return res;
  ...
  if (planId === 'test_weekly') {
    ...
    if (!allowed) {
      const res = NextResponse.json(
        { code: 'TEST_PLAN_ACCESS_DENIED', message: 'Not authorised for test plan' },
        { status: 403 }
      );
  ...
  if (plan.internal && planId !== 'test_weekly') {
    const res = NextResponse.json(
      { code: 'PLAN_UNAVAILABLE', message: 'Plan not available' },
      { status: 403 }
    );
    logger.logAPI(req, res, { className: 'CreateSubscriptionAPI', methodName: 'POST' }, start);
    return res;
  ...
  try {
    ...
    const res = NextResponse.json(
      { code: 'PAYMENT_UNAVAILABLE', message: 'Payment service temporarily unavailable' },
      { status: 503 }
    );
    logger.logAPI(req, res, { className: 'CreateSubscriptionAPI', methodName: 'POST' }, start);
    return res;
  ...
  try {
    ...
    const res = NextResponse.json(
      { code: 'SUBSCRIPTION_CREATE_FAILED', message: 'Could not create subscription' },
      { status: 502 }
    );
    logger.logAPI(req, res, { className: 'CreateSubscriptionAPI', methodName: 'POST' }, start);
    return res;
