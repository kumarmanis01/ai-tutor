...
function parseBody(body: any): TutorTurnRequest | null {
  ...
  if (typeof itemDifficulty === 'number' && Number.isFinite(itemDifficulty))
    parsed.itemDifficulty = itemDifficulty;
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!userId) {
      const res = await streamSingleError(
        401,
        errorPayload('FEATURE_DISABLED', 'Unauthorized', false)
      );
    ...
    if (!allowed) {
      const res = await streamSingleError(
        429,
        errorPayload('RATE_LIMITED', 'Rate limited. Please wait a moment and try again.', true)
      );
      logger.logAPI(req, res, { className: 'TutorTurnAPI', methodName: 'POST' }, start);
      return res;
    }

    const bodyRaw = await req.json().catch((e) => {
      logger.warn('Failed to parse request JSON', {
        className: 'TutorTurnAPI',
        methodName: 'POST',
        error: e,
      });
      return null;
    ...
    if (!parsed) {
      const res = await streamSingleError(
        400,
        errorPayload('SESSION_NOT_FOUND', 'Invalid request body.', false)
      );
      logger.logAPI(req, res, { className: 'TutorTurnAPI', methodName: 'POST' }, start);
      return res;
    }
    ...
    if (!canUseAI) {
      const res = await streamSingleError(
        403,
        errorPayload('CONSENT_REQUIRED', 'AI interaction consent required', false)
      );
      logger.logAPI(req, res, { className: 'TutorTurnAPI', methodName: 'POST' }, start);
      return res;
    }
    ...
    try {
      ...
      if (code === 'RATE_LIMITED') {
        const res = await streamSingleError(
          403,
          errorPayload('RATE_LIMITED', 'Free limit reached. Please upgrade to continue.', false)
        );
        logger.logAPI(req, res, { className: 'TutorTurnAPI', methodName: 'POST' }, start);
        return res;
      }
    ...
    if (parsed.turnNumber < state.lastTurnNumber) {
      const res = await streamSingleError(
        409,
        errorPayload('SESSION_NOT_FOUND', 'Stale turn number.', true)
      );
      logger.logAPI(req, res, { className: 'TutorTurnAPI', methodName: 'POST' }, start);
      return res;
    }
    ...
    await setTutorSession(nextState).catch((e) => {
      logger.warn('setTutorSession failed', {
        className: 'TutorTurnAPI',
        methodName: 'POST',
        error: e,
      });
          ...
          const payload =
            code === 'AI_UNAVAILABLE'
              ...
              : code === 'FEATURE_DISABLED'
                ? errorPayload(
                    'FEATURE_DISABLED',
                    'AI Tutor is not enabled for your account.',
                    false
                  )
