...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (body?.explainStyle === null || body?.explainStyle === '') {
      ...
      if (!VALID.has(body.explainStyle))
        return NextResponse.json({ error: 'Invalid explainStyle' }, { status: 400 });
    ...
    logger.logAPI(
      req,
      res,
      { className: 'TutorSessionStyleAPI', methodName: 'POST', error: String(err) },
      start
    );
    return res;
  }
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const explainStyle =
      s && typeof (s as any).explainStyle === 'string' ? (s as any).explainStyle : null;
    ...
    logger.logAPI(
      req,
      res,
      { className: 'TutorSessionStyleAPI', methodName: 'GET', error: String(err) },
      start
    );
    return res;
  }
