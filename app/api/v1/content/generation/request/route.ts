...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (typeof body.topic !== 'string' || !body.topic.trim()) {
      return NextResponse.json({ code: 'INVALID_REQUEST', message: 'topic is required' }, { status: 400 })
    }
    if (typeof body.subject !== 'string' || !body.subject.trim()) {
      return NextResponse.json({ code: 'INVALID_REQUEST', message: 'subject is required' }, { status: 400 })
    }
    if (typeof body.grade !== 'number' || body.grade < 1 || body.grade > 12) {
      return NextResponse.json({ code: 'INVALID_REQUEST', message: 'grade must be 1-12' }, { status: 400 })
    }
    if (typeof body.board !== 'string' || !body.board.trim()) {
      return NextResponse.json({ code: 'INVALID_REQUEST', message: 'board is required' }, { status: 400 })
    }
  ...
  if (existingJobId) {
    ...
    logger.info('Content generation dedup: merged with existing job', { existingJobId, requesterId })
