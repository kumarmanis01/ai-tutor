...
export async function GET(
  ...
  if (!session?.user?.id) {
    return new Response(
      sseEvent('error', { message: 'Authentication required' }),
      {
        status: 401,
  ...
  if (!job) {
    return new Response(
      sseEvent('error', { message: 'Job not found' }),
      { status: 404, headers: { 'Content-Type': 'text/event-stream', 'Cache-Control': 'no-cache' } },
    )
  }
  ...
  if (!isAuthorized) {
    return new Response(
      sseEvent('error', { message: 'Access denied' }),
      { status: 403, headers: { 'Content-Type': 'text/event-stream', 'Cache-Control': 'no-cache' } },
    )
  }
