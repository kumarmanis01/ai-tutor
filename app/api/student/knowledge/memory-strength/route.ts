...
export async function POST(req: Request) {
  ...
  try {
    ...
    const chapterIds = Array.isArray(body?.chapterIds)
      ? body.chapterIds.filter((x: any) => typeof x === 'string')
      : [];
    ...
    for (const cid of chapterIds) {
      if (agg[cid] && agg[cid].count > 0)
        strengths[cid] = Math.round((agg[cid].sum / agg[cid].count) * 1000) / 1000;
    ...
    logger.logAPI(
      req,
      res,
      { className: 'StudentChapterMemoryStrengthAPI', methodName: 'POST' },
      start
    );
    return res;
  } catch (err) {
