export async function GET(req: Request, { params }: { params: Promise<{ courseId: string }> }) {
  ...
  try {
    const { validatePackageSize } =
      await import('../../../../../../../lib/safety/validatePackageSize');
    const max = Number(process.env.MAX_COURSEPACKAGE_EXPORT_BYTES ?? 5_000_000);
    const { ok, size } = validatePackageSize(row.json || {}, max);
    if (!ok) {
      return new Response(
        JSON.stringify({ error: 'Course package too large to export', size, max }),
  ...
  try {
    ...
    logAuditEvent(db, {
      ...
      entityType: 'COURSE',
      entityId: courseId,
      metadata: { courseId, packageId: row.id, tenantId: (row as any)?.tenantId ?? null },
    });
  } catch {
    // swallow
  }
