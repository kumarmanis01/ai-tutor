...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (queue) {
      try {
        await queue.addBulk(toCreate.map((ev) => ({ name: 'analytics.ingest', data: ev })));
      } catch (qErr) {
        logger.warn('analytics.endpoint: queue unavailable, falling back to direct write', {
          error: String(qErr),
        });
    ...
    logger.logAPI(
      req,
      res,
      { className: 'AnalyticsAPI', methodName: 'POST', error: String(err) },
      start
    );
    return res;
  }
...
async function writeDirect(
  ...
  try {
    ...
    logger.warn('analytics.endpoint: createMany failed, falling back to single inserts', {
      error: String(dbErr),
    });
    for (const r of rows) {
      try {
