...
export async function GET() {
  try {
    ...
    logger.error('GET /api/parent/settings error', {
      className: 'api.parent.settings',
      methodName: 'GET',
      error: err,
    });
...
export async function POST(req: Request) {
  try {
    ...
    if (
      inactivityThresholdDays !== undefined &&
      !VALID_THRESHOLDS.includes(inactivityThresholdDays)
    ) {
      return NextResponse.json(
        { error: 'invalid_inactivityThresholdDays: must be 2, 3, 5, or 7' },
        { status: 400 }
      );
    }
    ...
    if (inactivityThresholdDays !== undefined)
      updateData.inactivityThresholdDays = inactivityThresholdDays;
    ...
    if (childrenUpdates !== undefined) {
      ...
      for (const c of childrenUpdates) {
        ...
        const exclude =
          c?.excludeFromParentReport === undefined ? undefined : Boolean(c.excludeFromParentReport);
        ...
        try {
          ...
          updatedChildren.push({
            id: studentId,
            excludeFromParentReport: updated.excludeFromParentReport,
            inactivityOptOut: updated.inactivityOptOut,
          });
        } catch (err) {
          logger.warn('POST /api/parent/settings: update child preference failed', {
            parentId: userId,
            studentId,
            error: String(err),
          });
    ...
    logger.error('POST /api/parent/settings error', {
      className: 'api.parent.settings',
      methodName: 'POST',
      error: err,
    });
