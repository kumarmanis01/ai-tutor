...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!plan) {
      const res = NextResponse.json({ planId: null, message: 'Diagnostic in progress' }, { status: 202 })
      logger.logAPI(req, res, { className: CLASS_NAME, methodName: 'GET' }, start)
      return res
    }

    const completedCount = await prisma.learningPlanItem.count({ where: { planId: plan.id, status: 'COMPLETED' } })
