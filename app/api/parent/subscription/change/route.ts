...
export async function POST(req: Request) {
  ...
  try {
    ...
    const useExplicitFamily = Boolean(
      isFamily && familyPlan && typeof planId === 'string' && planId.startsWith('standard_')
    );
      ...
      await prisma.payment.create({
        data: {
          ...
          meta: {
            ...
            newPlanId: planId,
            effectiveDate: current.endDate?.toISOString(),
          },
        },
      });
      return NextResponse.json(
        { ok: true, scheduledEffectiveDate: current.endDate?.toISOString() },
        { status: 200 }
      );
    }
    ...
    if (current && current.startDate && current.endDate) {
      ...
      if (curPlan) {
        const pr = calculateProrationCredit({
          startDate: current.startDate!,
          endDate: current.endDate!,
          now,
          billedRupees: curPlan.billedRupees,
        });
      ...
      await prisma.$transaction(async (tx) => {
        ...
        const payment = await tx.payment.create({
          data: {
            ...
            meta: { changeType: 'upgrade', previousExpiry: current?.endDate?.toISOString() },
          },
        });
        ...
        if (current) {
          ...
          await tx.subscription.create({
            data: {
              userId: user.id,
              plan: childIds.length > 1 ? 'family' : 'individual',
              billingCycle: planId,
              startDate: now,
              endDate: expiry,
              active: true,
              childSlots: childIds.length,
              paymentId: payment.id,
            },
          });
        }
    ...
    const order = await client.orders.create({
      ...
      notes: {
        ...
        planId,
        changeType: 'upgrade',
        previousExpiry: current?.endDate?.toISOString() || '',
