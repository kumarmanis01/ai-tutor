...
export async function POST(req: Request) {
  ...
  if (!verifySignature(orderId, paymentId, signature)) {
    logger.error('Invalid Razorpay signature (parent.verify)', {
      event: 'parent.subscription.verify.bad_sig',
  ...
  try {
    if (client) {
      ...
      if (rzNotes?.childIds) {
        try {
          childIds = JSON.parse(rzNotes.childIds);
        } catch {
          childIds = [];
        }
      }
    ...
    logger.warn('Could not fetch Razorpay order notes', {
      event: 'parent.subscription.verify.fetch_notes',
      context: { userId, orderId },
      err,
    });
  }
  ...
  const suffix =
    typeof planId === 'string' && planId.includes('_') ? planId.split('_').slice(-1)[0] : planId;
  ...
  const useExplicitFamily = Boolean(
    _isFamily && resolvedFamilyPlan && typeof planId === 'string' && planId.startsWith('standard_')
  );
  ...
  try {
    ...
    try {
      _createdPayment = await prisma.$transaction(
        async (tx: Prisma.TransactionClient) => {
          ...
          await recordPaymentEvent(tx, {
            ...
            transactionId: paymentId,
            orderId,
            eventType: 'payment.parent_subscription_verified',
            amount: order.amount,
            status: 'success',
            payload: { planId, childIds },
          });
          ...
          try {
            const existing = await tx.subscription.findFirst({
              ...
              select: { startDate: true, endDate: true, paymentId: true, creditBalance: true },
            });
            if (existing) {
              const paid = existing.paymentId
            ...
            logger.warn('parent.verify proration failed', {
              event: 'parent.subscription.verify.proration',
              context: { userId, orderId },
              err,
            });
          ...
          const createdSub = await tx.subscription.create({
            data: {
              ...
              active: true,
              childSlots: _isFamily
                ? (appliedPlan?.childSlots ?? childIds?.length ?? 1)
                : (childIds?.length ?? 1),
              paymentId: payment.id,
              meta: { childIds },
            ...
            for (const sid of childIds) {
              ...
              await tx.freeTierUsage
                .upsert({
                  ...
                  create: { studentId: sid, periodStart: now, sessionsUsed: 0 },
                })
                .catch((err) => {
                  logger.warn('freeTierUsage.upsert failed (parent.verify)', {
                    event: 'parent.subscription.verify.upsert',
                    context: { sid },
                    error: String(err),
                  });
                });
      ...
      logger.error('Failed to activate parent subscription', {
        event: 'parent.subscription.verify.activate_error',
        context: { userId, orderId },
        err,
      });
    ...
    if (parent?.email) {
      try {
        const invoiceResult = await createInvoiceForPayment({
          ...
          studentId: childIds && childIds.length > 0 ? childIds[0] : undefined,
          amountPaise: order.amount,
          planLabel: appliedPlan?.label ?? '',
          billingCycle: appliedPlan?.perMonthDisplay ?? '',
        });
        await sendEmail({
          to: parent.email,
          ...
          html: paymentReceiptHtml({
            studentName: parent.name ?? 'Student',
            plan: appliedPlan?.label ?? '',
            amountRupees: order.amount ? order.amount / 100 : appliedPlan?.billedRupees,
            billingCycle: appliedPlan?.perMonthDisplay ?? '',
            renewalDate,
          }),
          ...
          logger.error('Receipt email failed (parent.verify)', {
            event: 'parent.subscription.verify.email_error',
            context: { userId },
            err,
          });
        });
      } catch (err) {
        logger.error('Invoice generation/email failed (parent.verify)', {
          event: 'parent.subscription.verify.invoice_error',
          context: { userId, orderId },
          err,
        });
      }
    ...
    if (parent?.phone) {
      ...
      sendSms(parent.phone, smsText).catch((err: unknown) => {
        logger.error('Receipt SMS failed (parent.verify)', {
          event: 'parent.subscription.verify.sms_error',
          context: { userId },
          err,
        });
      });
    ...
    logger.error('Unexpected error in parent subscription verify', {
      event: 'parent.subscription.verify.unexpected',
      context: { userId, orderId },
      err,
    });
