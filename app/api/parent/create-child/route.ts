...
export async function POST(req: Request) {
  ...
  try {
    ...
    const parentUser = await prisma.user.findUnique({
      where: { id: parentId },
      select: { language: true, name: true, email: true, phone: true },
    });
    ...
    const child = await prisma.$transaction(async (tx) => {
      ...
      const resolvedLanguage =
        normalizedMedium === 'hi' || normalizedMedium === 'hindi'
          ? 'hi'
          : normalizedMedium === 'en' || normalizedMedium === 'english'
            ? 'en'
            : defaultLanguage;
...
<p>Happy learning!<br/>Team Spinzy</p>`,
      ...
      if (parent?.phone) {
        await sendSms(
          parent.phone,
          `Hi ${parentName}! ${child.name}'s Spinzy account is ready. Log in to track their progress. - Team Spinzy`
        );
      }
    } catch (err) {
    ...
    logger.error('ParentCreateChildAPI failed', {
      className: 'ParentCreateChildAPI',
      methodName: 'POST',
      error: String(err),
    });
