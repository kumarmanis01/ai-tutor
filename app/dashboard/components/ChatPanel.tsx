...
/**
 * FILE OBJECTIVE:
 * - Mobile-optimized chat message display with clean bubbles and minimal chrome.
 *
 * LINKED UNIT TEST:
...
const ChatPanel: React.FC<ChatPanelProps> = ({ messages }) => {
  ...
  if (messages.length === 0) {
    return (
      <div className="bg-muted/30 dark:bg-slate-800/30 rounded-xl p-6 text-center">
        <div className="text-3xl mb-2">💬</div>
        <p className="text-sm text-muted-foreground">Ask anything to start learning!</p>
        <p className="text-xs text-muted-foreground mt-1">कुछ भी पूछें!</p>
      </div>
    );
  }

  return (
    <div className="bg-muted/20 dark:bg-slate-800/20 rounded-xl p-3">
      <div ref={scrollRef} className="space-y-3 max-h-72 overflow-y-auto">
        {messages.map((m) => {
                ...
                {!isUser && (
                  <div className="flex items-center gap-2 mt-1.5 px-1">
                    <button
                      ...
                      className={`p-1 rounded-md text-xs ${isPlaying ? 'text-red-500' : 'text-muted-foreground'}`}
                    >
                      {isPlaying ? '⏹' : '🔊'}
                    </button>
                    {m.language && (
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {m.suggestions.map((s, i) => (
                      <button
                        ...
                        className="px-2.5 py-1 bg-primary/10 hover:bg-primary/20 text-primary rounded-full text-xs active:scale-95"
                      >
                        {s}
                      </button>
                    ))}
