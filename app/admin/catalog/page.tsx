...
export default function CatalogAdminPage() {
  ...
  async function parsePdf() {
    if (!file) {
      setResult({ error: 'no_file', message: 'Select a PDF first' });
      return;
    }
    setLoading(true);
    setResult(null);
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Paste JSON array or JSONL of catalog entries. Each entry requires contentId, title, subject,
        board, grade, language.
      </p>
      <div className="mb-4 border rounded p-3">
        ...
        <p className="text-xs text-muted-foreground mt-2">
          We only extract headings to build metadata (titles, tags). No textbook content is stored.
        </p>
      </div>
      ...
      <section className="space-y-3">
        ...
        <div className="flex flex-col md:flex-row gap-3 items-start md:items-end">
          <input
            ...
            onChange={async (e) => {
              ...
              setParsing(true);
              try {
                const res = await fetch('/api/admin/catalog/parse-image', {
                  method: 'POST',
                  body: fd,
                });
                const data = await res.json();
          ...
          <div className="grid grid-cols-2 gap-3 w-full md:w-auto">
            <input
              ...
              onChange={(e) => setDefaults({ ...defaults, board: e.target.value })}
            />
            <input
              className="input input-bordered"
              placeholder="Grade"
        ...
        <p className="text-sm text-muted">
          Uses OCR to extract headings from images (no content stored).
        </p>
      </section>
      <textarea
        ...
        placeholder='[{"contentId":"cbse-6-math-ch1-1",...}] or JSONL per line'
      />
