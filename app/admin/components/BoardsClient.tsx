  ...
  if (error) {
    return (
      <div className="p-6 text-red-600">
        ...
        <button className="ml-3 underline text-sm" onClick={fetchBoards}>Retry</button>
      </div>
    );
  }
  ...
  return (
    <div className="p-6">
      ...
      <table className="min-w-full border text-sm">
        ...
        <tbody>
          {boards.length === 0 && (
            <tr><td colSpan={3} className="border px-4 py-4 text-center text-gray-400">No boards found</td></tr>
          )}
          {boards.map(board => (
            <tr key={board.id} className="hover:bg-gray-50">
              ...
              <td className="border px-4 py-2">
                <span className={`px-2 py-0.5 rounded text-xs ${board.isActive !== false ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
