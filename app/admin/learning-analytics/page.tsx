...
async function fetchSubjectReadiness() {
  const topicProgress = await prisma.studentTopicProgress
    .groupBy({
      by: ['topicId'],
      _avg: { mastery: true },
      _count: { _all: true },
  ...
  const topics = await prisma.topicDef
    .findMany({
...
async function fetchWeakTopics() {
  const weak = await prisma.studentTopicProgress
    .groupBy({
      by: ['topicId'],
      _avg: { mastery: true },
      _count: { _all: true },
...
function ReadinessBar({ name, grade, mastery }: { name: string; grade: number; mastery: number }) {
  const color = mastery >= 70 ? 'bg-[#1D9E75]' : mastery >= 40 ? 'bg-[#BA7517]' : 'bg-[#E24B4A]';
  const label =
      ...
      <div className="p-5 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <StatCard
            ...
            sub="last 7 days"
          />
          <StatCard label="Avg readiness" value={`${stats.avgReadiness}%`} sub="all students" />
          <StatCard
            label="Diagnostic done"
            value={`${stats.diagPct}%`}
            sub="of enrolled students"
          />
          <StatCard label="Avg streak" value={`${stats.avgStreak} days`} sub="daily streak" />
        </div>
                ...
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-900">
                    {['Topic', 'Subject', 'Grade', 'Avg mastery', 'Students'].map((h) => (
                      <th
                        key={h}
                        className="text-left px-4 py-2.5 font-medium text-gray-500 dark:text-gray-400 whitespace-nowrap"
                      >
                        {h}
                      </th>
                    ))}
