...
export default function RollbacksIndexPage() {
  const { data, error } = useSWR('/api/admin/content-engine/rollbacks', fetcher);

  if (error) return <div className="p-4 text-red-600">Failed to load rollbacks.</div>;
  if (!data) return <div className="p-4">Loading...</div>;

  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">Content Rollbacks</h1>
        ...
        <table className="min-w-full border text-sm">
          ...
          <tbody>
            {data.rollbacks?.length === 0 && (
              <tr><td colSpan={5} className="text-center p-4">No rollbacks found.</td></tr>
            )}
            {data.rollbacks?.map((log: any) => (
