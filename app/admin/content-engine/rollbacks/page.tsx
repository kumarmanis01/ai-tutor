...
function ActionBadge({ action }: { action: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${colors[actionLower] || 'bg-blue-100 text-blue-800'}`}
    >
...
function EntityBadge({ entityType }: { entityType: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${colors[typeLower] || 'bg-gray-100 text-gray-800'}`}
    >
      {entityType || 'Content'}
    </span>
  );
  ...
  return (
    <div className="text-center py-16">
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400">
        Content rollback history will appear here.
      </p>
    </div>
  );
}
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                History of content status changes and reversions
              </p>
            </div>
          </div>
        </div>
        ...
        {error && (
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-6 text-center">
            ...
            <p className="text-sm text-red-600 dark:text-red-400">
              Unable to fetch rollback history. Please try again.
            </p>
          </div>
        )}
