...
export default function ContentReviewPage() {
  ...
  const { data, isLoading, error } = useSWR(
    contentId ? `/api/admin/moderation/content/${contentId}` : null,
    fetcher
  );
  ...
  return (
    <div className="max-w-2xl mx-auto p-8">
      ...
      <div className="bg-white rounded shadow p-4 mb-6">
        <div className="mb-2">
          <span className="font-semibold">Board:</span> {content.board || '-'}
        </div>
        <div className="mb-2">
          <span className="font-semibold">Class:</span> {content.classLevel || '-'}
        </div>
        <div className="mb-2">
          <span className="font-semibold">Subject:</span> {content.subject || '-'}
        </div>
        <div className="mb-2">
          <span className="font-semibold">Topic:</span> {content.topicName || '-'}
        </div>
        ...
        <div className="mb-2">
          ...
          <span
            className={`px-2 py-1 rounded text-xs font-semibold ${status === JobStatus.Pending ? 'bg-yellow-100 text-yellow-800' : status === ApprovalStatus.Approved ? 'bg-green-100 text-green-800' : status === ApprovalStatus.Rejected ? 'bg-red-100 text-red-800' : 'bg-gray-200 text-gray-800'}`}
          >
            {status}
          </span>
        </div>
      ...
      <div className="mb-6">
        ...
        <div className="p-4 bg-gray-50 rounded border text-sm whitespace-pre-wrap min-h-[120px]">
          {content.body}
        </div>
      </div>
      {/* Moderation Actions */}
