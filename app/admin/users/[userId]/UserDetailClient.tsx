...
function EditModal({
  ...
  user: UserDetail;
  onSave: (data: { name?: string; subscriptionStatus?: string; schoolName?: string }) => void;
  onCancel: () => void;
  busy: boolean;
}) {
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4">
        ...
        <div className="space-y-3">
          <label className="block">
            ...
            <input
              ...
              className="mt-1 w-full border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-[#534AB7]"
            />
          </label>

          <label className="block">
            <span className="text-[11px] text-gray-500 dark:text-gray-400">School</span>
            <input
              value={schoolName}
          ...
          <label className="block">
            <span className="text-[11px] text-gray-500 dark:text-gray-400">Subscription</span>
            <select
        ...
        <div className="flex gap-2 justify-end">
          ...
          <button
            ...
            disabled={busy}
            className="px-4 py-2 text-[12px] bg-[#534AB7] text-white rounded-xl min-h-[36px] hover:bg-[#3C3489] disabled:opacity-50 transition-colors"
          >
            {busy ? 'Saving...' : 'Save'}
          </button>
        </div>
...
export function UserDetailClient({ user: initial }: { user: UserDetail }) {
  ...
  async function callApi(path: string, method: string, body?: object) {
    setBusy(true);
    try {
      const res = await fetch(`/api/v1/admin/platform-users/${user.id}${path}`, {
        method,
  ...
  return (
    <>
      ...
      {/* Modals */}
      {modal === 'edit' && (
        <EditModal user={user} onSave={handleSave} onCancel={() => setModal(null)} busy={busy} />
      )}
      {modal === 'block' && (
        <ConfirmModal
          title="Block user"
          message={`Block ${user.name ?? user.email ?? user.id}? They will not be able to log in.`}
      ...
      {modal === 'unblock' && (
        <ConfirmModal
          title="Unblock user"
          message={`Restore access for ${user.name ?? user.email ?? user.id}?`}
      ...
      {modal === 'delete' && (
        <ConfirmModal
          title="Delete user"
          message={`Queue deletion for ${user.name ?? user.email ?? user.id}? This marks the account for permanent removal.`}
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5 space-y-4">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-base font-semibold text-gray-900 dark:text-gray-100">
                {user.name ?? <em className="text-gray-400 font-normal">No name</em>}
              </h1>
          ...
          <span
            className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${statusBadge(user.accountStatus)}`}
          >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          ...
          <InfoRow label="Age" value={user.age ? `${user.age}y${user.isAdult ? ' (adult)' : ''}` : '--'} />
          ...
          <InfoRow
            label="Last session"
            value={user.lastSessionDate ? new Date(user.lastSessionDate).toLocaleDateString('en-IN') : 'Never'}
          />
        ...
        <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-100 dark:border-gray-800">
          <button
            ...
            className="px-3 py-1.5 text-[11px] rounded-lg border border-[#534AB7] bg-[#EEEDFE] text-[#3C3489] hover:bg-[#e0defe] min-h-[32px] transition-colors disabled:opacity-40"
          >
            Edit
          </button>
