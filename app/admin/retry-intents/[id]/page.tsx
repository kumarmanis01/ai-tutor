...
export default async function Page({ params }: Props) {
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ marginBottom: 8 }}>
        <strong>Source Job:</strong> {intent.sourceJobId}
      </div>
      <div style={{ marginBottom: 8 }}>
        <strong>Reason:</strong> {intent.reasonCode} -- {intent.reasonText}
      </div>
      <div style={{ marginBottom: 8 }}>
        <strong>Approved By:</strong> {intent.approvedBy}{' '}
        {intent.approvedAt ? `on ${new Date(intent.approvedAt).toLocaleString()}` : ''}
      </div>
...
function AuditTrailClient({ logs }: { logs: any[] }) {
  if (!logs || logs.length === 0)
    return <div className="text-sm text-gray-600">No audit events found.</div>;
  return (
    <div className="mt-2">
          ...
          {logs.map((l: any) => (
            <tr key={l.id} className="border-t">
              ...
              <td className="border px-3 py-1">
                <ReadOnlyJsonViewer data={l.details ?? {}} collapsedByDefault={true} />
              </td>
            </tr>
          ))}
        </tbody>
