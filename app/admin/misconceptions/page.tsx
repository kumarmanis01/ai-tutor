...
export default function AdminMisconceptionsPage() {
  ...
  async function save(edit: Misconception) {
    ...
    if (!Array.isArray(edit.triggerPatterns) || edit.triggerPatterns.length < 2) {
      setError('At least 2 trigger patterns required');
      return;
    }
    setError(null);
    ...
    <div>
      ...
      <div className="p-6">
        <div className="flex items-center gap-3 mb-4">
          <input
            placeholder="Search name, description or correction"
            value={q}
            onChange={(e) => {
              setQ(e.target.value);
              setPage(1);
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {rows.map((r) => (
              <div
                key={r.id}
                className="bg-white dark:bg-gray-900 border rounded p-3 flex justify-between"
              >
                <div>
                  <div className="font-medium">{r.name}</div>
                  <div className="text-sm text-gray-500">
                    {r.subjectId} • {r.conceptId}
                  </div>
                  <div className="mt-2 text-sm">{r.description}</div>
                </div>
                <div className="ml-4 flex items-start gap-2">
                  <button
                    className="px-3 py-1 bg-[#534AB7] text-white rounded"
        ...
        {editing && (
          <div className="fixed inset-0 bg-black/30 flex items-center justify-center">
            <div className="bg-white dark:bg-gray-900 border rounded-lg p-6 w-[800px] max-w-full">
              ...
              <textarea
                className="w-full border rounded px-2 py-1 mb-2"
                value={editing.correction}
                onChange={(e) => setEditing({ ...editing, correction: e.target.value })}
              />
              ...
              <input
                ...
                value={editing.triggerPatterns.join(', ')}
                onChange={(e) =>
                  setEditing({
                    ...editing,
                    triggerPatterns: e.target.value.split(',').map((s) => s.trim()),
                  })
                }
              />
