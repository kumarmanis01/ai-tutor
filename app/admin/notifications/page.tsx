...
function statusBadge(status: string, failedCount: number) {
  if (status === 'sent' && failedCount === 0) {
    return (
      <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full bg-[#EAF3DE] text-[#27500A]">
        Sent
      </span>
    );
  }
  if (status === 'sent' && failedCount > 0) {
    return (
      <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full bg-[#FAEEDA] text-[#633806]">
  ...
  if (status === 'queued') {
    return (
      <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full bg-[#E6F1FB] text-[#0C447C]">
        Queued
      </span>
    );
  }
  return (
    <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full bg-[#FCEBEB] text-[#791F1F]">
      Failed
    </span>
  );
}
...
export default async function NotificationsPage() {
  ...
  const [allStudents, inactive7d, grade10, paidSubscribers, studentsWithDiag, recentLogs] =
    await Promise.all([
              ...
              <table className="w-full text-[11px]">
                ...
                <tbody>
                  {recentLogs.map((log) => (
                    ...
                    >
                      <td className="px-3 py-2.5 max-w-[180px] truncate text-gray-700 dark:text-gray-300">
                        {log.title}
                      </td>
