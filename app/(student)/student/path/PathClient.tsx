...
import { AppHeader, BottomNav, EmptyState, SubjectChip, Bar } from '@/components/ui'
...
export default function PathClient({ initialData }: { initialData: PathData }) {
      ...
      <div className="px-4 pb-6 relative">
        ...
        <div className="flex flex-col gap-3">
          {filtered.map(topic => {
            ...
            const isNeutral = topic.status === 'upcoming' || topic.status === 'locked'
            return (
              <div key={topic.id} className="flex gap-[14px] relative z-[1]">
                <div className="shrink-0 w-[38px] flex justify-center pt-4">
                  <div
                    className={[dotSize, 'rounded-full flex items-center justify-center text-white text-[12px]'].join(' ')}
                    style={{
                      background: isNeutral ? 'var(--surface)' : dotColor,
                      border: `3px solid ${isNeutral ? 'var(--border)' : dotColor}`,
                    }}
                  >
