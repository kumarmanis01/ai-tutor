...
export default async function Page({ params }: Props) {
  ...
  try {
    const res = await fetch(`${baseUrl}/api/learn/courses/${courseId}/lessons/${index}`, {
      cache: 'no-store',
    });
    if (res.ok) {
  ...
  if (!lesson) {
    return (
      <div style={{ padding: 16, maxWidth: 600, margin: '0 auto' }}>
        <Link href={`/learn/${courseId}`} style={{ fontSize: 14, color: '#0070f3' }}>
          ← Back to course
        </Link>
  ...
  const idx = lessons.findIndex(
    (l) => Number(l.lessonIndex) === Number(index) || (l.id && l.id === lesson?.id)
  );
