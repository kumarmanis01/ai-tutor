...
const ChatPanel: React.FC<ChatPanelProps> = ({ messages }) => {
    ...
    try {
      messages.forEach((m) => {
        if (m.from === 'ai' && m.suggestions?.length) {
          try {
            analyticsClient.trackEvent('suggestion.shown', {
              messageId: m.id,
    ...
    return (
      <div className="bg-muted/30 dark:bg-slate-800/30 rounded-xl p-6 lg:p-10 text-center">
        ...
        <p className="text-sm lg:text-base text-muted-foreground">
          Ask anything to start learning!
        </p>
  ...
  return (
    <div className="bg-muted/20 dark:bg-slate-800/20 rounded-xl p-3 lg:p-4">
      <div
        ref={scrollRef}
        className="space-y-3 lg:space-y-4 max-h-72 lg:max-h-[500px] xl:max-h-[600px] overflow-y-auto"
      >
        {messages.map((m) => {
          ...
          return (
            <div key={m.id} className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[85%] lg:max-w-[75%] ${isUser ? 'order-1' : ''}`}>
                {/* Message bubble */}
                <div
                  className={`px-3 py-2 lg:px-4 lg:py-3 rounded-2xl text-sm lg:text-base ${
                    isUser
                  ...
                  <div className="flex flex-wrap gap-1.5 lg:gap-2 mt-2 lg:mt-3">
                    {m.suggestions.map((s, i) => (
                      <button
                        ...
                        onClick={() => {
                          try {
                            analyticsClient.trackEvent('suggestion.clicked', { suggestion: s });
                          } catch {}
                          try {
                            window.dispatchEvent(
                              new CustomEvent('chatSuggestionPicked', { detail: { suggestion: s } })
                            );
                          } catch {}
                        }}
