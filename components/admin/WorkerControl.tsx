...
export default function WorkerControl() {
  ...
  async function fetchWorkers() {
    try {
      ...
      logger.warn('fetchWorkers failed', {
        component: 'WorkerControl',
        methodName: 'fetchWorkers',
        error: String(err),
      });
    }
  }
  ...
  async function copyToClipboard(text: string) {
    try {
      ...
      logger.warn('copyToClipboard failed', {
        component: 'WorkerControl',
  ...
  return (
    <div className="p-4 bg-white rounded shadow-sm max-w-3xl">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <button onClick={fetchWorkers} className="px-3 py-1 border rounded">
          Refresh
        </button>
      </div>
        ...
        <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded flex items-start justify-between">
          <div>
            <div className="text-sm">
              Started: <strong>{result.type}</strong>
            </div>
            <div className="text-sm">
              Lifecycle ID: <code className="break-all">{result.lifecycleId}</code>
            </div>
          </div>
          <div className="flex flex-col gap-2">
            <button
              onClick={() => copyToClipboard(result.lifecycleId)}
            ...
            <div key={w.id} className="p-2 border rounded flex items-center justify-between">
              <div className="text-sm">
                ...
                <div className="text-xs text-gray-500">
                  id: <code className="break-all">{w.id}</code>
                </div>
                <div className="text-xs text-gray-500">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="bg-white rounded shadow-lg p-4 z-10 max-w-md w-full">
            ...
            <p className="text-sm text-gray-700 mb-3">
              Are you sure you want to stop worker <code className="break-all">{confirmId}</code> ?
            </p>
            <div className="flex items-center gap-2 mb-3">
              <input
                id="drain"
                type="checkbox"
                checked={confirmDrain}
                onChange={(e) => setConfirmDrain(e.target.checked)}
              />
              <label htmlFor="drain" className="text-sm text-gray-600">
                Drain (allow current job to finish)
              </label>
            </div>
