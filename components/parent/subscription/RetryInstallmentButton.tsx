...
const STRINGS: Record<string, any> = {
  en: {
    ...
    confirmBody: 'Do you want to retry charging this installment? We will attempt to process the payment immediately.',
  ...
  hi: {
    ...
    confirmBody: 'क्या आप इस किस्त का भुगतान फिर से करने का प्रयास करना चाहते हैं? हम तुरंत भुगतान करने का प्रयास करेंगे।',
...
function detectLang() {
  try {
    if (typeof navigator !== 'undefined') {
      const locale = navigator.language || (document.documentElement && document.documentElement.lang) || 'en'
...
export default function RetryInstallmentButton({ installmentId }: Props) {
  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)} aria-haspopup="dialog" aria-expanded={open} className="inline-block rounded-md bg-indigo-600 px-3 py-1 text-white text-xs">
        {t.confirmButton}
      </button>
        ...
        <div role="dialog" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-md p-4 max-w-sm w-full" role="document">
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button type="button" onClick={() => setOpen(false)} className="px-3 py-1 rounded bg-gray-100 text-sm">{t.cancelButton}</button>
              <button type="button" onClick={onConfirm} disabled={loading} className="px-3 py-1 rounded bg-indigo-600 text-white text-sm">{t.confirmButton}</button>
            </div>
          </div>
        </div>
      )}
