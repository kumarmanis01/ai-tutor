...
export default function ParentSettings() {
  ...
  const [children, setChildren] = useState<
    Array<{
      ...
      excludeFromParentReport?: boolean;
      inactivityOptOut?: boolean;
    }>
  >([]);
  ...
  async function togglePause(
    studentId: string,
    pause: boolean,
    pausedUntilParam?: string | null,
    pauseReasonParam?: string | null
  ) {
    setSaving(true);
    setMessage(null);
    try {
          ...
          c.map((ch) =>
            ch.id === studentId
              ? {
                  ...ch,
                  isPaused: pause,
                  pausedUntil: data.link?.pausedUntil ?? pausedUntil ?? null,
                  pauseReason: data.link?.pauseReason ?? pauseReason,
                }
              : ch
          )
        );
  ...
  async function _confirmPauseDialog() {
    ...
    const d = pauseDialogDate
      ? new Date(pauseDialogDate + 'T00:00:00')
  ...
  async function toggleExclude(studentId: string, exclude: boolean) {
    ...
    try {
      ...
      if (res.ok && data.ok) {
        setChildren((c) =>
          c.map((ch) => (ch.id === studentId ? { ...ch, excludeFromParentReport: exclude } : ch))
        );
  ...
  async function toggleChildInactivity(studentId: string, optOut: boolean) {
    ...
    try {
      ...
      if (res.ok && data.ok) {
        setChildren((c) =>
          c.map((ch) => (ch.id === studentId ? { ...ch, inactivityOptOut: optOut } : ch))
        );
        setMessage(
          optOut ? 'Child muted for inactivity alerts' : 'Child will receive inactivity alerts'
        );
      } else {
        setMessage('Action failed');
      }
    ...
    <div className="mx-auto max-w-md p-4 space-y-4">
      ...
      <label className="flex items-center gap-2">
        <input
          ...
          onChange={(e) => setProfile({ ...profile, digestOptOut: e.target.checked })}
        />
      ...
      <label className="flex items-center gap-2">
        <input
          type="checkbox"
          checked={profile.inactivityOptOut ?? false}
          onChange={(e) => setProfile({ ...profile, inactivityOptOut: e.target.checked })}
        />
      ...
      <div>
        ...
        <p className="text-xs text-gray-500 mt-1">
          Parents will be notified if a child hasn't had a study session for the selected number of
          days.
        </p>
      </div>

      <div>
        <label htmlFor="digestDay" className="text-sm block mb-1">
          Delivery day
        </label>
        <select
          id="digestDay"
          value={profile.digestDay}
          onChange={(e) => setProfile({ ...profile, digestDay: e.target.value })}
              ...
              <div key={child.id}>
                <div className="flex items-center justify-between rounded border p-2">
                  <div>
                    ...
                    {child.grade && (
                      <div className="text-xs text-gray-500">Grade {child.grade}</div>
                    )}
                    ...
                    {child.isPaused && (
                      <div className="text-xs text-amber-700">
                        Paused until {child.pausedUntil ?? 'indefinitely'}
                      </div>
                    )}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() =>
                          child.isPaused ? togglePause(child.id, false) : openPauseDialog(child.id)
                        }
                        disabled={saving}
                      ...
                      <label className="flex items-center gap-2 text-sm">
                        <input
                          type="checkbox"
                          checked={child.excludeFromParentReport ?? false}
                          onChange={(e) => toggleExclude(child.id, e.target.checked)}
                        />
                      ...
                      <label className="flex items-center gap-2 text-sm ml-2">
                        <input
                          type="checkbox"
                          checked={child.inactivityOptOut ?? false}
                          onChange={(e) => toggleChildInactivity(child.id, e.target.checked)}
                        />
      ...
      <div className="flex items-center gap-2">
        <label htmlFor="digestTime" className="text-sm block mb-1">
          Delivery time
        </label>
        <input
          id="digestTime"
      ...
      <div>
        ...
        <div className="flex gap-2">
          <input
            ...
            onChange={(e) => setProfile({ ...profile, digestTimezone: e.target.value || null })}
      ...
      <div className="flex items-center gap-2">
        <button
          onClick={save}
          disabled={saving}
          className="rounded bg-[#534AB7] px-4 py-2 text-white min-h-[44px] min-w-[44px]"
        >
          {saving ? 'Saving...' : 'Save'}
        </button>
