        ...
        try {
          ...
          if (data && typeof data === 'object') {
            for (const k of tokenKeys) {
              if ((data as any)[k]) {
                token = String((data as any)[k]);
                break;
              }
            }
            if (!token && (data as any).data && typeof (data as any).data === 'object') {
              for (const k of tokenKeys) {
                if ((data as any).data[k]) {
                  token = String((data as any).data[k]);
                  break;
                }
              }
            }
            ...
            if (!phone && (data as any).data && typeof (data as any).data === 'object') {
              for (const k of phoneKeys) {
                if ((data as any).data[k]) {
                  phone = String((data as any).data[k]);
                  break;
                }
              }
            }
          ...
          logger.info('[OtpProvider] success payload', {
            className: 'OtpProviderForm',
            methodName: 'success',
            raw: data,
            token,
            phone,
          ...
          if (autoVerify && token) {
            try {
              logger.info('[OtpProvider] autoVerify sending token to server', {
                className: 'OtpProviderForm',
                methodName: 'success',
              ...
              logger.info('[OtpProvider] verify response', {
                className: 'OtpProviderForm',
                methodName: 'verify',
                status: resp.status,
                ok: resp.ok,
              ...
              logger.warn('auto verify failed', {
                className: 'OtpProviderForm',
                methodName: 'verify',
                error: String(e),
              });
            ...
            logger.warn('[OtpProvider] autoVerify enabled but token not found in payload', {
              className: 'OtpProviderForm',
              methodName: 'success',
              raw: data,
            });
    ...
    const loadProvider = () => {
      ...
      if ((window as any).initSendOTP) {
        try {
          ...
          logger.error('initSendOTP failed', {
            className: 'OtpProviderForm',
            methodName: 'loadProvider',
            error: String(e),
          });
          onFailure?.(e);
        } finally {
      ...
      script.onload = () => {
        try {
          ...
          if ((window as any).initSendOTP) {
            ...
            logger.warn('OTP provider script loaded but initSendOTP not found', {
              className: 'OtpProviderForm',
              methodName: 'script.onload',
            });
            onFailure?.(new Error('initSendOTP not found after script load'));
          }
        } catch (e) {
          logger.error('initSendOTP call failed', {
            className: 'OtpProviderForm',
            methodName: 'script.onload',
            error: String(e),
          });
      ...
      script.onerror = (e) => {
        logger.error('Failed to load OTP provider script', {
          className: 'OtpProviderForm',
          methodName: 'script.onerror',
    ...
    useImperativeHandle(
      ...
      () => ({
        callMethod: (method: string, ...args: any[]) => {
          ...
          const provider =
            (window as any).MSG91 || (window as any).otpProvider || (window as any).OtpProvider;
    ...
    return (
      <div className="w-full max-w-lg mx-auto">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              try {
                ...
                logger.warn('onIdentifierChange callback errored', {
                  className: 'OtpProviderForm',
        ...
        <p className="mt-2 text-xs text-muted-foreground">
          We will initialize and attempt to send an One Time Password (OTP) to the number above.
        </p>
      </div>
    );
  }
