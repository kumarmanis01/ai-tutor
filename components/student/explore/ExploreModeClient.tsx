    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 pb-10">
      ...
      {consent.status === 'EXPIRED' && (
        <div className="mx-4 mt-4 p-6 rounded-2xl bg-[#FAEEDA] dark:bg-amber-900 text-center">
          ...
          <Link
            href="/register"
            className="mt-4 inline-block min-h-[44px] px-6 py-2 rounded-xl bg-[#BA7517] text-white font-semibold text-sm"
          >
            Send New Request
          </Link>
        </div>
        ...
        {topicsLoading ? (
          <div className="flex flex-col gap-3">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-20 rounded-2xl bg-gray-200 dark:bg-slate-700 animate-pulse"
              />
            ))}
          </div>
        ) : topics.length > 0 ? (
            ...
            {topics.map((t) => (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-xs bg-[#534AB7] text-white px-2 py-0.5 rounded-full">
                    Free Preview
                  </span>
                </div>
                {t.sampleNote && (
