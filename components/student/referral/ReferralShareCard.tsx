 ...
 * - 2026-05-07T00:00:00Z | copilot | fetch referral only on explicit copy/share user actions
 */

import { useState, useCallback } from 'react';

type State = 'idle' | 'loading' | 'error' | 'populated';
...
export default function ReferralShareCard() {
  ...
  const fetchCode = useCallback(async (): Promise<ReferralData | null> => {
    if (data) return data;
    ...
    try {
      ...
      const referralData = { code: json.code, url: json.url };
  ...
  async function handleCopy() {
    const referralData = await fetchCode();
    if (!referralData) return;

    const shareText = `Join me on Spinzy AI Tutor -- get 20% off your first month! Sign up here: ${referralData.url}`;
  ...
  async function handleWhatsAppShare() {
    const referralData = await fetchCode();
    if (!referralData || typeof window === 'undefined') return;

    const text = encodeURIComponent(
      `Join me on Spinzy AI Tutor -- get 20% off your first month! Sign up here: ${referralData.url}`,
    );
    ...
    <div className="rounded-2xl border border-[#534AB7]/20 bg-[#EEEDFE] dark:bg-[#534AB7]/10 p-5">
      ...
      <div className="flex items-center gap-2 mb-4 bg-white dark:bg-slate-800 rounded-xl px-3 py-2 border border-[#534AB7]/20">
        <span className="flex-1 text-sm font-mono font-semibold text-gray-800 dark:text-gray-100 tracking-widest">
          {data?.code ?? 'Tap Copy or Share to generate your code'}
        </span>
        <span className="text-xs text-gray-400 dark:text-gray-500">Your invite code</span>
      </div>

      {state === 'error' && (
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{"Couldn't load invite link. Tap copy or share to retry."}</p>
      )}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        >
          {copied ? (
            ...
            <>
              ...
              {state === 'loading' ? 'Generating invite...' : 'Copy invite link'}
            </>
          )}
        </button>
        ...
        >
          ...
          {state === 'loading' ? 'Generating invite...' : 'Share on WhatsApp'}
        </button>
      </div>
    </div>
  );
