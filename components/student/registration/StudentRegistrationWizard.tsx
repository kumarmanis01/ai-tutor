 ...
 * - 2026-04-24T12:00:00Z | copilot | replace <a> with Link; call register API for adults; use canonical board slugs
 */
...
/**
 * Boards use canonical slug values that match Board.slug in the database.
 * The label is displayed to the user; the value is sent to the backend.
 */
const BOARDS = [
...
export default function StudentRegistrationWizard({ onComplete }: Props) {
  ...
  if (step === 'role') {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 dark:bg-slate-950 p-6 gap-6">
        ...
        {/* Use Link for client-side navigation — repo convention (no full reload). */}
  ...
  if (step === 'profile') {
    ...
    /** Adults call the register API on this step before advancing to success. */
    ...
    if (isAdult || success?.isAdult) {
      return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 dark:bg-slate-950 p-6 gap-6">
          ...
          {/* Use Link for internal navigation — avoids full page reload. */}
          <Link
