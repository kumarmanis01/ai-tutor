...
export default function StudentRegistrationWizard({ onComplete }: Props) {
  ...
  if (step === 'role') {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 dark:bg-slate-950 p-6 gap-6">
        ...
        <Link href="/login" className="text-[#534AB7] text-sm underline">
          {"I'm a Parent"}
        </Link>
      </div>
    )
  ...
  if (step === 'age') {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 dark:bg-slate-950 p-6 gap-6">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 text-center">We need this to comply with India's data protection laws.</p>
        <input
          type="date"
      ...
      if (isAdult) {
        ...
        try {
          const res = await fetch('/api/v1/students/register', {
            ...
            body: JSON.stringify({ name, dateOfBirth: dob, grade, board }),
          })
          ...
          const sd: SuccessData = { isAdult: true, exploreToken: null, name, contactMask: null }
          setSuccess(sd)
          setStep('success')
          onComplete?.(sd)
        } catch (e) {
          logger.warn('StudentRegistrationWizard: adult register fetch failed', { error: String(e) })
  ...
  if (step === 'parent') {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 dark:bg-slate-950 p-6 gap-4">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 text-center">Who should we ask for permission?</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400 text-center">Indian law requires parental consent for learners under 18. We will send a quick approval message.</p>
  ...
  if (step === 'success') {
    if (isAdult || success?.isAdult) {
      return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 dark:bg-slate-950 p-6 gap-6">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-center">You are all set. Let us find out where you stand with a quick diagnostic quiz.</p>
          <Link href="/student/diagnostic" className="w-full max-w-xs min-h-[44px] rounded-2xl bg-[#534AB7] text-white font-bold text-lg flex items-center justify-center">Take Diagnostic</Link>
          <Link href="/dashboard" className="text-[#534AB7] text-sm underline">Skip for now</Link>
        </div>
      )
    }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 dark:bg-slate-950 p-6 gap-6">
        ...
        <Link href={`/explore?token=${encodeURIComponent(success?.exploreToken ?? '')}`} className="w-full max-w-xs min-h-[44px] rounded-2xl bg-[#534AB7] text-white font-bold text-lg flex items-center justify-center">Start Exploring</Link>
        <button className="text-[#534AB7] text-sm underline" onClick={() => { setStep('parent'); setParentContact('') }}>Send a different contact</button>
      </div>
    )
  }
