...
export default function RevisionFlow({ cards, totalDue }: RevisionFlowProps) {
  ...
  const handleNext = useCallback(() => {
    ...
    if (lastResult) {
      ...
      const conceptCorrect = results.filter(
        (r) => r.card.conceptId === lastResult.card.conceptId && r.isCorrect
      ).length;
  ...
  if (capReached || (isOverCap && !submitted && !done)) {
    return (
      <div className="fixed inset-0 z-[100] bg-white dark:bg-gray-950 flex flex-col items-center justify-center px-4 py-12 text-center">
        ...
        <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Daily cap reached.</h2>
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-6 max-w-sm mx-auto w-full">
          ...
          {isGood ? (
            <div className="w-full rounded-2xl bg-[#EAF3DE] dark:bg-[#1D9E75]/10 p-5 text-center">
              <p className="text-base font-semibold text-[#1D9E75] mb-1">Great work!</p>
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-white dark:bg-gray-950 flex flex-col">
      ...
      <div className="sticky top-0 bg-white dark:bg-gray-950 border-b border-gray-100 dark:border-slate-800 px-4 py-3 flex items-center justify-between min-h-[44px] z-10">
        <h1 className="text-sm font-semibold text-gray-900 dark:text-white">Revision cards</h1>
        <div className="flex flex-col items-end gap-0.5">
          <span className="text-xs text-gray-500 dark:text-gray-400">
            {totalDue} due · {DAILY_CAP_MINUTES} min cap
          </span>
          {nearCap && (
            <span className="text-[10px] text-[#BA7517] font-medium">⏰ Approaching daily cap</span>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-4">
        ...
        {/* Question text */}
        <p className="text-[15px] leading-7 text-gray-800 dark:text-gray-100">{card.prompt}</p>
        ...
        <div className="flex flex-col gap-3" role="radiogroup" aria-label="Answer options">
          {card.choices.map((choice, idx) => {
            ...
            if (!submitted) {
              ...
              optionClass +=
                ' border-[#E24B4A] bg-[#FCEBEB] dark:bg-[#E24B4A]/10 text-[#E24B4A] font-medium';
            } else {
              optionClass +=
                ' border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900 text-gray-400 dark:text-gray-500';
            }
            ...
            return (
              ...
              >
                ...
                {submitted && isCorrectChoice && (
                  <span className="ml-2 text-[#1D9E75]" aria-hidden="true">
                    ✓
                  </span>
                )}
              </button>
        ...
        {submitted && (
          <div
            className={[
              'rounded-2xl p-4',
              isCorrect ? 'bg-[#EAF3DE] dark:bg-[#1D9E75]/10' : 'bg-[#FCEBEB] dark:bg-[#E24B4A]/10',
            ].join(' ')}
            role="alert"
          >
            {isCorrect ? (
