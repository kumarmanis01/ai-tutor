...
const contentGenerationLinks: NavItem[] = [
  ...
  { href: '/admin/content-engine/hydrate-all', label: 'Hydrate All', icon: 'Rocket', badge: 'New', badgeColor: 'bg-green-500' },
];
          ...
          {items.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.badge && (
                  <span className={`text-xs px-2 py-0.5 rounded-full text-white ${item.badgeColor || 'bg-blue-500'}`}>
                    {item.badge}
                  </span>
                )}
              </Link>
...
function EnvironmentBadge() {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-xs font-semibold text-gray-600 dark:text-gray-300">AI EdTech Platform</span>
    </div>
  );
}
  ...
  return (
    <aside className="hidden md:flex flex-col w-72 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 h-full">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        ...
        <NavSection title="General Admin" items={generalAdminLinks} pathname={pathname} defaultExpanded={false} />
      </nav>
