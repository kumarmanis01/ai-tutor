...
// Basic metrics
const jobsSpawned = new client.Counter({
  name: 'orchestrator_jobs_spawned_total',
  help: 'Total worker jobs spawned',
});
const workersRunning = new client.Gauge({
...
const analyticsJobRuns = new client.Counter({
  name: 'analytics_job_runs_total',
  help: 'Total analytics job runs',
  ...
  app.get('/health', async (_req: any, res: any) => {
    try {
      ...
      if (fs.existsSync(STATUS_FILE))
        orchestrator = JSON.parse(fs.readFileSync(STATUS_FILE, 'utf8'));

      const counts = await prisma.workerLifecycle.groupBy({
        by: ['status'],
        _count: { status: true },
      });
...
export function incJobsSpawned() {
  jobsSpawned.inc();
}
