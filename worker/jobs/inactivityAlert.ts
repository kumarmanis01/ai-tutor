...
export async function runInactivityAlerts(): Promise<number> {
  ...
  const inactiveStudents = await prisma.user.findMany({
    where: {
      role: UserRole.user,
      OR: [{ lastSessionDate: { lt: prefilterCutoff } }, { lastSessionDate: null }],
    },
  ...
  for (const s of inactiveStudents) {
    try {
      ...
      const lastLocalDateStr = s.lastSessionDate
        ? getLocalDateString(new Date(s.lastSessionDate), studentTz)
        : null;
      ...
      const minThresholdStartUtc = new Date(
        startOfTodayUtc.getTime() - MIN_INACTIVITY_DAYS * 24 * 60 * 60 * 1000
      );
      ...
      for (const link of links) {
        ...
        if (parentDigestOptOut || parentInactivityOptOut) {
          logger.info('inactivityAlert: parent opted out (digest/inactivity)', {
            parentId: parent.id,
            studentId: s.id,
          });
          continue;
        ...
        if ((link as any).excludeFromParentReport) {
          logger.info('inactivityAlert: child excluded from parent report', {
            parentId: parent.id,
            studentId: s.id,
          });
          continue;
        ...
        if ((link as any).inactivityOptOut) {
          logger.info('inactivityAlert: child opted out of inactivity alerts', {
            parentId: parent.id,
            studentId: s.id,
          });
          continue;
        ...
        const parentThresholdDays: number =
          parentProfile?.inactivityThresholdDays ?? DEFAULT_INACTIVITY_DAYS;
        const parentThresholdStart = new Date(
          startOfTodayUtc.getTime() - parentThresholdDays * 24 * 60 * 60 * 1000
        );
        ...
        if ((link as any).isPaused) {
          ...
          if (pausedUntil && new Date(pausedUntil) > new Date()) {
            logger.info('inactivityAlert: child paused by parent', {
              parentId: parent.id,
              studentId: s.id,
              pausedUntil,
            });
        ...
        try {
          ...
          if (!allowed.allowed) {
            logger.info('inactivityAlert: policy prevents send', {
              parentId: parent.id,
              studentId: s.id,
          ...
          try {
            ...
            for (const days of MUTE_OPTIONS) {
              ...
              muteLinks.push({
                days,
                url: `${muteBaseUrl}/api/parent/alerts/mute?t=${encodeURIComponent(token)}`,
              });
            }
          } catch (e) {
            logger.warn('inactivityAlert: failed to generate mute tokens', {
              parentId: parent.id,
              studentId: s.id,
              err: String(e),
            });
          ...
          const lastStudiedLabel = s.lastSessionDate
            ? new Date(s.lastSessionDate).toLocaleDateString('en-IN', {
                day: 'numeric',
                month: 'short',
              })
            : '';
          ...
          await sendParentMilestoneNotification(parent.id, {
            ...
            html: brandedHtml,
            text: t(
              'inactivity.body_html',
              {
                parentName: parent.name ?? 'Parent',
                studentName: s.name,
                days: String(parentThresholdDays),
                deepLink,
              },
              locale
            ),
          ...
          logger.error('inactivityAlert: send_failed', {
            parentId: parent.id,
            studentId: s.id,
            error: String(err),
          });
      ...
      logger.error('inactivityAlert: error processing student', {
        studentId: s.id,
        error: String(err),
      });
    }
