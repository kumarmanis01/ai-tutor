...
import { parentDigestHtml } from '@/lib/email/templates';
...
// Template moved to lib/email/templates.parentDigestHtml

// Backwards-compatible shim: some callers and unit tests expect
...
export function buildDigestHtml(parentName: string, childSections: string[]): string {
  ...
  if (html.includes('</style>')) {
    return html.replace('</style>', `${darkModeCss}</style>`);
  }

  // Fallback: inject a <style> block in the head
  return html.replace('<head>', `<head><style>${darkModeCss}</style>`);
}
