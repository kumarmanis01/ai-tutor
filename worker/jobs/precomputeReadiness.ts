...
export async function precomputeReadiness(): Promise<{ students: number; scores: number }> {
  ...
  const allSubjectStrings = [
    ...new Set(
      (users as UserRow[]).flatMap((u: UserRow) => (u.subjects as string[]).filter(Boolean))
    ),
  ];
  ...
  for (const user of users as UserRow[]) {
    ...
    try {
      parentLinks = (await prisma.parentStudent.findMany({
        ...
        select: {
          parent: {
            select: { id: true, email: true, whatsappPhone: true, name: true, language: true },
          },
        },
      })) as ParentLinkRow[];
    } catch (err) {
      logger.warn('precomputeReadiness.parentLinksFailed', {
        studentId: user.id,
        error: String(err),
      });
    }
    ...
    for (const subjectId of subjectIds) {
      try {
        ...
        try {
          ...
          logger.error('precomputeReadiness.redisSnapshotFailed', {
            studentId: user.id,
            subjectId,
            err: String(e),
          });
        ...
        await maybeFireReadinessMilestone(
          user.id,
          subjectId,
          subjectName,
          readiness.score,
    ...
    if (drop > READINESS_DROP_THRESHOLD) {
      ...
      if (parentLinks.length > 0) {
        try {
          ...
          logger.warn('precomputeReadiness.parentNotify.drop failed', {
            studentId,
            subjectId,
            error: String(err),
          });
    ...
    for (const threshold of READINESS_NOTIFICATION_THRESHOLDS) {
      ...
      if (parentLinks.length > 0) {
        try {
          ...
          logger.warn('precomputeReadiness.parentNotify.milestone failed', {
            studentId,
            subjectId,
            threshold,
            error: String(err),
