...
export async function runMonthlyMisconceptionPrevalence(): Promise<{ updated: number }> {
  ...
  try {
    ...
    for (const m of misconceptions) {
      ...
      const attempts = await prisma.answerEvent.count({
        where: { conceptId: m.conceptId, createdAt: { gte: since } },
      });
      ...
      try {
        const THRESHOLD = 0.3;
        const mis = await prisma.misconception.findUnique({
          where: { id: m.id },
          select: { id: true, name: true, conceptId: true, subjectId: true },
        });
        if (mis) {
          ...
          if (prevalenceRate > THRESHOLD) {
            ...
            const existing = await prisma.systemAlert.findFirst({
              where: { type: 'MISCONCEPTION_HIGH_PREVALENCE', message },
            });
            if (existing) {
              ...
              await prisma.systemAlert.create({
                data: {
                  type: 'MISCONCEPTION_HIGH_PREVALENCE',
                  severity: 'WARNING',
                  message,
                  payload: {
                    misconceptionId: mis.id,
        ...
        logger.error('misconceptionPrevalence.alert.failed', {
          error: String((alertErr as any)?.message ?? alertErr),
        });
      }
      updated++;
    }

    logger.info('misconceptionPrevalence.run.complete', {
      since: since.toISOString(),
      count: misconceptions.length,
    });
    return { updated };
  } catch (err) {
    logger.error('misconceptionPrevalence.run.failed', {
