...
export async function generateSignalsForCourse(courseId: string) {
  ...
  const latest = await db.analyticsDailyAggregate.findFirst({
    where: { courseId },
    orderBy: { day: 'desc' },
  });
  if (latest && latest.completionRate != null && latest.completionRate < 0.2) {
    await db.analyticsSignal.create({
      data: {
        courseId,
  ...
  const attempted = await db.analyticsEvent.count({
    where: { courseId, eventType: 'quiz_attempted', createdAt: { gte: since } },
  });
  const passed = await db.analyticsEvent.count({
    where: { courseId, eventType: 'quiz_passed', createdAt: { gte: since } },
  });
  if (attempted >= 10) {
  ...
  const enrollments = await db.enrollment
    .count({ where: { courseId, createdAt: { gte: since } } })
    .catch(() => 0);
  if (purchases >= 10) {
...
export async function generateSignalsForAllCourses() {
  ...
  const courses = await db.analyticsDailyAggregate.findMany({
