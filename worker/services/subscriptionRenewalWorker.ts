...
export async function processRenewals(now = new Date()) {
  ...
  for (const sub of dueSubs) {
    try {
      ...
      const pendingForUser = await prisma.payment.findMany({
        where: { userId: sub.userId, status: 'pending' },
        select: { id: true, meta: true },
      });
      const existing = pendingForUser.find(
        (pp: any) => (pp.meta && (pp.meta as any).renewal) === true
      );
      if (!existing) {
      ...
      logger.error('processRenewals: failed creating pending renewal', {
        err: String(err),
        subscriptionId: sub.id,
      });
    }
  ...
  const simulateFailures: boolean =
    String(process.env.SUBSCRIPTION_RENEWAL_SIMULATE_FAILURES ?? '1') !== '0';
  ...
  for (const p of pendingPayments) {
    ...
    const firstAttemptAt = meta.firstAttemptAt
      ? new Date(meta.firstAttemptAt)
      : isEmi && meta.dueDate
    ...
    if (simulateFailures && attempts < 2) {
      ...
      if (attempts === 0) {
        try {
          ...
          const graceStr = predictedGrace.toLocaleDateString('en-IN', {
            day: 'numeric',
            month: 'long',
            year: 'numeric',
          });
          ...
          if (user?.email)
            await sendMailSafe({ to: user.email, subject, html, text: subject }).catch((e) =>
              logger.error('sendMailSafe failed (renewal first-failure)', {
                err: String(e),
                userId: p.userId,
              })
            );
          if (user?.phone)
            await sendSms(
              ...
              logger.error('sendSms failed (renewal first-failure)', { err: e, userId: p.userId })
            );
        } catch (err) {
          logger.error('Failed to notify user on first payment failure', {
            err: String(err),
            paymentId: p.id,
          });
        }
      ...
      await prisma.payment.update({
        where: { id: p.id },
        data: { meta: { ...meta, attempts: newAttempts, nextAttemptAt: newNext.toISOString() } },
      });
      ...
      const updatedMeta = {
        ...
        lastNotifiedGraceAt: now.toISOString(),
      };
      ...
      try {
        ...
        if (user?.email)
          await sendMailSafe({ to: user.email, subject, html, text: subject }).catch((e) =>
            logger.error('sendMailSafe failed (enter grace)', { err: String(e), userId: p.userId })
          );
        if (user?.phone)
          await sendSms(
            user.phone,
            `Payment overdue. Access will be suspended on ${graceUntil.toLocaleDateString('en-IN')}. Update: ${appUrl}/account/billing`
          ).catch((e) =>
            logger.error('sendSms failed (enter grace)', { err: String(e), userId: p.userId })
          );
      } catch (err) {
        logger.error('Failed to notify user on entering grace', {
          err: String(err),
          paymentId: p.id,
        });
      }
      ...
      logger.info('subscription:entered_grace', {
  ...
  for (const p of inGracePayments) {
    ...
    if (graceUntil.getTime() > now.getTime()) {
      ...
      if (shouldNotify) {
        try {
          ...
          if (user?.email)
            await sendMailSafe({ to: user.email, subject, html, text: subject }).catch((e) =>
              logger.error('sendMailSafe failed (grace reminder)', {
                err: String(e),
                userId: p.userId,
              })
            );
          if (user?.phone)
            await sendSms(
              user.phone,
              `Reminder: payment overdue. Update billing: ${appUrl}/account/billing`
            ).catch((e) =>
              logger.error('sendSms failed (grace reminder)', { err: e, userId: p.userId })
            );
          await prisma.payment.update({
            where: { id: p.id },
            data: { meta: { ...meta, lastNotifiedGraceAt: now.toISOString() } },
          });
        } catch (err) {
    ...
    try {
      logger.info('subscription:grace_expired', {
        paymentId: p.id,
        subscriptionId: meta.subscriptionId,
        userId: p.userId,
      });
      await prisma.$transaction(async (tx) => {
        if (meta.subscriptionId) {
          try {
            ...
            logger.error('subscription:update_by_id_failed', {
              err: e,
              subscriptionId: meta.subscriptionId,
            });
          }
          ...
          await tx.subscription.updateMany({
            where: { userId: p.userId, active: true, endDate: { lte: now } },
            data: { active: false },
          });
        }
        ...
        await tx.payment.update({
          where: { id: p.id },
          data: { status: 'failed', meta: { ...meta, failedAt: now.toISOString() } },
        });
      });
      ...
      logger.error('processRenewals: failed to revoke subscription', {
        err,
        paymentId: p.id,
        subscriptionId: meta.subscriptionId,
      });
