...
async function processEscalation(esc: any, now: Date): Promise<void> {
  try {
      ...
      try {
        ...
        if (concept?.subjectId) {
          ...
          try {
            ...
            logger.warn('doubtEscalationNotifier.cacheSharedFailed', {
              escalationId: esc.id,
          ...
          try {
            ...
            logger.warn('doubtEscalationNotifier.cacheStudentFailed', {
              escalationId: esc.id,
              err: String((e as any)?.message ?? e),
            });
          }
        ...
        logger.warn('doubtEscalationNotifier.cacheFailed', {
          escalationId: esc.id,
          err: String((e as any)?.message ?? e),
        });
      }
    ...
    try {
      ...
      logger.warn('doubtEscalationNotifier.pushCallFailed', {
        escalationId: esc.id,
        err: String((e as any)?.message ?? e),
      });
      pushSent = 0;
      ...
      try {
        ...
        if (user?.phone) {
          ...
          try {
            ...
            logger.warn('doubtEscalationNotifier.smsFailed', {
              escalationId: esc.id,
              err: String((e as any)?.message ?? e),
            });
          }
        ...
        logger.warn('doubtEscalationNotifier.fallbackFailed', {
          escalationId: esc.id,
          err: String((e as any)?.message ?? e),
        });
      }
    ...
    await prisma.doubtEscalation
      .update({ where: { id: esc.id }, data: updateData })
      .catch(() => {});
  } catch (err) {
    logger.error('doubtEscalationNotifier.rowProcessingError', {
...
export async function processDoubtEscalationNotifications(now = new Date()): Promise<number> {
  try {
    ...
    logger.error('doubtEscalationNotifier.failed', {
      err: err instanceof Error ? err.message : String(err),
    });
    return 0;
  }
...
export async function processSingleDoubtEscalationNotification(
  id: string,
  now = new Date()
): Promise<boolean> {
