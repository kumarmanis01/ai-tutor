...
export async function runDailyCostReport(): Promise<CostReportResult> {
  ...
  const needsAlert =
    isCostThreshold || isRollingAnomaly || isCeiling || isDropout || hasTrendingDoubts;
  ...
  logger.info('costReportingWorker.report', {
    ...
    rollingAvg,
    cacheHitRate,
    trendingDoubtCount: trendingDoubts.length,
  ...
  if (needsAlert) {
    ...
    if (oncallEmail) {
      try {
        ...
        const trendingText =
          trendingDoubts.length > 0
            ? '\n\nTrending doubts (last 7 days):\n' +
              trendingDoubts
                .map((d) => `  - ${d.conceptName}: ${d.studentCount} escalations`)
                .join('\n')
            : '';
        const cacheText =
          cacheHitRate !== null
        ...
        alertSent = true;
        logger.info('costReportingWorker.alertSent', {
          to: oncallEmail,
          dateLabel,
          costPerSession,
        });
    ...
    try {
      ...
      if (qFlags.length > 0) {
        ...
        if (directAnswerCount > 0) {
          logger.error('costReportingWorker.CRITICAL_DIRECT_ANSWER_GIVEN', {
            count: directAnswerCount,
          });
        }
      }
    } catch (err) {
