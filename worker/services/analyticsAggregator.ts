...
export async function aggregateDay(date: Date) {
  ...
  const byCourse = new Map<
    string,
    { views: number; completions: number; timeSum: number; timeCount: number; users: Set<string> }
  >();
  ...
  for (const ev of events) {
    ...
    if (!agg) {
      agg = { views: 0, completions: 0, timeSum: 0, timeCount: 0, users: new Set() };
  ...
  for (const [courseId, agg] of byCourse.entries()) {
    ...
    try {
      ...
      try {
        ...
        logger?.error?.('aggregateDay: failed to upsert aggregate', {
