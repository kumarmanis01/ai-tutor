...
export async function processPaymentDunning(): Promise<void> {
  ...
  try {
    ...
    const subs = (await prisma.subscription.findMany({
      where: { active: true, dunningAttempts: { gt: 0, lt: 3 } },
    })) as SubscriptionRow[];
    ...
    for (const s of subs) {
      try {
        ...
        const shouldRunNext =
          (attempts === 1 && elapsedMs >= 24 * 60 * 60 * 1000) ||
          (attempts === 2 && elapsedMs >= 2 * 24 * 60 * 60 * 1000);
        ...
        const pm = (await prisma.paymentMethod.findFirst({
          where: { userId: s.userId, verified: true },
          orderBy: [{ isDefault: 'desc' }, { updatedAt: 'desc' }],
          include: { customer: true },
        })) as PaymentMethodRow | null;
        let charged = false;
        ...
        if (pm && pm.provider === 'razorpay') {
          try {
            ...
            const { netAmountPaise, remainingCreditPaise } = applyCreditsToCharge(
              amountPaise,
              s.creditBalance ?? 0
            );
            ...
            if (netAmountPaise === 0) {
              ...
              await prisma.$transaction(async (tx) => {
                await tx.payment.create({
                  data: {
                    ...
                    meta: {
                      subscriptionId: s.id,
                      autoCharge: true,
                      creditApplied: s.creditBalance ?? 0,
                    },
                  },
                });
                ...
                await recordPaymentEvent(tx, {
                  ...
                  provider: 'credit',
                  providerIdempotencyKey: idempotencyKey,
                  eventType: 'charge.credit_applied',
                  amount: 0,
                  status: 'success',
                ...
                await tx.subscription.update({
                  ...
                  data: {
                    ...
                    graceUntil: null,
                    endDate: newEnd,
                    creditBalance: remainingCreditPaise,
                  },
                });
                ...
                for (const cid of childIds) {
                  await tx.user.update({
                    where: { id: cid },
                    data: { subscriptionStatus: 'active', subscriptionExpiry: newEnd },
                  });
                }
              });
              ...
              try {
                ...
                logger.warn('paymentDunning: invoice/email after credit-apply failed', {
                  subscriptionId: s.id,
                  err: String(err),
                });
              }
              ...
              const paymentId =
                (resp &&
                  (resp.id || resp.razorpay_payment_id || resp.payment_id || resp.transactionId)) ||
                null;
              const orderId =
                (resp && (resp.order_id || resp.razorpay_order_id || resp.orderId)) || null;
              ...
              if (paymentId) {
                await prisma.$transaction(async (tx) => {
                  ...
                  await recordPaymentEvent(tx, {
                    ...
                    amount: netAmountPaise,
                    status: 'success',
                    payload: { subscriptionId: s.id, autoCharge: true },
                  });
                ...
                try {
                  await createInvoiceForPayment({
                    userId: s.userId,
                    paymentId: chargePaymentId ?? undefined,
                    studentId: undefined,
                    amountPaise: netAmountPaise,
                    planLabel: plan.label,
                    billingCycle: plan.perMonthDisplay,
                  ...
                  logger.warn('paymentDunning: invoice/email after auto-charge failed', {
                    subscriptionId: s.id,
                    err: String(err),
                  });
                }
            ...
            logger.error('paymentDunning: auto-charge attempt failed', {
              subscriptionId: s.id,
              error: String(err),
            });
          }
        ...
        if (charged) {
          logger.info('paymentDunning: auto-charge succeeded', {
            subscriptionId: s.id,
        ...
        await sendMailSafe({ to: parent.email ?? '', subject, html });
        if (parent.phone)
          await sendSms(parent.phone, `Subscription retry: please update payment at ${retryLink}`);
        ...
        logger.error('paymentDunning: error processing retry attempt', {
          subscriptionId: s.id,
          error: String(err),
        });
      }
    ...
    const subsToGrace = (await prisma.subscription.findMany({
      where: { active: true, dunningAttempts: { gte: 3 }, graceUntil: null },
    })) as SubscriptionRow[];
    for (const s of subsToGrace) {
      try {
        ...
        if (parent.phone)
          await sendSms(
            parent.phone,
            `Spinzy: grace period started until ${graceUntil.toLocaleDateString('en-IN')}. Update payment: ${process.env.NEXTAUTH_URL ?? 'https://spinzyacademy.com'}/parent/billing`
          );
      } catch (err) {
        logger.error('paymentDunning: error starting grace', {
          subscriptionId: s.id,
          error: String(err),
        });
      }
    ...
    const subsInGrace = (await prisma.subscription.findMany({
      where: { active: true, graceUntil: { gt: now } },
    })) as SubscriptionRow[];
    for (const s of subsInGrace) {
      try {
        ...
        if (parent.phone)
          await sendSms(
            parent.phone,
            `Reminder: Spinzy grace until ${new Date(s.graceUntil!).toLocaleDateString('en-IN')}. Update payment: ${process.env.NEXTAUTH_URL ?? 'https://spinzyacademy.com'}/parent/billing`
          );
      } catch (err) {
        logger.error('paymentDunning: error sending grace reminder', {
          subscriptionId: s.id,
          error: String(err),
        });
      }
    ...
    const subsToExpire = (await prisma.subscription.findMany({
      where: { active: true, graceUntil: { lt: now } },
    })) as SubscriptionRow[];
      ...
      try {
        await prisma.$transaction(async (tx) => {
          ...
          for (const cid of childIds) {
            await tx.user.update({
              where: { id: cid },
              data: { subscriptionStatus: 'free', subscriptionExpiry: null },
            });
          }
        });
        ...
        logger.error('paymentDunning: error expiring subscription', {
          subscriptionId: s.id,
          error: String(err),
        });
      }
