...
export async function processInstallmentDunning(jobData?: any): Promise<void> {
  ...
  try {
    ...
    if (
      jobData &&
      jobData.notes &&
      (jobData.notes.subscriptionId || jobData.notes.subscription_id)
    ) {
      const subscriptionId =
        jobData.notes.subscriptionId || jobData.notes.subscription_id || jobData.notes.subscription;
      const installmentNumberRaw =
        jobData.notes.installmentNumber ||
        jobData.notes.installment_no ||
        jobData.notes.installment ||
        jobData.notes.installmentIndex ||
        jobData.notes.installment_index;
    ...
    const failed = await prisma.installment.findMany({
      where: { status: 'FAILED', attemptCount: { gt: 0, lt: 3 } },
    });
    for (const inst of failed) {
      try {
        ...
        logger.error('installmentDunning: error processing installment', {
...
async function attemptInstallmentCharge(inst: any): Promise<void> {
  ...
  const pm = await prisma.paymentMethod.findFirst({
    where: { userId: subscription.userId, verified: true },
  ...
  try {
    ...
    if (resp) {
      const paymentId =
        resp.id || resp.razorpay_payment_id || resp.payment_id || resp.transactionId || null;
      ...
      await prisma.$transaction(async (tx) => {
        ...
        await recordPaymentEvent(tx, {
          ...
          transactionId: paymentId ?? undefined,
          orderId: resp.order_id ?? undefined,
          eventType: 'installment.charge.succeeded',
          amount: inst.amount,
          status: 'success',
      ...
      try {
        ...
        logger.warn('installmentDunning: invoice/email after installment charge failed', {
          installmentId: inst.id,
          err: String(err),
        });
      }
    ...
    await prisma.installment.update({
      where: { id: inst.id },
      data: { attemptCount: { increment: 1 }, lastAttemptAt: now, status: 'FAILED' },
    });
    ...
    if (attempts >= 3) {
      ...
      try {
        ...
        logger.error('installmentDunning: error notifying parent about grace', {
          installmentId: inst.id,
          error: String(err),
        });
      }
    ...
    logger.error('installmentDunning: error attempting charge', {
      installmentId: inst.id,
      error: String(err),
    });
  }
