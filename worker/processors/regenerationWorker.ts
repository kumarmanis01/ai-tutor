  ...
  try {
    return await prisma.$transaction(async (tx) => {
      const updated = await tx.regenerationJob.updateMany({
        ...
        data: { status: 'RUNNING', lockedAt: new Date() as any } as any,
      });
    ...
    try {
      logger?.warn?.('claimJob: transaction failed', { err });
    } catch {}
    return null;
  }
}
...
export async function processNextJob() {
  const pending = await prisma.regenerationJob.findFirst({
  ...
  try {
    ...
    await logAuditEvent(prisma as any, {
      ...
      targetId: claimed.id,
      action: AuditEvents.REGEN_JOB_LOCKED as any,
      details: { jobId: claimed.id, legacyAction: AuditEvents.REGEN_JOB_LOCKED } as any,
    });
  } catch {}
  try {
    await logAuditEvent(prisma as any, {
      targetEntity: 'RegenerationJob',
      targetId: claimed.id,
      action: AuditEvents.REGEN_JOB_STARTED as any,
      details: { jobId: claimed.id, legacyAction: AuditEvents.REGEN_JOB_STARTED } as any,
    });
  } catch {}
  ...
  try {
    ...
    const out = await prisma.regenerationOutput.create({
      data: {
        jobId: claimed.id,
        targetType: (claimed as any).targetType,
        targetId: (claimed as any).targetId,
    ...
    try {
      const updated = await prisma.regenerationJob.updateMany({
      ...
      if ((updated as any).count > 0) {
        try {
          await logAuditEvent(prisma as any, {
            targetEntity: 'RegenerationJob',
            targetId: claimed.id,
            action: AuditEvents.REGEN_JOB_COMPLETED as any,
            details: {
              jobId: claimed.id,
              legacyAction: AuditEvents.REGEN_JOB_COMPLETED,
              outputRef,
            } as any,
          });
        } catch {}
    ...
    try {
      ...
      await prisma.regenerationJob.updateMany({
        where: { id: claimed.id },
    ...
    try {
      await logAuditEvent(prisma as any, {
        targetEntity: 'RegenerationJob',
        targetId: claimed.id,
        action: AuditEvents.REGEN_JOB_FAILED as any,
        details: {
          jobId: claimed.id,
          legacyAction: AuditEvents.REGEN_JOB_FAILED,
          errorJson,
        } as any,
      });
    } catch {}
  ...
  const runner = async () => {
    while (_running && !_stopRequested) {
      try {
        ...
        try {
          logger.error(`regenerationWorker: error ${String(err)}`);
        } catch {}
        await sleep(interval);
      }
    }
