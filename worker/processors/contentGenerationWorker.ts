...
function buildPrompt(data: ContentGenerationJobData): string {
  const langInstruction = data.language === 'hi'
    ...
    for await (const chunk of stream) {
      ...
      if (chunkCount % 5 === 0) {
        ...
        await redis.publish(
          ...
          JSON.stringify({ type: 'partial', content: fullContent, progress: Math.min(90, chunkCount * 5) }),
        )
      }
    }
      ...
      if ((job.attemptsMade ?? 0) >= (job.opts.attempts ?? 3)) {
        ...
        if (redis) {
          await redis.publish(
            ...
            JSON.stringify({ type: 'error', message: 'Content generation failed. We will try again.' }),
          ).catch(() => {})
        }
      }
    })
