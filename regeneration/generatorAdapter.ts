...
export default async function generatorAdapter(
  job: RegenerationJobShape
  ...
  switch (job.targetType) {
    case 'LESSON': {
      ...
      const lessons = await generateLessons(
        { syllabusId, moduleId, moduleTitle, learningObjectives, lessonCount },
        llm
      );
    ...
    case 'PROJECT': {
      ...
      const project = await generateProject(
        { syllabusId, moduleId, moduleTitle, learningObjectives },
        llm
      );
