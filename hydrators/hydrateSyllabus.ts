...
export async function enqueueSyllabusHydration(input: {
  ...
  const resolved = await resolveSubjectId({
    board: input.board,
    grade: input.grade,
  ...
  try {
    ...
    if (HYDRATION_DEBUG)
      logger.debug('[hydration][DEBUG] created Outbox row for HydrationJob', {
        jobId: job.id,
        outboxId: outbox.id,
      });
    return { created: true, jobId: job.id, bullJobId: undefined, outboxId: outbox.id };
  } catch (err) {
    logger.error('Failed to create outbox row for HydrationJob', {
