...
export async function enqueueNoteHydration(noteId: string) {
  ...
  if (HYDRATION_DEBUG) console.log('[hydration][DEBUG] enqueueNoteHydration called', { noteId })
  ...
  if (HYDRATION_DEBUG) console.log('[hydration][DEBUG] enqueued note job', { noteId, jobId: job.id })
  return job.id
}
