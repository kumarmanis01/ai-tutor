...
module.exports = {
  meta: {
    ...
    docs: {
      description:
        'Disallow direct imports of LLM SDKs outside of the centralized callLLM implementation',
    },
    schema: [],
  },
  create(context) {
      ...
      ImportDeclaration(node) {
        ...
        if (isLLMSource(src) && !isAllowed()) {
          context.report({
            node,
            message:
              'Direct LLM SDK imports are forbidden. Use the centralized `lib/callLLM` module.',
          });
        }
      },
      CallExpression(node) {
        ...
        if (node.callee && node.callee.name === 'require' && node.arguments && node.arguments[0]) {
          ...
          if (arg.type === 'Literal' && isLLMSource(arg.value) && !isAllowed()) {
            context.report({
              node,
              message:
                'Direct LLM SDK requires are forbidden. Use the centralized `lib/callLLM` module.',
            });
          }
        }
      },
