    ...
    return {
      Program(node) {
        ...
        const comments = sourceCode
          ...
          .map((c) => c.value)
          .join('\n');
        const ok = /AI CONTENT ENGINE NOTICE|COPILOT RULES|COPILOT_AI_LOCK|AI CONTENT ENGINE/i.test(
          comments
        );
        if (!ok) {
          context.report({
            node,
            message:
              'Missing Copilot AI lock header comment. Add the AI CONTENT ENGINE NOTICE header per docs/COPILOT_AI_LOCK.md',
          });
        }
      },
    };
