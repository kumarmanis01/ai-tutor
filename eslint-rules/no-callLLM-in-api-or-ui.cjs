...
module.exports = {
  meta: {
    type: 'problem',
    docs: { description: 'Disallow callLLM imports in API routes or UI' },
    schema: [],
  },
  create(context) {
      ...
      ImportDeclaration(node) {
        ...
        if (
          ...
          context.report({
            node,
            message:
              'Importing callLLM in API routes or UI is forbidden. Use workers or server APIs.',
          });
        }
      },
      CallExpression(node) {
        if (node.callee && node.callee.name === 'require' && node.arguments && node.arguments[0]) {
          ...
          if (arg.type === 'Literal' && /callLLM/.test(String(arg.value))) {
            context.report({
              node,
              message:
                'Requiring callLLM in API routes or UI is forbidden. Use workers or server APIs.',
            });
          }
        }
      },
