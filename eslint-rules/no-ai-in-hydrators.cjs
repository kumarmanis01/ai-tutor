...
module.exports = {
  meta: {
    type: 'problem',
    docs: { description: 'Disallow AI calls/imports in hydrators' },
    schema: [],
  },
  create(context) {
    ...
    function isAIImport(src) {
      ...
      if (
        normalized.endsWith('/callLLM') ||
        /callLLM\.(ts|js)$/.test(normalized) ||
        normalized === 'lib/callLLM' ||
        normalized.endsWith('/lib/callLLM')
      )
        return true;
      return false;
    }
    ...
    return {
      ImportDeclaration(node) {
        ...
        if (isAIImport(src))
          context.report({
            node,
            message: 'Hydrators must not import or call AI libraries. Enqueue a job instead.',
          });
      },
      CallExpression(node) {
        if (node.callee && node.callee.name === 'require' && node.arguments && node.arguments[0]) {
          ...
          if (arg.type === 'Literal' && isAIImport(arg.value)) {
            context.report({
              node,
              message: 'Hydrators must not require AI libraries. Enqueue a job instead.',
            });
          }
        }
      },
