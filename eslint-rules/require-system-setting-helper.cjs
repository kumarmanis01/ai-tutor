...
module.exports = {
  meta: {
    type: 'problem',
    docs: { description: 'Require isSystemSettingEnabled helper' },
    schema: [],
  },
  create(context) {
      ...
      MemberExpression(node) {
        ...
        try {
          if (
            ...
            node.object.name === 'systemSetting' &&
            node.property &&
            ((node.property.type === 'Identifier' && node.property.name === 'value') ||
              (node.property.type === 'Literal' && node.property.value === 'value'))
          ) {
            context.report({
              node,
              message:
                'Do not access systemSetting.value directly. Use isSystemSettingEnabled() helper.',
            });
          }
        } catch (e) {
          // ignore
