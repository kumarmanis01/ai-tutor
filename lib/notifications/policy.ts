...
const DEFAULT_INACTIVITY_SUPPRESSION_DAYS = Number(
  process.env.PARENT_INACTIVITY_SUPPRESSION_DAYS ?? '3'
);
...
export async function canSendNotification(
  ...
  try {
    ...
    if (type === 'milestone') {
      ...
      if (count && Number(count) >= DEFAULT_WEEKLY_CAP)
    ...
    logger.warn('notification.policy: canSend failed -- permissive allow', {
      parentId,
      type,
      error: String(err),
    });
...
export async function recordSendNotification(
  parentId: string,
  type: NotificationType,
  studentId?: string
) {
  ...
  try {
      ...
      try {
        ...
        if (!setRes) {
          try {
            ...
            logger.debug('notification.policy: failed to ensure TTL on suppression key', {
              parentId,
              studentId,
              type,
              error: String(e),
        ...
        logger.warn('notification.policy: recordSend suppression set failed', {
          parentId,
          type,
          studentId,
          error: String(err),
    ...
    logger.warn('notification.policy: recordSend failed (non-fatal)', {
      parentId,
      type,
      error: String(err),
    });
...
export {
  DEFAULT_WEEKLY_CAP as weeklyCap,
  DEFAULT_INACTIVITY_SUPPRESSION_DAYS as inactivitySuppressionDays,
};
