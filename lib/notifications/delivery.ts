...
export async function sendParentMilestoneNotification(
  ...
  try {
    ...
    if (type === 'digest' && profile?.digestOptOut)
      return { sent: false, reason: 'parent_digest_opt_out' };
    if (type === 'inactivity' && opts.meta?.studentId) {
      try {
        const pp = await prisma.parentStudent.findFirst({
          where: { parentId, studentId: opts.meta.studentId },
        });
        if (pp?.inactivityOptOut) return { sent: false, reason: 'child_inactivity_opt_out' };
      } catch (e) {
        logger.debug('[notifications] parentStudent lookup failed (best-effort)', {
          parentId,
          studentId: opts.meta?.studentId,
          error: String(e),
        });
      }
    }
  } catch (e) {
    logger.debug('[notifications] failed to load parent profile (best-effort)', {
      parentId,
      error: String(e),
    });
  }
  ...
  await Promise.allSettled(
    sentChannels.map(async (channel) => {
      try {
        ...
        logger.warn('[notifications] audit persist failed', {
          parentId,
          channel,
          error: String(e),
        });
