...
export async function resolveSubjectId(input: {
  board?: string;
  grade?: number;
  subject?: string;
  subjectId?: string;
  ...
  const candidates = await prisma.subjectDef.findMany({
    where: {
      classId: cls.id,
      OR: [{ slug: subject }, { name: { equals: subject, mode: 'insensitive' } }],
    },
  });
  if (candidates.length === 1)
    return { success: true, subjectId: candidates[0].id, boardSlug: boardRow.slug };
