...
export function normalizeStudentAnswerForLogging(raw: unknown, maxLength = 1000): string {
  try {
    ...
    try {
      logger.error('normalizeStudentAnswerForLogging:redaction_failed', { error: (err as Error)?.message })
    } catch {
      // swallow logger failures
    }

    // Basic masking: replace digits with # and obfuscate email-like tokens
    const fallback = (raw == null ? '' : String(raw)).replace(/\d/g, '#').replace(/[A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}/g, '[EMAIL]')
