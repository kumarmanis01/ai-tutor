...
export function buildCoursePackage(params: {
  ...
  const notApproved = (items: Array<InputItem<any>>) =>
    items.find((it) => !it.approval || it.approval.status !== 'APPROVED');
  ...
  function addLesson(item: InputItem<Lesson>) {
    ...
    const entry = (modulesMap.get(moduleId) as {
      moduleId: string;
      title: string;
      lessons: Lesson[];
      quizzes?: Quiz[];
  ...
  for (const m of modules) {
    if (!m.lessons || m.lessons.length < 1)
      throw new Error(`Module ${m.moduleId} must have at least one lesson`);
    if ((m.quizzes?.length || 0) + (m.projects?.length || 0) < 1)
      throw new Error(`Module ${m.moduleId} must have at least one quiz or project`);
  }
  ...
  const pkg = {
    ...
    modules: modules.map((m) => ({
      ...
      title: m.title,
      lessons: m.lessons,
      quizzes: (m.quizzes || []) as Quiz[],
      projects: (m.projects || []) as ProjectAssignment[],
    })),
