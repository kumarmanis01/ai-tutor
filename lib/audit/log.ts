...
export function logAuditEvent(db: any, ev: AuditEvent) {
  try {
    ...
    if (targetEntity && !(detailsObj as any).entityType)
      (detailsObj as any).entityType = targetEntity;
    ...
    if (!db || !db.auditLog || typeof db.auditLog.create !== 'function') {
      try {
        logger?.warn?.('logAuditEvent: prisma.auditLog.create not available; skipping audit', {
          event: ev,
        });
      } catch {}
