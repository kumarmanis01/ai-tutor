...
export function exportCourseToLMS(coursePackage: any): Buffer {
  ...
  const indexHtml = [
    ...
    ...lessons.map(
      (l, i) =>
        `<li><a href="${slug}/lessons/lesson-${pad(i + 1, 2)}.html">${escapeHtml(l.title ?? 'Lesson ' + (i + 1))}</a></li>`
    ),
    `</ul>`,
    `</body></html>`,
  ].join('\n');
  ...
  for (let i = 0; i < lessons.length; i++) {
    ...
    files.push({
      name: `${slug}/lessons/lesson-${pad(i + 1, 2)}.html`,
  ...
  const manifest = {
    ...
    lessons: lessons.map((l: any, i: number) => ({
      ...
      file: `lessons/lesson-${pad(i + 1, 2)}.html`,
    })),
  };
  files.push({
