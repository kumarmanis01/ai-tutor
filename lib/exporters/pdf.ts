...
export async function exportCourseToPDF(coursePackage: any): Promise<Buffer> {
  ...
  if (lessons.length === 0) {
    ...
    page.drawText(coursePackage?.title ?? 'Course', {
      x: 50,
  ...
  for (let i = 0; i < lessons.length; i++) {
    ...
    if (lesson.explanation?.overview) {
      ...
      for (const line of lines) {
        if (y < 60) {
          y = newPage(doc, pageSize, font, margin);
        }
...
export async function exportProgressReportToPDF(report: {
  ...
  y -= 18;
  page.drawText(`Date: ${report.date ?? new Date().toLocaleDateString()}`, {
    x: margin,
    y,
    size: 12,
    font,
  ...
  y -= 16;
  page.drawText(`- Sessions in last 30 days: ${report.sessionCount ?? 0}`, {
    x: margin + 8,
    y,
    size: 11,
    font,
