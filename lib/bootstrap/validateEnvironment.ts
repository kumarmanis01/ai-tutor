...
export async function validateEnvironment(opts: ValidateOptions = { checkMigrations: false }) {
  ...
  try {
    ...
    throw new Error(
      `Environment validation failed: Prisma could not connect: ${String(err?.message ?? err)}`
    );
  }
  ...
  if (opts.checkMigrations) {
    try {
      ...
      if (cnt > 0)
        throw new Error(
          `Environment validation failed: ${cnt} pending migrations detected in _prisma_migrations`
        );
    } catch (err: any) {
      // If the migrations table does not exist or query fails, surface the error
      throw new Error(
        `Environment validation failed during migration check: ${String(err?.message ?? err)}`
      );
    }
  }
