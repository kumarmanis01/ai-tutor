          ...
          if (name === 'findMany' || name === 'findFirst' || name === 'findUnique') {
            return async (args: any) => {
              ...
              const normalize = (r: any) => {
                ...
                if (r.metadata && r.metadata.severity && !out.severity)
                  out.severity = r.metadata.severity;
                return out;
              };
          ...
          if (name === 'findMany' || name === 'findFirst') {
            return async (args: any) => {
              try {
                ...
                const isEnumValidation =
                  lmsg.includes('invalid input value for enum') ||
                  lmsg.includes('invalid value for argument') ||
                  lmsg.includes('expected adminactiontype');
                ...
                if (where) {
                  ...
                  else if (where.action && typeof where.action.equals === 'string')
                    actions = [where.action.equals];
                  else if (where.action && Array.isArray(where.action.in))
