...
export async function submitJob(input: SubmitJobInput) {
  ...
  logger.info(`submitJob: enqueue request`, {
  ...
  switch (entityType) {
    case 'BOARD':
      ...
      break;
    case 'CLASS':
      entityRow = await prisma.classLevel.findUnique({
        where: { id: entityId },
        include: { board: true },
      });
      if (entityRow) {
      ...
      break;
    case 'SUBJECT':
      entityRow = await prisma.subjectDef.findUnique({
        where: { id: entityId },
      ...
      break;
    case 'CHAPTER':
      entityRow = await prisma.chapterDef.findUnique({
        where: { id: entityId },
      ...
      break;
    case 'TOPIC':
      entityRow = await prisma.topicDef.findUnique({
        where: { id: entityId },
        include: {
  ...
  const requiresLanguage = ['syllabus', 'notes', 'questions', 'tests'].includes(
    String(normalizedJobType)
  );
  if (requiresLanguage && !payload?.language) {
    logger.warn('submitJob: missing required language for content job', {
      jobType: normalizedJobType,
      entityType,
      entityId,
    });
  ...
  if (normalizedJobType === JobType.syllabus) {
    if (!resolvedMeta.classLevel || !resolvedMeta.board) {
      logger.warn('submitJob: insufficient academic context for syllabus job', {
        entityType,
        entityId,
        resolvedMeta,
      });
  ...
  if (existing) {
    logger.info(`submitJob: idempotent hit returning existing job`, {
      jobId: existing.id,
  ...
  try {
    await prisma.jobExecutionLog.create({
      data: {
        ...
        prevStatus: null,
        newStatus: 'pending',
        meta: { resolvedMeta: resolvedMeta },
      },
    });
  } catch (e) {
  ...
  try {
    await prisma.auditLog.create({
      data: {
        ...
        targetId: job.id,
        action: null,
        details: { legacyAction: 'create_job', jobType: normalizedJobType, entityType, entityId },
      },
    });
  } catch (err) {
  ...
  try {
    if (normalizedJobType === JobType.syllabus) {
      ...
      try {
        logger.info('submitJob: invoking enqueueSyllabusHydration', {
          jobId: job.id,
          resolvedMeta,
          hydrateInput,
          payload,
      ...
      if (res.created) {
        ...
        try {
          await prisma.executionJob.update({
            where: { id: job.id },
            data: { payload: { ...(payload || {}), hydrationJobId: res.jobId } },
          });
        } catch (e) {
          logger?.warn?.('submitJob: failed to update ExecutionJob payload with hydrationJobId', {
            err: e,
            jobId: job.id,
          });
        }
        try {
          ...
          logger?.warn?.('submitJob: failed to write JobExecutionLog ENQUEUED', {
            err: e,
            jobId: job.id,
          });
        }
        ...
        logger.info('submitJob: enqueueSyllabusHydration aborted', {
          reason,
          jobId: job.id,
          result: res,
        });
        ...
        if (
          String(reason).startsWith('resolve_') ||
          reason === 'resolve_not_found' ||
          ...
          try {
            ...
            await prisma.executionJob.update({
              where: { id: job.id },
              data: { status: 'failed', lastError: le },
            });
            ...
            logger?.warn?.('submitJob: failed to mark ExecutionJob failed after hydrator abort', {
              err: e,
              jobId: job.id,
            });
          }
      ...
      if (normalizedJobType === JobType.notes) {
        workerType = 'NOTES';
        logger.info('submitJob: invoking enqueueNotesHydration', {
          jobId: job.id,
          topicHydrationInput,
        });
        ...
        logger.info('submitJob: invoking enqueueQuestionsHydration', {
          jobId: job.id,
          topicHydrationInput,
        });
        ...
        logger.info('submitJob: invoking enqueueTestsHydration', {
          jobId: job.id,
          topicHydrationInput,
        });
        ...
        workerType = 'ASSEMBLE_TEST';
        logger.info('submitJob: invoking enqueueAssembleHydration', {
          jobId: job.id,
          topicHydrationInput,
        });
        ...
        logger.error('submitJob: unhandled jobType after validation', {
          jobType: normalizedJobType,
        });
        throw new Error(`Unhandled jobType: ${normalizedJobType}`);
      }

      logger.info('submitJob: hydrator returned', {
        jobId: job.id,
        workerType,
      ...
      if (enqueueResult?.created) {
        ...
        try {
          const meta: any = {
            queue: CONTENT_HYDRATION_QUEUE,
            workerType,
            hydrationJobId: enqueueResult.jobId,
          };
        ...
        logger.info('submitJob: hydrator aborted', {
          reason,
          jobId: job.id,
          workerType,
          result: enqueueResult,
    ...
    try {
      ...
      logger?.warn?.('submitJob: failed to write JobExecutionLog ENQUEUE_FAILED', {
        err: e,
        jobId: job.id,
      });
    }
