...
export type ValidateResult = {
  status: number;
  body: { ok?: boolean; error?: string; coupon?: any };
};
export type RedeemResult = {
  status: number;
  body: { ok?: boolean; error?: string; amountAppliedPaise?: number; percentApplied?: number };
};
...
export async function validateCoupon(
  ...
  if (c.maxUses != null && c.uses >= c.maxUses)
    return { status: 400, body: { error: 'usage_limit_reached' } };
  ...
  try {
      ...
      if (targetSubscriptionId && applyAsCredit) {
        try {
          await tx.subscription.update({
            ...
            data: { creditBalance: { increment: amountAppliedPaise } },
          });
        } catch (err) {
          logger.warn('redeemCoupon: failed applying credit to subscription', {
            err: String(err),
            couponId: coupon.id,
        ...
        try {
          ...
          if (sub && sub.billingCycle) {
            ...
            if (plan) {
              ...
              amountAppliedPaise = Math.max(
                1,
                Math.floor((planPaise * (percentApplied as number)) / 100)
              );
              // Apply as credit
              await tx.subscription.update({
                where: { id: targetSubscriptionId },
          ...
          logger.warn('redeemCoupon: percent apply best-effort failed', {
            err: String(err),
            couponId: coupon.id,
            subscriptionId: targetSubscriptionId,
          });
    ...
    await tx.couponRedemption.create({
      data: {
        ...
        amountApplied: amountAppliedPaise ?? null,
        percentApplied: percentApplied ?? null,
        metadata: null,
      },
    });
    ...
    return {
      ...
      body: {
        ok: true,
        amountAppliedPaise: amountAppliedPaise ?? undefined,
        percentApplied: percentApplied ?? undefined,
      },
    };
  } catch (err: any) {
