...
export function classifyStreakGap(
  ...
  try {
    ...
    try {
      logger.debug('classifyStreakGap: local-date calculation failed, falling back to UTC', {
        error: String(_err),
      });
    } catch {}
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        if (m && typeof (m as any).incSuppressionDeleteFailure === 'function')
          (m as any).incSuppressionDeleteFailure();
      } catch {}
    }
