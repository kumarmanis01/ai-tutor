...
type ConceptRow = {
  ...
  topic?: {
    ...
    chapter?: { id?: string | null; name?: string | null } | null;
  } | null;
};
...
export async function computeReadinessScore(
  ...
  try {
    ...
    try {
      const signalStates = states.map((s: StudentConceptStateRow) => ({
      ...
      try {
        ...
        if (m && m.finishedAt)
          lastMock = { finishedAt: m.finishedAt, scorePercent: m.scorePercent ?? undefined };
      } catch {
      ...
      const _predictedOpts = {
        states: signalStates,
        lastMock,
        daysToExam: null,
      } as unknown as Parameters<typeof computePredictedScoreRange>[1];
...
export function computePredictedScoreRange(
  ...
  if (n <= 1) {
    const delta = 6;
    return {
      low: Math.max(0, score - delta),
      high: Math.min(100, score + delta),
      confidenceLevel: 80,
      daysUsed: daysToExam ?? null,
    };
  }
  ...
  if (
    lastMock &&
    (lastMock as WeeklyMockRow).finishedAt &&
    typeof (lastMock as WeeklyMockRow).scorePercent === 'number'
  ) {
