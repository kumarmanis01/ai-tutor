...
async function getCurriculumTopics(params: GenerateDiagnosticTestParams) {
  ...
  if (params.subjectId) {
    ...
    if (!subject) {
      throw new Error(`DiagnosticQuestionService: SubjectDef not found for id=${params.subjectId}`);
    }
  } else {
...
async function generateDiagnosticQuestionsOnDemand(
  ...
  try {
    ...
    const {
      ...
      medium: mediumCount,
      hard: hardCount,
    } = computeDifficultyCounts(totalCount);
    ...
    try {
      ...
      logger.warn('DiagnosticQuestionService.onDemand.parseError', {
    ...
    for (let i = 0; i < parsed.length && results.length < totalCount; i++) {
      ...
      const difficulty = (
        ['easy', 'medium', 'hard'].includes(String(item.difficulty ?? ''))
          ...
          : 'medium'
      ) as DiagnosticDifficulty;

      if (!questionText || rawChoices.length < 2 || !correctAnswer) continue;

      const options: DiagnosticQuestionOption[] = rawChoices
        .map((c: unknown, idx: number) => ({
