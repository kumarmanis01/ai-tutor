...
export interface GenerateDiagnosticTestParams {
  ...
  boardSlug: string;
  /**
   * Grade as an integer (e.g. 8 for Class 8).
   */
  grade: number;
  /**
...
export async function generateSubjectDiagnosticTest(
  ...
  const { subjectId, subjectName, topicIds } = await getCurriculumTopics(params);

  const [easy, medium, hard] = await Promise.all([
    pickQuestionsForDifficulty(params, topicIds, 'easy', EASY_COUNT),
    pickQuestionsForDifficulty(params, topicIds, 'medium', MEDIUM_COUNT),
    pickQuestionsForDifficulty(params, topicIds, 'hard', HARD_COUNT),
  ]);
