...
export function emitConsentApproved(
  ...
  _io
    ?.to(rooms.consent(consentToken))
    .emit('consent_approved', { consentToken, studentId, parentName });
}
 ...
 */
export function emitPremiumActivated(studentId: string, plan: string, expiresAt: string): void {
 ...
 */
export function emitContentGenerated(studentId: string, contentId: string, topic: string): void {
