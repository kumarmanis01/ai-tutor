...
export async function createStartedAIContentLog(
  db: PrismaLike,
  params: {
    model?: string;
    promptType: string;
  ...
  if (params.renderer) {
    ...
    data.requestBody.renderer = {
      schemaHash: params.renderer.schemaHash ?? null,
      version: params.renderer.version ?? null,
    };
  }
...
export async function finalizeAIContentLog(
  db: PrismaLike,
  id: string,
  updates: {
    success: boolean;
