export type EmotionalState = 'NEUTRAL' | 'ENGAGED' | 'CONFUSED' | 'FRUSTRATED' | 'DISTRESSED'; // never set by this module -- reserved for safety layer
...
export function computeFrustrationScore(
  ...
  const consecutiveWrongComponent = 0.4 * clamp01(consecutiveWrongRaw);
  ...
  const hintsComponent = 0.2 * clamp01(hintsRaw);
  ...
  const negativeLanguageComponent = 0.3 * clamp01(negRaw);
  ...
  if (sessionBaselineLatencyMs && sessionBaselineLatencyMs > 0) {
    ...
    if (latencies.length > 0) {
      ...
      latencyComponent = 0.1 * clamp01(capped);
    }
  }

  const rawScore =
    consecutiveWrongComponent + hintsComponent + negativeLanguageComponent + latencyComponent;
  ...
  if (frustrationScore >= 0.75) {
    ...
    if (anyNegativeLanguage && frustrationScore < 0.45) {
      emotionalState = 'CONFUSED';
    } else if (frustrationScore < 0.2 && !anyNegativeLanguage) {
