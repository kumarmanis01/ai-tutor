  ...
  try {
    ...
    if (process.env.JEST_WORKER_ID || process.env.NODE_ENV === 'test') {
      ...
      try {
        ...
        logger.warn('analyticsEvent.embed.create.failed', {
    ...
    try {
      ...
      if (process.env.JEST_WORKER_ID || process.env.NODE_ENV === 'test') {
        ...
        try {
          ...
          logger.warn('analyticsEvent.embed.batch.create.failed', {
            error: String((e as any)?.message ?? e),
          });
        }
        ...
        try {
          ...
          const inputTokensEstimate = Math.max(1, Math.ceil(totalInputChars / 4));
          const totalOutputTokens = batchEmbeddings.reduce(
            (s, e) => s + (Array.isArray(e) ? e.length : 0),
            0
          );
          await prisma.analyticsEvent.create({
            data: {
