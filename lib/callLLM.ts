...
export function buildPromptRequestBody(prompt: string): {
  prompt_hash: string;
...
async function prepareRetrievalContext(meta: any, charLimit = 2000) {
  try {
    ...
    if (meta.topicId) {
      const t = await prisma.topicDef.findUnique({
...
function ensureWorkerAllowed() {
  ...
  if (process.env.ALLOW_LLM_CALLS !== '1') {
    throw new Error(
      'LLM calls are restricted to worker processes. Set ALLOW_LLM_CALLS=1 in worker runtime.'
    );
  }
}
...
export async function createChatCompletion(input: any) {
  if (process.env.LLM_MODE === 'mock') {
    return {
      choices: [{ message: { content: JSON.stringify({ mock: true, input }) } }],
      usage: {},
    };
  }
...
export async function callLLM({ prompt, model, meta, timeoutMs }: CallLLMArgs) {
  ...
  const HYDRATION_DEBUG =
    process.env.HYDRATION_DEBUG === '1' || process.env.AI_CONTENT_DEBUG === '1';
  ...
  if (
    ['orchestration', 'workflow', 'reconciler'].includes(promptType) &&
    selectedModel === envLarge
  ) {
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      ...
      if (HYDRATION_DEBUG) {
        try {
          logger.debug('[ai][DEBUG] callLLM prompt length', {
            promptLength: (prompt || '').length,
          });
          logger.debug('[ai][DEBUG] callLLM response length', {
            responseLength: (content || '').length,
          });
          logger.info('callLLM debug', {
            promptLength: (prompt || '').length,
            responseLength: (content || '').length,
            meta,
            model: selectedModel,
      ...
      if (AI_CONTENT_DEBUG) {
        try {
          logger.info('AI_CONTENT_DEBUG: raw LLM content', { rawContent: content, meta });
        } catch (e) {
      ...
      try {
        ...
        const allowRawPersistence =
          String(process.env.ALLOW_PERSIST_RAW_AI_OUTPUT || '').toLowerCase() === '1' ||
          Boolean(meta?.persistRaw);
        ...
        if (LOG_RAW_LLM_OUTPUT_CONSOLE_ONLY) {
          try {
            ...
            logger.warn('callLLM LOG_RAW_LLM_OUTPUT_CONSOLE_ONLY snippet logging failed', {
              error: String(e),
            });
          }
          try {
            ...
            logger.warn('callLLM LOG_RAW_LLM_OUTPUT_CONSOLE_ONLY redaction failed', {
              error: String(e),
            });
          }
        }
          ...
          if (isTutorCall) {
            await prisma.aITutorTurnLog.create({
              data: {
                ...
                ragChunksUsed: Array.isArray(meta?.ragChunksUsed)
                  ? meta.ragChunksUsed.map((x: any) => String(x))
                  : [],
                frustrationScore:
                  typeof meta?.frustrationScore === 'number' ? meta.frustrationScore : null,
              },
            });
          } else {
        ...
        try {
          ...
          try {
            logger.warn('analyticsEvent.llm.create.failed', {
        ...
        if (!meta?.suppressLog) {
          if (isTutorCall) {
            ...
            try {
              ...
              try {
                logger.warn('analyticsEvent.llm.failed.create.failed', {
                  error: String((e as any)?.message ?? e),
                });
              } catch {}
            }
      ...
      if (!retryable || isLastAttempt) {
        if (isTutorCall) {
          ...
          if (lower.includes('rate limit') || lower.includes('rate_limit'))
            throw new LLMError('RATE_LIMITED', msg || 'Rate limited');
          if (
            lower.includes('prompt_too_large') ||
            lower.includes('context too long') ||
            lower.includes('prompt too large')
          )
            throw new LLMError('CONTEXT_TOO_LONG', msg || 'Context too long');
          if (
            lower.includes('llm_timeout') ||
...
export async function batchCallLLM(
  calls: Array<{ prompt: string; meta: any }>,
  ...
  const resolvedModel =
    model ||
    ((): string => {
      if (['topics', 'structure', 'syllabus'].includes(promptType))
        return process.env.MODEL_SMALL || 'gpt-4o-mini';
      if (['notes', 'doubts', 'practice'].includes(promptType))
        return process.env.MODEL_MEDIUM || 'gpt-4o';
  ...
  if (
    ['orchestration', 'workflow', 'reconciler'].includes(promptType) &&
    resolvedModel === (process.env.MODEL_LARGE || 'gpt-4o')
  ) {
    throw new Error('forbidden_model_for_orchestration');
  }
