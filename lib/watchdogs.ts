...
async function emitOrUpdateAlert(spec: AlertSpec) {
  try {
    const existing = await prisma.systemAlert.findFirst({
      where: { type: spec.type as any, active: true },
    });
    if (existing) {
      await prisma.systemAlert.update({
        where: { id: existing.id },
        data: { lastSeen: new Date(), message: spec.message, payload: spec.payload },
      });
      return;
    }
...
export async function runWatchdogs() {
  ...
  if (health.dependencies.redis.status === 'unhealthy') {
    await emitOrUpdateAlert({
      type: 'REDIS_DOWN',
      severity: 'CRITICAL',
      message: 'Redis is unreachable',
      payload: { redis: health.dependencies.redis },
    });
  } else {
  ...
  if (health.dependencies.database.status === 'unhealthy') {
    await emitOrUpdateAlert({
      ...
      payload: { database: health.dependencies.database },
    });
  } else {
  ...
  if (health.workers.stale > 0) {
    await emitOrUpdateAlert({
      type: 'WORKER_STALE',
      severity: 'WARNING',
      message: `${health.workers.stale} worker(s) stale`,
      payload: { workers: health.workers },
    });
  } else {
  ...
  if (health.workers.failed > 0) {
    await emitOrUpdateAlert({
      type: 'WORKER_FAILED',
      severity: 'CRITICAL',
      message: `${health.workers.failed} worker(s) failed`,
      payload: { workers: health.workers },
    });
  } else {
  ...
  if (health.jobs.stuckRunning > 0) {
    await emitOrUpdateAlert({
      type: 'JOB_STUCK',
      severity: 'WARNING',
      message: `${health.jobs.stuckRunning} job(s) stuck`,
      payload: { jobs: health.jobs },
    });
  } else {
  ...
  if (health.queue.depth > 100 || (health.queue.oldestJobAgeSec ?? 0) > 300) {
    await emitOrUpdateAlert({
      type: 'QUEUE_BACKLOG',
      severity: 'WARNING',
      message: `Queue backlog: ${health.queue.depth}, oldest ${health.queue.oldestJobAgeSec ?? 'N/A'}s`,
      payload: { queue: health.queue },
    });
  } else {
