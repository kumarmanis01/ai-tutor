...
import { systemHealth, HealthStatus } from './systemHealth';
...
async function emitOrUpdateAlert(spec: AlertSpec) {
  try {
    const existing = await prisma.systemAlert.findFirst({ where: { type: spec.type, active: true } });
    if (existing) {
...
async function resolveAlert(type: string) {
  try {
    const existing = await prisma.systemAlert.findFirst({ where: { type, active: true } });
    if (!existing) return;
