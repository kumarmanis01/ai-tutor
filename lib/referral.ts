...
export async function redeemReferral(
  ...
  if (referral.createdBy === redeemerId) {
    return {
      status: 400,
      body: { error: 'Referral code cannot be used by its creator.', voided: true },
    };
  }
  ...
  if (creatorIp && redeemerIp && creatorIp === redeemerIp) {
    return {
      status: 400,
      body: { error: 'Referral reward voided: same device or network detected.', voided: true },
    };
  }

  const newMetadata =
    referral.metadata && typeof referral.metadata === 'object'
      ? { ...(referral.metadata as Record<string, any>), rewardApplied: true }
      : { rewardApplied: true };

  await tx.referral.update({
    where: { id: referral.id },
    data: { redeemedBy: redeemerId, redeemedAt: new Date(), metadata: newMetadata },
  });
    ...
    try {
      if (referral.createdBy) {
        ...
        const sub = await tx.subscription.findFirst({
          where: { userId: referrerId, active: true },
        ...
        const rr = await tx.referralReward.create({
          data: {
