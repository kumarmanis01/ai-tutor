...
export async function evaluateAlerts(prisma: PrismaClient, opts: EvalOptions = {}) {
  ...
  const samples30 = await prisma.telemetrySample.findMany({
    where: { key: 'queue.depth.value', timestamp: { gte: window30From } },
    select: { value: true },
  });
    ...
    if (decision1 !== 'OK') {
      ...
      if (existing) {
        ...
        await prisma.systemAlert.create({
          data: { type: 'QUEUE_BACKLOG', severity: severity as any, message, payload },
        });
      }
    } else if (existing) {
      await prisma.systemAlert.update({
        where: { id: existing.id },
        data: { active: false, resolvedAt: new Date(), lastSeen: new Date() },
      });
    }
  }
  ...
  const recentSamples = await prisma.telemetrySample.findMany({
    where: { key: 'jobs.failed.count', timestamp: { gte: recentFrom } },
    select: { value: true },
  });
  ...
  const baselineSamples = await prisma.telemetrySample.findMany({
    where: { key: 'jobs.failed.count', timestamp: { gte: baselineFrom, lt: recentFrom } },
    select: { value: true },
  });
  const baselineSamplesTyped = baselineSamples as TelemetrySampleRow[];
  const baselineAvg = baselineSamplesTyped.length
    ? baselineSamplesTyped.reduce((s, r) => s + Number(r.value), 0) / baselineSamplesTyped.length
    : 0;

  const triggered =
    recentSum >= minAbs &&
    (baselineAvg === 0 ? recentSum >= minAbs * spikeMult : recentSum > baselineAvg * spikeMult);
  ...
  if (!DRY_RUN) {
    ...
    if (triggered) {
      ...
      if (existing) {
        ...
        await prisma.systemAlert.create({
          data: { type: 'JOB_STUCK', severity: severity as any, message, payload },
        });
      }
    } else if (existing) {
      await prisma.systemAlert.update({
