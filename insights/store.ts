...
export async function saveSuggestions(
  db: PrismaClient,
  suggestions: SuggestionRecordInput[],
  actorId?: string
) {
  ...
  for (const s of suggestions) {
    try {
      ...
      if (s.sourceSignalId) {
        const exists = await (db as any).contentSuggestion.findUnique({
          where: {
            sourceSignalId_type_targetId: {
              sourceSignalId: s.sourceSignalId,
              type: s.type,
              targetId: s.targetId,
            },
          },
...
export async function listSuggestions(
  db: PrismaClient,
  filters: { courseId?: string; scope?: string; status?: string } = {}
) {
  const where: any = {};
...
export async function updateSuggestionStatus(
  ...
  const updated = await (db as any).contentSuggestion.update({
    where: { id },
  ...
  try {
    logAuditEvent(db, {
      ...
      action: null,
      details: {
        legacyAction: status === 'ACCEPTED' ? 'SUGGESTION_ACCEPTED' : 'SUGGESTION_DISMISSED',
