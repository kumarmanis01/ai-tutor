...
(async function main() {
  try {
    ...
    let classLevel = await prisma.classLevel.findFirst({
      where: { boardId: board.id, grade: GRADE },
    });
    if (!classLevel) {
      classLevel = await prisma.classLevel.create({
        data: { grade: GRADE, slug: `grade-${GRADE}`, boardId: board.id },
      });
    ...
    for (let s = 1; s <= SUBJECT_COUNT; s++) {
      ...
      let subject = await prisma.subjectDef.findFirst({
        where: { classId: classLevel.id, slug: subjSlug },
      });
      if (!subject) {
        subject = await prisma.subjectDef.create({
          data: { name: `Subject ${s}`, slug: subjSlug, classId: classLevel.id },
        });
      ...
      for (let c = 1; c <= CHAPTERS_PER_SUBJECT; c++) {
        ...
        let chapter = await prisma.chapterDef.findFirst({
          where: { subjectId: subject.id, slug: chapSlug },
        });
        if (!chapter) {
          chapter = await prisma.chapterDef.create({
            data: { name: `Chapter ${c}`, slug: chapSlug, order: c, subjectId: subject.id },
          });
      ...
      for (let ci = 0; ci < chapters.length; ci++) {
        ...
        for (let t = 1; t <= count; t++) {
          ...
          let topic = await prisma.topicDef.findFirst({
            where: { chapterId: chapter.id, slug: topicSlug },
          });
          if (!topic) {
            topic = await prisma.topicDef.create({
              data: {
                name: `Topic ${topicGlobalIndex}`,
                slug: topicSlug,
          ...
          const existingQuestions = await prisma.question.findMany({
            where: { subject: subject.name, chapter: chapter.name },
            take: 1,
          });
          ...
          const existingCount = await prisma.question.count({
            where: { prompt: { contains: `__seed:${topic.slug}:` } },
          });
          ...
          if (!existingTest) {
            ...
            const createdTest = await prisma.generatedTest.create({
              data: {
                topicId: topic.id,
                title,
                difficulty: 'easy',
            ...
            const genQ = await prisma.generatedQuestion.create({
              data: {
                testId: createdTest.id,
                type: 'mcq',
                question: `__seed-gen:${topic.slug}: What is ${topic.name}?`,
