...
async function main() {
  ...
  const res = await enqueueSyllabusHydration({
    board: subj.class.board.name,
    grade: subj.class.grade,
    subject: subj.name,
    subjectId: subj.id,
    language: 'en',
  });
  ...
  const topic = await prisma.topicDef.findFirst({
    where: { chapter: { subjectId: subj.class ? subj.class.id : undefined } } as any,
  });
  ...
  const jobs = await prisma.hydrationJob.findMany({
    where: { OR: [{ subjectId: subj.id }, { topicId: anyTopic.id }] },
