...
;(async () => {
  ...
  try {
    ...
    console.log('Flags: H=HALLUCINATION I=INCORRECT_EXPLANATION P=POOR_PEDAGOGY T=OFF_TOPIC D=DIRECT_ANSWER_GIVEN S=SAFETY_CONCERN')
    ...
    if (!flagEnum) {
      console.error(`❌ Unknown flag code: ${flagCode}. Use one of: ${Object.keys(FLAG_MAP).join(' ')}`)
      process.exit(1)
    }
