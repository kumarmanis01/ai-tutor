...
export async function runOneOff(jobId: string) {
  ...
  const job = await (prisma as any).regenerationJob.findUnique({
    ...
    select: {
      ...
      targetType: true,
      targetId: true,
      instructionJson: true,
    },
  });
  if (!job) throw new Error('Job not found after claiming');

  await logAuditEvent(prisma as any, {
    action: AuditEvents.REGEN_JOB_STARTED,
    entityId: job.id,
    metadata: { jobId: job.id, manual: true },
  });
  ...
  try {
    ...
    await logAuditEvent(prisma as any, {
      action: AuditEvents.REGEN_JOB_COMPLETED,
      entityId: job.id,
      metadata: { jobId: job.id, outputRef },
    });
    ...
    await (prisma as any).regenerationJob
      ...
      .catch(() => {});
    await logAuditEvent(prisma as any, {
      action: AuditEvents.REGEN_JOB_FAILED,
      entityId: job.id,
      metadata: { jobId: job.id, errorJson },
    });
    throw err;
  }
}
