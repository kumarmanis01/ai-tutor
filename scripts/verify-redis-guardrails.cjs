...
async function main() {
  ...
  try {
    ...
    if (!STRICT_REDIS_CONFIG && (aof === null || policy === null)) {
      console.log('\nManaged Redis note:')
      console.log('- Your provider may not allow CONFIG verification; ensure persistence/eviction via provider settings.')
      console.log('- You can force strict verification by setting STRICT_REDIS_CONFIG=1 (only if CONFIG is permitted).')
    }
  } catch (e) {
