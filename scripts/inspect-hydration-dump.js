...
async function main() {
  ...
  if (!hydrationId || !executionId || !subjectId) {
    console.error(
      'Usage: node scripts/inspect-hydration-dump.js <hydrationId> <executionId> <subjectId>'
    );
    process.exit(2);
  }
  ...
  try {
    ...
    const respLog = await prisma.jobExecutionLog.findFirst({
      where: { jobId: executionId, event: 'RESPONSE_RECEIVED' },
      orderBy: { createdAt: 'desc' },
    });
    let startWindow = respLog
      ? new Date(new Date(respLog.createdAt).getTime() - 3 * 60 * 1000)
      : null;
    let endWindow = respLog
      ? new Date(new Date(respLog.createdAt).getTime() + 3 * 60 * 1000)
      : null;
    ...
    console.log(
      `Searching AIContentLog entries from ${startWindow.toISOString()} to ${endWindow.toISOString()}`
    );
    const aiLogs = await prisma.aIContentLog.findMany({
      where: { promptType: 'syllabus', createdAt: { gte: startWindow, lte: endWindow } },
    ...
    const chapters = await prisma.chapterDef.findMany({
      where: { subjectId: subjectId, lifecycle: 'active' },
      orderBy: { order: 'asc' },
    });
