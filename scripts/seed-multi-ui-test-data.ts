...
async function upsertCurriculum() {
  ...
  const class7 = await prisma.classLevel.upsert({
  ...
  // Subjects
  const subjects = {};
  const math = await prisma.subjectDef.upsert({
    where: { classId_slug: { classId: class7.id, slug: 'math' } },
    update: {},
    create: { name: 'Math', slug: 'math', classId: class7.id },
  });
  const science = await prisma.subjectDef.upsert({
    where: { classId_slug: { classId: class7.id, slug: 'science' } },
    update: {},
    create: { name: 'Science', slug: 'science', classId: class7.id },
  });
  subjects['Math'] = math;
  ...
  // Math chapters
  const algebra = await prisma.chapterDef.upsert({
    where: { subjectId_slug_version: { subjectId: math.id, slug: 'algebra', version: 1 } },
    update: {},
    create: { name: 'Algebra', slug: 'algebra', order: 1, subjectId: math.id },
  });
  const geometry = await prisma.chapterDef.upsert({
    where: { subjectId_slug_version: { subjectId: math.id, slug: 'geometry', version: 1 } },
    update: {},
    create: { name: 'Geometry', slug: 'geometry', order: 2, subjectId: math.id },
  });
  chapters['Algebra'] = algebra;
  chapters['Geometry'] = geometry;

  // Science chapters
  const physics = await prisma.chapterDef.upsert({
    where: {
      subjectId_slug_version: { subjectId: science.id, slug: 'physics-basics', version: 1 },
    },
    update: {},
    create: { name: 'Physics Basics', slug: 'physics-basics', order: 1, subjectId: science.id },
  });
  const chemistry = await prisma.chapterDef.upsert({
    where: {
      subjectId_slug_version: { subjectId: science.id, slug: 'chemistry-basics', version: 1 },
    },
    update: {},
    create: { name: 'Chemistry Basics', slug: 'chemistry-basics', order: 2, subjectId: science.id },
  });
...
async function seedStudentWeak(userEmail: string, topicMap: Record<string, any[]>) {
  const user = await prisma.user.upsert({
    ...
    update: {},
    create: { email: userEmail, name: 'Weak Student', language: 'en' },
  });
  ...
  try {
    ...
    const existing = await prisma.studentLearningProfile.findUnique({
      where: { studentId: user.id } as any,
    });
    if (existing) {
      await prisma.studentLearningProfile.update({
        where: { id: existing.id } as any,
        data: { strengths: { dailyTargetMin: 30, studyDaysPerWeek: 5 }, recommendations } as any,
      });
    } else {
      await prisma.studentLearningProfile.create({
        data: {
          studentId: user.id,
          strengths: { dailyTargetMin: 30, studyDaysPerWeek: 5 },
          recommendations,
        } as any,
      });
    }
  ...
  await prisma.learningSession.createMany({
    data: [
      {
        studentId: user.id,
        activityType: 'topic_activity',
        activityRef: linear.id,
        startedAt: daysAgo(2),
        endedAt: daysAgo(2),
        difficultyLevel: 'medium',
      },
...
async function seedStudentActive(userEmail: string, topicMap: Record<string, any[]>) {
  const user = await prisma.user.upsert({
    where: { email: userEmail },
    update: {},
    create: { email: userEmail, name: 'Active Student', language: 'en' },
  });
  ...
  try {
    await prisma.studentLearningProfile.upsert({
      ...
      update: {
        dailyTargetMin: 45,
        studyDaysPerWeek: 6,
        recommendations: { learningPath: [] } as any,
      },
      create: {
        studentId: user.id,
        dailyTargetMin: 45,
    ...
    if (existing) {
      await prisma.studentLearningProfile.update({
        where: { id: existing.id } as any,
        data: { strengths: { dailyTargetMin: 45, studyDaysPerWeek: 6 }, recommendations } as any,
      });
    } else {
      await prisma.studentLearningProfile.create({
        data: {
          studentId: user.id,
          strengths: { dailyTargetMin: 45, studyDaysPerWeek: 6 },
          recommendations,
        } as any,
      });
    }
  ...
  for (const t of topicMap['Algebra']) {
    await prisma.studentTopicMastery.upsert({
      where: { studentId_topicId: { studentId: user.id, topicId: t.id } },
      update: {
        accuracy: 80 + Math.floor(Math.random() * 11),
        questionsAttempted: 15,
        masteryLevel: 'intermediate' as MasteryLevel,
        lastAttemptedAt: daysAgo(Math.floor(Math.random() * 5)),
      },
      create: {
        studentId: user.id,
        topicId: t.id,
  ...
  await prisma.studentTopicMastery.upsert({
    where: { studentId_topicId: { studentId: user.id, topicId: motion.id } },
    update: {
      accuracy: 70,
  ...
  const activityTopics = [
    ...
    topicMap['Algebra'][1],
    topicMap['Algebra'][2],
    topicMap['Physics Basics'][0],
    topicMap['Chemistry Basics'][0],
  ];
      ...
      data: [
        {
          ...
          endedAt: daysAgo(Math.floor(Math.random() * 5)),
...
async function seedStudentNew(userEmail: string) {
  const user = await prisma.user.upsert({
    where: { email: userEmail },
    update: {},
    create: { email: userEmail, name: 'New Student', language: 'en' },
  });
  ...
  try {
    ...
    if (existing) {
      await prisma.studentLearningProfile.update({
        where: { id: existing.id } as any,
        data: { strengths: { dailyTargetMin: 20, studyDaysPerWeek: 3 }, recommendations } as any,
      });
    } else {
      await prisma.studentLearningProfile.create({
        data: {
          studentId: user.id,
          strengths: { dailyTargetMin: 20, studyDaysPerWeek: 3 },
          recommendations,
        } as any,
      });
    }
...
export async function seed() {
  ...
  const studentCount = await prisma.user.count({
    where: { email: { in: ['weak@student.com', 'active@student.com', 'new@student.com'] } },
  });
  const topicCount = await prisma.topicDef.count({
    where: {
      slug: {
        in: Object.values(topicMap)
          .flat()
          .map((t: any) => t.slug),
      },
    },
  });
  const practiceAttemptCount = await prisma.studentTopicMastery.count({
    where: {
      studentId: {
  ...
  console.log(
    '- PracticeAttempt (studentTopicMastery) count for seeded students:',
