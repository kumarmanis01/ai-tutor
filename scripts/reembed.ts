...
export async function main() {
  ...
  if (!args.runId && !args.chunkIds) {
    console.error(
      'Usage: npx tsx scripts/reembed.ts --run-id <id> | --chunk-ids id1,id2 [--dry-run]'
    );
    process.exit(1);
  }
  ...
  if (args.runId) {
    const run = await prisma.ingestRunLog.findUnique({
      where: { id: args.runId },
      select: { id: true, errorDetails: true },
    });
    if (!run) {
  ...
  const toEmbed = rows
    .filter((r) => r.content)
    .map((r) => ({ id: r.id, content: r.content as string }));
