...
function printManualSQL() {
  ...
  console.log("SELECT count(*) FROM \"TopicNote\" WHERE (content_json -> 'sections') IS NOT NULL AND jsonb_typeof(content_json -> 'sections') = 'array' AND jsonb_array_length(content_json -> 'sections') > 0;\n")
  console.log("-- Legacy-format: presence of common flat fields (adjust column name as needed)")
  console.log("SELECT count(*) FROM \"TopicNote\" WHERE (content_json ? 'explanation') OR (content_json ? 'concept') OR (content_json ? 'notes') OR (content_json ? 'example') OR (content_json ? 'keyPoints') OR (content_json ? 'commonMistakes');\n")
...
async function main() {
  ...
  try {
    ...
    console.error('Failed to load @prisma/client. Ensure dependencies are installed and `prisma generate` has been run.');
  ...
  try {
    ...
    const colRes = await prisma.$queryRawUnsafe(`SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name = '${tableName.replace(/'/g, "''")}'`)
    ...
    // Execute
    const [totalRes, vidyaRes, legacyRes, bothRes, vidyaOnlyRes, legacyOnlyRes, neitherRes] = await Promise.all([
    ...
    const vidyaOnly = Array.isArray(vidyaOnlyRes) ? vidyaOnlyRes[0].cnt : (vidyaOnlyRes?.cnt ?? vidyaOnlyRes)
    const legacyOnly = Array.isArray(legacyOnlyRes) ? legacyOnlyRes[0].cnt : (legacyOnlyRes?.cnt ?? legacyOnlyRes)
