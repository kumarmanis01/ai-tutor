...
async function reconcileOne(parent: any, dryRun = false) {
  ...
  if (dryRun) {
    return {
      ...
      completedChildren: counts.completed,
      failedChildren: counts.failed,
      wouldSetStatus: desired,
    };
  }
  ...
  try {
    ...
    if (desired === 'failed') {
      if (freshParent.status !== JobStatus.Failed) {
        ...
        try {
          const { formatChildFailure, inferFailureCodeFromMessage } =
            await import('@/lib/failureCodes');
          if (failedChild) {
            const raw = failedChild.lastError || '';
            const childCode = raw.includes('::')
              ? raw.split('::')[0]
              : inferFailureCodeFromMessage(raw);
            parentLastError = formatChildFailure(
              String(failedChild.id),
              String(failedChild.jobType || 'unknown'),
              childCode as any
            );
          }
        } catch {}
        ...
        await prisma.hydrationJob.update({
          where: { id: parentId },
          data: { status: JobStatus.Failed, lastError: parentLastError },
        });
    ...
    try {
      await prisma.hydrationJob.updateMany({
        where: { id: parentId, lockedAt: now },
        data: { lockedAt: null },
      });
    } catch (e) {
