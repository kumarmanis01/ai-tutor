  ...
  for (const job of pending) {
    try {
      const logs = await prisma.jobExecutionLog.findMany({
        where: { jobId: job.id, event: 'ENQUEUED' },
      ...
      if (String(job.jobType) === 'syllabus') {
        ...
        const subjectId =
          payload.subjectId ||
          payload.resolvedMeta?.subjectId ||
          payload.resolvedMeta?.subject?.id ||
          null;
        ...
        if (!subjectId || !board || !grade) {
          ...
          console.warn(
            '[requeue] skipping legacy enqueue for',
            job.id,
            'missing subject/board/grade'
          );
          try {
            await prisma.jobExecutionLog.create({
              data: {
                jobId: job.id,
                event: 'REQUEUE_SKIPPED_MISSING_METADATA',
                prevStatus: 'pending',
                newStatus: 'pending',
                message: 'Missing subject/board/grade -- manual review required',
                meta: {
                  missingFields: { subjectId: !!subjectId, board: !!board, grade: !!grade },
        ...
        let hydration = await prisma.hydrationJob.findFirst({
          where: {
            jobType: 'syllabus',
            subjectId,
            grade,
        ...
        if (!hydration) {
          ...
          hydration = await prisma.hydrationJob.create({
            data: {
              id: generatedId,
              rootJobId: generatedId,
              jobType: 'syllabus',
              board,
              grade,
              subjectId,
          ...
          console.log(
            '[requeue] reusing existing HydrationJob',
            hydration.id,
            'for ExecutionJob',
            job.id
          );
        ...
        const bullJob = await q.add(
          `syllabus-${hydration.id}`,
          { type: 'SYLLABUS', payload: { jobId: hydration.id } },
          { jobId: hydration.id }
        );
        await prisma.jobExecutionLog.create({
          data: {
            ...
            meta: {
              ...
              hydrationJobId: hydration.id,
            },
          },
        });
        console.log(
          '[requeue] enqueued HydrationJob',
          hydration.id,
          'for ExecutionJob',
          job.id,
          'bullJobId=',
      ...
      const bullJob = await q.add(
        `${workerType.toLowerCase()}-${job.id}`,
        { type: workerType, payload: { jobId: job.id, ...(job.payload || {}) } },
        { jobId: job.id }
      );
      ...
      await prisma.jobExecutionLog.create({
        data: {
          ...
          prevStatus: 'pending',
          newStatus: 'pending',
          meta: { queue: 'content-hydration', workerType, bullJobId: bullJob?.id },
        },
      });
