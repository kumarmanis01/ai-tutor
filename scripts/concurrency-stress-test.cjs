...
process.env.DATABASE_URL =
  process.env.DATABASE_URL || 'postgresql://postgres:spinzyacademy@localhost:5432/neon_local';
...
const ACTIONS_PER_USER =
  parseInt(getCliArg('--actions') || process.env.ACTIONS_PER_USER || '25', 10) || 25;
...
(function loadPrisma() {
  ...
  console.error(
    'Prisma client not found. Ensure @prisma/client is installed and reachable from scripts.'
  );
  process.exit(1);
})();
...
if (!_fetch) {
  try {
    ...
    console.error(
      'fetch not available and node-fetch not installed. Please run on Node >=18 or install node-fetch.'
    );
    process.exit(1);
  }
}
...
async function runStudentSimulation(user, cookie) {
  try {
    for (let i = 0; i < ACTIONS_PER_USER; i++) {
      ...
      if (
        String(actionKind).includes('practice') ||
        String(JSON.stringify(body)).toLowerCase().includes('practice')
      ) {
        // best-effort payload
        ...
        String(actionKind).includes('note') ||
        String(JSON.stringify(body)).toLowerCase().includes('note')
      ) {
...
async function scanForSTMCompositeKeys() {
  ...
  try {
    ...
    return { error: 'ioredis not installed' };
  }
    ...
    try {
      ...
      return {
        error:
          ...
          (connErr && connErr.message ? connErr.message : String(connErr)),
      };
    }
...
(async function main() {
  console.log(
    'Starting concurrency stress test: users=',
    USER_COUNT,
    'actionsPerUser=',
    ACTIONS_PER_USER
  );
  ...
  try {
    ...
    if (Array.isArray(badSessions) && badSessions.length > 0) {
      console.error(
        'Learning sessions inconsistent (endedAt=null && isCompleted=true):',
        badSessions.length
      );
