...
;(async () => {
  try {
    ...
    if (level2Count > 0) {
      const l2 = await prisma.hydrationJob.findMany({ where: { rootJobId: jobId, hierarchyLevel: 2 }, take: 100 })
      console.log('\nLEVEL2 JOBS:\n', JSON.stringify(l2, null, 2))
    }

    if (level3Count > 0) {
      const l3 = await prisma.hydrationJob.findMany({ where: { rootJobId: jobId, hierarchyLevel: 3 }, take: 200 })
