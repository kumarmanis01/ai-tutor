...
async function main() {
  ...
  for (const j of args) {
    try {
      ...
      const outMatches = await prisma.$queryRawUnsafe(
        `SELECT "id", "queue", "payload", "meta", "createdAt" FROM "Outbox" WHERE "payload"::text ILIKE '%${String(j).replace(/'/g, "''")}%' LIMIT 20`
      );
      ...
      if (!rows || rows.length === 0) {
        ...
        for (const r of rows) {
          ...
          try {
            console.log(' requestBody:', JSON.stringify(r.requestBody, null, 2));
          } catch (e) {
            console.log(' requestBody:', String(r.requestBody));
          }
          try {
            console.log(' responseBody:', JSON.stringify(r.responseBody, null, 2).slice(0, 2000));
          } catch (e) {
            console.log(' responseBody (raw):', String(r.responseBody).slice(0, 2000));
          }
        }
      }
    } catch (err) {
