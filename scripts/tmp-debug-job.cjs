...
;(async () => {
  try {
    ...
    let topics = []
    if (hj && hj.subjectId) {
      topics = await prisma.topicDef.findMany({ where: { chapter: { subjectId: hj.subjectId }, lifecycle: 'active' }, take: 200 })
      console.log('\nTOPICS_FOR_SUBJECT count=', topics.length)
      console.log(JSON.stringify(topics.map(t => ({ id: t.id, title: t.title, chapterId: t.chapterId })), null, 2))
    } else {
      console.log('\nNo subjectId on root job; skipping topic query')
    }

    const recent = await prisma.outbox.findMany({ where: { meta: { path: ['hydrationJobId'], equals: jobId } }, orderBy: { createdAt: 'desc' }, take: 20 })
