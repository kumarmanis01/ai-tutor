...
async function seed() {
  ...
  for (let i = 0; i < N; i += batch) {
    ...
    for (let j = 0; j < chunk; j++) {
      ...
      values.push(
        `('shared','shared',NULL,${escapeLiteral(q)},${escapeLiteral(a)},${escapeLiteral(SUBJECT_ID)},NULL,NULL,NULL,NULL,NULL)`
      );
    }
...
async function latencyTrial(ids: string[], trials = TRIALS, probes = 1) {
  ...
  for (let t = 0; t < trials; t++) {
    ...
    const rows: any[] = await prisma.$queryRawUnsafe(
      `SELECT embedding FROM "DoubtKb" WHERE id = $1`,
      id
    );
...
async function main() {
  ...
  for (const p of probesToTry) {
    ...
    console.log(
      `probes=${p} avgMs=${result.avgMs.toFixed(2)}ms recall=${(result.recall * 100).toFixed(2)}%`
    );
  }
