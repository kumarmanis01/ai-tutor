...
export async function main() {
  ...
  if (!board || !grade) {
    console.error(
      'Usage: npx tsx scripts/check-curriculum-seed.ts --board CBSE --grade 10 [--subjects math,science] [--expected 100]'
    );
  ...
  if (subjects && subjects.length > 0) {
    for (const subject of subjects) {
      const cnt = await prisma.curriculumChunk.count({
        where: { board: String(board), grade: String(grade), subject: subject },
      });
