...
async function findLegacyTopicIds(count, lang) {
  ...
  const topicIdCol =
    cols.find((c) => /topic[_]?id$/i.test(c)) ||
    cols.find((c) => /^topicid$/i.test(c)) ||
    cols.find((c) => /topic/i.test(c)) ||
    'topicId';
  const updatedAtCol =
    cols.find((c) => /^updated_at$/i.test(c)) ||
    cols.find((c) => /^updatedat$/i.test(c)) ||
    cols.find((c) => /updated/i) ||
    'updatedAt';
  const languageCol =
    cols.find((c) => /^language$/i.test(c)) || cols.find((c) => /^lang$/i.test(c)) || 'language';
    ...
    if (opts.dryRun) {
      ...
      for (const tid of topicIds) {
        const note = await prisma.topicNote.findFirst({
          where: { topicId: tid, language: lang },
          select: { id: true, updatedAt: true, contentJson: true },
        });
        if (note) {
          const snippet = String(JSON.stringify(note.contentJson))
            .slice(0, 400)
            .replace(/\s+/g, ' ');
          console.log(
            `  sample note: id=${note.id} updatedAt=${note.updatedAt} snippet=${snippet}`
          );
        } else {
