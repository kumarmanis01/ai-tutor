    ...
    try {
      ...
      for (const exec of linkedExecs) {
        if (exec.status !== 'failed' && exec.status !== 'completed') {
          ...
          await prisma.executionJob.update({ where: { id: exec.id }, data: { status: 'failed', lastError: h.lastError ?? 'hydration_failed' } })
          await prisma.jobExecutionLog.create({ data: { jobId: exec.id, event: 'RECONCILE_MARK_FAILED', prevStatus: exec.status ?? null, newStatus: 'failed', message: h.lastError ?? 'hydration_failed', meta: { hydrationJobId: h.id } } })
        }
      }
    } catch (e) {
