...
async function main() {
  ...
  for (const h of failedHydrations) {
    try {
      const linkedExecs = await prisma.executionJob.findMany({
        where: { payload: { path: ['hydrationJobId'], equals: h.id } },
      });
      for (const exec of linkedExecs) {
        if (exec.status !== 'failed' && exec.status !== 'completed') {
          ...
          await prisma.executionJob.update({
            where: { id: exec.id },
            data: { status: 'failed', lastError: h.lastError ?? fallback },
          });
          await prisma.jobExecutionLog.create({
            data: {
              jobId: exec.id,
              event: 'RECONCILE_MARK_FAILED',
              prevStatus: exec.status ?? null,
              newStatus: 'failed',
              message: h.lastError ?? fallback,
              meta: { hydrationJobId: h.id },
            },
          });
        }
      }
