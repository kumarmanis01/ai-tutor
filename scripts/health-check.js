(async () => {
  try {
    ...
    if (!jres.ok) {
      ...
      if (jres.status === 403) {
        console.warn(
          'Jobs status endpoint returned 403 (forbidden). Skipping jobs check in local/dev.'
        );
      } else {
    ...
    try {
      ...
      console.warn(
        'Jobs status endpoint did not return JSON; treating as no jobs for health check'
      );
    }
