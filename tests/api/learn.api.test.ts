...
const mockPrisma: any = {
  coursePackage: { findMany: jest.fn(), findFirst: jest.fn() },
...
describe('learn APIs', () => {
  ...
  test('GET /api/learn/courses/:courseId returns json or 404', async () => {
    ...
    (global as any).__TEST_SESSION__ = { user: { id: 'u1' } };
    const res2 = await course.GET(
      new Request('http://localhost') as any,
      { params: { courseId: 'missing' } } as any
    );
  ...
  test('GET lesson returns a lesson by index', async () => {
    ...
    mockPrisma.regenerationOutput = {
      findUnique: jest
        .fn()
        .mockResolvedValue({ id: 'o1', contentJson: { id: 'l2', title: 'L2 promoted' } }),
    };

    const res = await lesson.GET(
      new Request('http://localhost') as any,
      { params: { courseId: 'c1', index: '2' } } as any
    );
    const data = await res.json();
    expect(data.id).toBe('l2');

    const res2 = await lesson.GET(
      new Request('http://localhost') as any,
      { params: { courseId: 'c1', index: '99' } } as any
    );
    expect(res2.status).toBe(404);
  });
});
