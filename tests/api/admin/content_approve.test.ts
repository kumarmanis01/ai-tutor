describe('admin content approve route', () => {
  ...
  test('creates audit log with resolved user id when DB user exists', async () => {
    ...
    const req = new Request('http://localhost', {
      method: 'POST',
      body: JSON.stringify({ type: 'topic', id: 't1', action: 'approve' }),
    ...
    expect(mockPrisma.auditLog.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ adminId: 'u1', action: 'CONTENT_APPROVE' }),
      })
    );
  });
  ...
  test('falls back to null userId when session user not in DB', async () => {
    ...
    (global as any).__TEST_PRISMA__ = mockPrisma;
    (global as any).__TEST_SESSION__ = {
      user: { id: 'ghost', email: 'ghost@example.com', role: 'admin' },
    };

    const route = await import('../../../app/api/admin/content/approve/route');
    const req = new Request('http://localhost', {
      method: 'POST',
      body: JSON.stringify({ type: 'topic', id: 't2', action: 'approve' }),
      headers: { 'Content-Type': 'application/json' },
    });
    const res = await route.POST(req as any);
    expect(res.status).toBe(200);
    ...
    expect(mockPrisma.auditLog.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ adminId: null, action: 'CONTENT_APPROVE' }),
      })
    );
  });
});
