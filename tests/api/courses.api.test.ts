...
describe('courses API', () => {
  ...
  test('GET /api/courses/:syllabusId/:version returns package or 404', async () => {
    (mockPrisma.coursePackage.findUnique as jest.Mock).mockResolvedValue({
      syllabusId: 's1',
      version: 1,
      status: 'PUBLISHED',
      json: { foo: 'bar' },
    ...
    const res = await single.GET(
      undefined as any,
      { params: { syllabusId: 's1', version: '1' } } as any
    );
    const data = await res.json();
    expect(data).toEqual({ foo: 'bar' });
    (mockPrisma.coursePackage.findUnique as jest.Mock).mockResolvedValue(null);
    const res2 = await single.GET(
      undefined as any,
      { params: { syllabusId: 's1', version: '9' } } as any
    );
    expect(res2.status).toBe(404);
  });
});
