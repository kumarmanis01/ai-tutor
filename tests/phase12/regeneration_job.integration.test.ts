...
describe('Phase 12 — RegenerationJob creation + audit', () => {
  ...
  it('only allows ACCEPTED suggestions, is idempotent, and emits REGEN_JOB_CREATED', async () => {
    ...
    if (process.env.NODE_ENV === 'test') {
      try {
        console.log('[debug] test sees suggestion:', testDirect);
      } catch {}
    }
    ...
    const res1: any = await (
      await createJob(
        makeReq({ suggestionId: suggestion.id, targetType: 'LESSON', targetId: 'lesson-42' })
      )
    ).json();
    ...
    const auditFound = await waitFor(
      async () => {
        const audits = await prisma.auditLog.findMany({
          where: { action: AuditEvents.REGEN_JOB_CREATED },
        });
        return audits.some(
          (a) => (a as any).details?.suggestionId === suggestion.id || (a as any).entityId === jobId
        );
      },
      2000,
      50
