...
describe('Phase 11 — ContentSuggestion idempotency', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      const pingDb = (urlStr: string, timeout = 500) =>
    ...
    try {
      ...
      logger.warn(
        'Skipping Phase 11 idempotency test: ContentSuggestion table missing or schema drift',
        String(e)
      );
      SKIP_TEST = true;
      return;
  ...
  it('creates exactly one suggestion per signal across repeated runs and audits once', async () => {
    ...
    await saveSuggestions(
      prisma as any,
      suggestions1.map((s: any) => ({ ...s, sourceSignalId: dbSignal.id }))
    );
    ...
    const found = await waitFor(
      async () => {
        ...
        return audits.some(
          (a) =>
            (a as any).entityId === suggestion.id || (a as any).details?.entityId === suggestion.id
        );
      },
      2000,
      50
    ...
    await saveSuggestions(
      prisma as any,
      suggestions2.map((s: any) => ({ ...s, sourceSignalId: dbSignal.id }))
    );
    ...
    const auditsAfterSecond = await prisma.auditLog.findMany({
    ...
    const createAuditsForSuggestion = auditsAfterSecond.filter(
      (a) => (a as any).entityId === suggestion.id || (a as any).details?.entityId === suggestion.id
    );
