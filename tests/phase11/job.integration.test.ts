...
describe('Phase 11 — Job-style suggestion processing idempotency', () => {
  ...
  it('processes all analytics signals into suggestions exactly once and audits creation', async () => {
    ...
    const foundAudit = await waitFor(
      async () => {
        const audits = await prisma.auditLog.findMany({
          where: { targetEntity: 'ContentSuggestion', targetId: { in: suggestionIds } },
        });
    ...
    const auditsAfterFirst = await prisma.auditLog.findMany({
      where: { targetEntity: 'ContentSuggestion', targetId: { in: suggestionIds } },
    });
    ...
    const auditsAfterSecond = await prisma.auditLog.findMany({
      where: { targetEntity: 'ContentSuggestion', targetId: { in: suggestionIdsAfter } },
    });
