...
const valid = {
  ...
  problemStatement:
    'Create a full-stack ToDo application implementing CRUD operations, user authentication, and persistent storage. The app must include pagination, search, and tests for core business logic. Provide deployment instructions and a short demo guide.'.repeat(
      2
    ),
...
test('valid mock LLM output succeeds', async () => {
  ...
  const out = await generateProject(
    { syllabusId: 's1', moduleId: 'm1', moduleTitle: 'M', learningObjectives: ['o1'] },
    llm
  );
...
test('schema violation throws', async () => {
  ...
  ).rejects.toThrow(/Project validation failed/i);
});
