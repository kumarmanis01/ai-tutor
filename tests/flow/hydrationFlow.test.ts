...
describe('Full hydration flow: syllabus -> notes -> questions -> assembleTest', () => {
  ...
  test('generates syllabus, topics, notes and questions and approves test', async () => {
    ...
    (prisma.hydrationJob.findUnique as jest.Mock).mockResolvedValue({
      id: 'job-1',
      subjectId: 'sub-1',
      board: 'CBSE',
      grade: 5,
      ...
      chapters: [
        {
          ...
          order: 1,
          topics: [
            { title: 'Integers', order: 1 },
            { title: 'Fractions', order: 2 },
          ],
        },
        { title: 'Geometry', order: 2, topics: [{ title: 'Lines', order: 1 }] },
      ],
    };
    ...
    const tx = {
      chapterDef: {
        ...
        create: jest.fn().mockResolvedValue({ id: 'chap-1', name: 'Number System' }),
      },
      topicDef: {
        findFirst: jest.fn().mockResolvedValue(null),
        create: jest.fn().mockResolvedValue({ id: 'topic-1', name: 'Integers' }),
      },
      hydrationJob: { update: jest.fn().mockResolvedValue({}) },
      executionJob: {
        findFirst: jest.fn().mockResolvedValue({ id: 'exec-1', status: 'pending' }),
    ...
    (prisma.topicDef.findUnique as jest.Mock).mockResolvedValue({
      ...
      chapter: {
        id: 'chap-1',
        subject: { id: 'sub-1', name: 'Mathematics', class: { grade: 5, board: { name: 'CBSE' } } },
      },
    });
    ...
    expect(prisma.topicNote.create).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ title: 'Integers - Notes' }) })
    );
    ...
    const questionsJson = {
      questions: [
        ...
        { type: 'mcq', question: 'What is -1 + 1?', options: ['0', '1', '-1'], answer: '0' },
        { type: 'mcq', question: 'Which is even?', options: ['3', '4', '5'], answer: '4' },
        {
          type: 'mcq',
          question: 'Integer between 1 and 3?',
          options: ['1', '2', '4'],
          answer: '2',
        },
        { type: 'mcq', question: 'Add 10 and 5', options: ['15', '14', '16'], answer: '15' },
      ],
    };
    ...
    expect(prisma.generatedTest.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ title: expect.stringContaining('Integers') }),
      })
    );
    ...
    (prisma.generatedTest.findMany as jest.Mock).mockResolvedValue([
      { id: 'test-1', questions: [1, 2, 3, 4, 5] },
    ]);
    (prisma.generatedTest.update as jest.Mock).mockResolvedValue({});

    await assembleTest('topic-1');

    expect(prisma.generatedTest.update).toHaveBeenCalledWith({
      where: { id: 'test-1' },
