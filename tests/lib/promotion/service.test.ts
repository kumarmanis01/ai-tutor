...
describe('Promotion service', () => {
  ...
  test('approveCandidate throws when already approved or rejected', async () => {
    ...
    await expect(svc.approveCandidate('pc1', 'admin')).rejects.toThrow(
      'candidate already approved'
    );
    prisma.promotionCandidate.findUnique.mockResolvedValue({ id: 'pc2', status: 'REJECTED' });
    await expect(svc.approveCandidate('pc2', 'admin')).rejects.toThrow(
  ...
  test('approveCandidate updates candidate status to APPROVED', async () => {
    ...
    expect(prisma.promotionCandidate.update).toHaveBeenCalledWith({
      where: { id: 'pc1' },
  ...
  test('rejectCandidate updates candidate and audits', async () => {
    prisma.promotionCandidate.findUnique.mockResolvedValue({
      id: 'pc1',
      status: 'PENDING',
      scope: 'LESSON',
      scopeRefId: 'l1',
