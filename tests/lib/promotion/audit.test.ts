...
describe('Promotion audit events', () => {
  ...
  test('store.createPromotionCandidate emits audit event', async () => {
    ...
    await store.createPromotionCandidate({
      scope: 'LESSON',
      scopeRefId: 'l1',
      regenerationJobId: 'j1',
      outputRef: 'o1',
      createdBy: 'admin',
    });
    expect(logAuditEvent).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        action: null,
        details: expect.objectContaining({ legacyAction: 'PROMOTION_CANDIDATE_CREATED' }),
      })
    );
  });
  ...
  test('service.approveCandidate emits audit event', async () => {
    const prisma: any = {
      promotionCandidate: {
        ...
        update: jest.fn().mockResolvedValue({ id: 'pc1', status: 'APPROVED' }),
      },
    ...
    expect(logAuditEvent).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        action: null,
        details: expect.objectContaining({ legacyAction: 'PROMOTION_APPROVED' }),
      })
    );
  });
  ...
  test('service.rejectCandidate emits audit event', async () => {
    const prisma: any = {
      promotionCandidate: {
        findUnique: jest
          .fn()
          .mockResolvedValue({ id: 'pc1', status: 'PENDING', scope: 'LESSON', scopeRefId: 'l1' }),
        update: jest.fn().mockResolvedValue({ id: 'pc1', status: 'REJECTED' }),
      },
    };
    const svc = makePromotionService(prisma);
    ...
    expect(logAuditEvent).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        action: null,
        details: expect.objectContaining({ legacyAction: 'PROMOTION_REJECTED' }),
      })
    );
  });
});
