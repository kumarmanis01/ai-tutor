  ...
  it('creates low completion signal when completionRate low', async () => {
    const mockDb: any = {
      analyticsDailyAggregate: {
        findFirst: jest
          .fn()
          .mockResolvedValue({ completionRate: 0.1, day: new Date('2025-12-21') }),
      },
    ...
    const mockDb: any = {
      ...
      analyticsEvent: {
        count: jest.fn().mockImplementation(({ where }: any) => {
          if (where.eventType === 'quiz_attempted') return Promise.resolve(20);
          if (where.eventType === 'quiz_passed') return Promise.resolve(4);
