jest.mock('@/lib/prisma', () => ({
  prisma: {
    regenerationJob: { findFirst: jest.fn(), updateMany: jest.fn(), update: jest.fn() },
    $transaction: jest.fn(),
    regenerationOutput: { create: jest.fn() },
  },
}));
...
test('processNextJob persists output exactly once and emits COMPLETED audit', async () => {
  ...
  const actions = mockedLogAudit.mock.calls.map(
    (c: any) => c[1]?.details?.legacyAction || c[0]?.details?.legacyAction
  );
...
test('processNextJob marks FAILED and emits FAILED audit when generator throws', async () => {
  ...
  mockedGenerator.mockImplementation(async () => {
