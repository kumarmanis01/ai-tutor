...
describe('Regeneration worker (DB-backed, mocked generator)', () => {
  ...
  it('1) PENDING job is claimed and completed', async () => {
    ...
    const job = await prisma.regenerationJob.create({
      data: {
        id,
        suggestionId: `s-${id}`,
        targetType: 'LESSON',
    ...
    if ((adapter as any).default && (adapter as any).default.mockResolvedValue) {
      (adapter as any).default.mockResolvedValue({
        validated: true,
        outputJson: { content: 'new-lesson' },
      });
    }
    if ((adapter as any).mockResolvedValue) {
      (adapter as any).mockResolvedValue(
        Promise.resolve({ validated: true, outputJson: { content: 'new-lesson' } })
      );
    }
    ...
    const audits = await prisma.auditLog.findMany({
      where: { action: { in: ['REGEN_JOB_LOCKED', 'REGEN_JOB_STARTED', 'REGEN_JOB_COMPLETED'] } },
    });
    const actionsForJob = audits
      .filter((a) => a.details && (a.details as any).jobId === job.id)
      .map((a) => a.action);
    expect(actionsForJob).toEqual(
      expect.arrayContaining(['REGEN_JOB_LOCKED', 'REGEN_JOB_STARTED', 'REGEN_JOB_COMPLETED'])
    );
  });
  ...
  it('4) Double execution prevented', async () => {
    ...
    if ((adapter as any).default && (adapter as any).default.mockResolvedValue) {
      (adapter as any).default.mockResolvedValue({
        validated: true,
        outputJson: { content: 'race' },
      });
    }
    if ((adapter as any).mockResolvedValue) {
      (adapter as any).mockResolvedValue(
        Promise.resolve({ validated: true, outputJson: { content: 'race' } })
      );
    }
  ...
  it('5) Output written once', async () => {
    ...
    if ((adapter as any).default && (adapter as any).default.mockResolvedValue) {
      (adapter as any).default.mockResolvedValue({
        validated: true,
        outputJson: { content: 'once' },
      });
    }
    if ((adapter as any).mockResolvedValue) {
      (adapter as any).mockResolvedValue(
        Promise.resolve({ validated: true, outputJson: { content: 'once' } })
      );
    }
  ...
  it('6) Audit events emitted on COMPLETED and FAILED', async () => {
    ...
    const audits = await prisma.auditLog.findMany({
      where: { action: { in: ['REGEN_JOB_LOCKED', 'REGEN_JOB_STARTED', 'REGEN_JOB_FAILED'] } },
    });
    const actionsForJob = audits
      .filter((a) => a.details && (a.details as any).jobId === job.id)
      .map((a) => a.action);
    expect(actionsForJob).toEqual(
      expect.arrayContaining(['REGEN_JOB_LOCKED', 'REGEN_JOB_STARTED', 'REGEN_JOB_FAILED'])
    );
  });
});
