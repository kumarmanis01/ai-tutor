...
describe('Promotion invariants', () => {
  test('two candidates for same scope can both be approved sequentially', async () => {
    const prisma: any = {
      promotionCandidate: {
        findUnique: jest.fn(),
        update: jest.fn().mockResolvedValue({ id: 'pc', status: 'APPROVED' }),
      },
    ...
    // First approval
    prisma.promotionCandidate.findUnique.mockResolvedValueOnce({
      id: 'pc1',
      status: 'PENDING',
      scope: 'LESSON',
      scopeRefId: 'l1',
    ...
    expect(prisma.promotionCandidate.update).toHaveBeenNthCalledWith(1, {
      where: { id: 'pc1' },
      data: expect.objectContaining({ status: 'APPROVED' }),
    });
    expect(prisma.promotionCandidate.update).toHaveBeenNthCalledWith(2, {
      where: { id: 'pc2' },
      data: expect.objectContaining({ status: 'APPROVED' }),
    });
  });
