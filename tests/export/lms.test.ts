...
describe('LMS exporter', () => {
          ...
          lessons: [
            {
              title: 'L1',
              objectives: ['o1'],
              explanation: { overview: 'ov1', concepts: [{ title: 'c1', explanation: 'ce1' }] },
            },
            { title: 'L2' },
          ],
        },
  ...
  it('LMS export endpoint writes audit log', async () => {
    const mockPrisma: any = {
      coursePackage: {
        findFirst: jest.fn().mockResolvedValue({ id: 'pkg1', json: { title: 'T' } }),
      },
  ...
  it('cannot access LMS export for different tenant', async () => {
    const mockPrisma: any = {
      ...
      product: {
        findFirst: jest
          .fn()
          .mockResolvedValue({ id: 'prod1', courseId: 'c1', tenantId: 'tenantX', active: true }),
      },
      purchase: { findFirst: jest.fn().mockResolvedValue(null) },
      enrollment: { findFirst: jest.fn().mockResolvedValue(null) },
      auditLog: { create: jest.fn() },
