jest.mock('@/lib/callLLM.js', () => ({
  callLLM: jest
    .fn()
    .mockResolvedValue({
      content: JSON.stringify({ questions: [{ type: 'numeric', question: '2+2', answer: '4' }] }),
    }),
}));
...
const mockPrisma = {
  ...
  topicDef: {
    findUnique: jest
      ...
      .mockResolvedValue({
        ...
        chapter: {
          name: 'ChapterQ',
          subject: { name: 'Mathematics', class: { grade: 7, board: { name: 'CBSE' } } },
        },
      }),
  },
  generatedTest: {
    findFirst: jest.fn().mockResolvedValue(null),
    create: jest.fn().mockResolvedValue({ id: 'test1' }),
  },
  ...
  executionJob: {
    findFirst: jest
      .fn()
      .mockResolvedValue({ id: 'execQ1', payload: { hydrationJobId: 'jobQ1' }, status: 'pending' }),
    update: jest.fn().mockResolvedValue({}),
  },
