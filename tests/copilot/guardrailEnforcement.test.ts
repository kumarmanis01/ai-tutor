...
import { processPrompt, RewriteStrategy } from '@/lib/ai/guardrails/promptRewriter';
...
export const MOCK_AI_RESPONSES = {
  ...
  VALID_COMPLEX: {
    content:
      'Photosynthesis is the process by which plants convert light energy into chemical energy. The equation is 6CO₂ + 6H₂O + light → C₆H₁₂O₆ + 6O₂.',
    confidence: 0.85,
  },
  ...
  FALSE_CERTAINTY: {
    content:
      'It is absolutely certain that all plants need exactly 12 hours of sunlight. This is always true without exception.',
    confidence: 0.5,
  },

  TOO_COMPLEX_FOR_JUNIOR: {
    content:
      'The epistemological implications of mathematical axioms necessitate a comprehensive understanding of metamathematical frameworks.',
    confidence: 0.7,
  },
};
...
describe('Intent Classification Guardrails', () => {
  ...
  describe('Shortcut Seeking Detection', () => {
    ...
    it('should detect "tell me the solution quickly" as SHORTCUT_SEEKING', () => {
      const result = classifyIntent(
        'tell me the solution quickly without explanation',
        8,
        'Mathematics'
      );
      expect(result.category).toBe(StudentIntentCategory.SHORTCUT_SEEKING);
  ...
  describe('Homework Dump Detection', () => {
    ...
    it('should detect "solve all these" as HOMEWORK_DUMP', () => {
      const result = classifyIntent(
        'solve all these questions for me: Q1... Q2... Q3...',
        7,
        'Mathematics'
      );
      expect(result.category).toBe(StudentIntentCategory.HOMEWORK_DUMP);
...
describe('Prompt Rewriting Guardrails', () => {
  describe('Shortcut to Learning Rewrite', () => {
    ...
    it('should preserve the core question in rewrite', () => {
      const result = processPrompt('just tell me what is photosynthesis', 5, 'Science');
  ...
  describe('Homework Redirect', () => {
    it('should redirect homework dumps to single-question format', () => {
      const result = processPrompt('Solve: 1) 2+2 2) 3+3 3) 4+4', 3, 'Mathematics');
  ...
  describe('No Rewrite Needed', () => {
    it('should not rewrite valid conceptual questions', () => {
      const result = processPrompt('Can you explain how photosynthesis works?', 7, 'Science');
  ...
  describe('Junior Grade Adjustments', () => {
    it('should use simpler rewrite templates for junior grades', () => {
      const juniorResult = processPrompt('just give answer', 2, 'Mathematics');

      const seniorResult = processPrompt('just give answer', 9, 'Mathematics');
...
describe('Hallucination Detection Guardrails', () => {
  ...
  describe('Valid Content Passes', () => {
    ...
    it('should pass complex valid content for appropriate grade', () => {
      const result = checkForHallucinations(MOCK_AI_RESPONSES.VALID_COMPLEX.content, 8, 'Science');
