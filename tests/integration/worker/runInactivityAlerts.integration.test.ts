...
describe('runInactivityAlerts (integration with Redis)', () => {
  ...
  it('sets suppression key, prevents double-send, and updateStreak clears it', async () => {
    ...
    const prismaMock = {
      user: {
        findMany: jest.fn(async () => [{ id: studentId, name: 'Ivy', timezone: 'UTC', lastSessionDate: new Date(Date.now() - 6 * 24 * 60 * 60 * 1000) }]),
        findUnique: jest.fn(async () => ({ lastSessionDate: new Date(Date.now() - 6 * 24 * 60 * 60 * 1000), currentStreak: 2, longestStreak: 5, timezone: 'UTC' })),
        update: jest.fn(async (args: any) => args),
      },
      parentStudent: {
        findMany: jest.fn(async (args: any) => {
          if (args && args.select) return [{ parentId }]
          return [{ parent: { id: parentId, email: 'p@ex.com', phone: null, name: 'P', parentProfile: { digestOptOut: false, inactivityOptOut: false } } }]
        }),
      },
