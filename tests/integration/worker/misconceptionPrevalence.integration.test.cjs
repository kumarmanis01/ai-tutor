...
(async function main() {
  ...
  try {
    ...
    const mis = await prisma.misconception.create({
      data: {
        subjectId: 'INT_SUB',
        conceptId,
        name: `INT_MIS_${Date.now()}`,
    ...
    for (let i = 0; i < 4; i++) {
      detections.push(
        prisma.studentMisconception.create({
          data: {
            studentId: `s-${100 + i}`,
    ...
    const p = spawn(process.execPath, ['-r', 'ts-node/register/transpile-only', runnerPath], {
      env: process.env,
    });
    ...
    if (refreshed.prevalenceRate <= 0.3)
      throw new Error(`Expected prevalenceRate > 0.3, got ${refreshed.prevalenceRate}`);

    // Verify a system alert exists for high prevalence
    const alerts = await prisma.systemAlert.findMany({
      where: { type: 'MISCONCEPTION_HIGH_PREVALENCE' },
    });
    if (!alerts || alerts.length === 0)
      throw new Error('Expected at least one MISCONCEPTION_HIGH_PREVALENCE alert');
    ...
    await prisma.systemAlert
      .deleteMany({ where: { type: 'MISCONCEPTION_HIGH_PREVALENCE' } })
      .catch(() => {});
    await prisma.studentMisconception
      .deleteMany({ where: { misconceptionId: mis.id } })
      .catch(() => {});
