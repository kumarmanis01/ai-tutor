...
jest.mock('@/lib/callLLM', () => ({ callLLM: (...a: any[]) => Promise.resolve(mockCallLLM(...a)) }))
...
test('integration: readiness drop alerts send remediation', async () => {
  ...
  mockParentFind.mockResolvedValueOnce([{ parentId, studentId, parent: { name: 'Kiran', email: 'k@example.com', phone: '9000000001' } }])
