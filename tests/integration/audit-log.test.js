...
(async function main() {
  ...
  try {
    ...
    const approveLogs = await prisma.auditLog.findMany({
      where: { action: 'approve_topic', details: { path: ['topicId'], equals: topic.id } },
    });
    ...
    const pendingJob = await prisma.executionJob.create({
      data: {
        jobType: 'notes',
        entityType: 'TOPIC',
        entityId: topic.id,
    ...
    await prisma.$transaction([
      ...
      prisma.executionJob.update({
        ...
        data: { status: 'cancelled', lastError: 'TEST_CANCELLED::Cancelled by test' },
      }),
      prisma.auditLog.create({
        data: {
          userId: null,
          action: 'cancel_job',
          details: { jobId: pendingJob.id, prevStatus: 'pending' },
          createdAt: new Date(),
        },
      }),
    ]);
    const cancelLogs = await prisma.auditLog.findMany({
      where: { action: 'cancel_job', details: { path: ['jobId'], equals: pendingJob.id } },
    });
    ...
    const failedJob = await prisma.executionJob.create({
      data: {
        jobType: 'notes',
        entityType: 'TOPIC',
        entityId: topic.id,
    ...
    const retryJob = await prisma.executionJob.create({
      data: {
        jobType: 'notes',
        entityType: 'TOPIC',
        entityId: topic.id,
    ...
    await prisma.auditLog.create({
      data: {
        userId: null,
        action: 'retry_job',
        details: { originalJobId: failedJob.id, newJobId: retryJob.id },
        createdAt: new Date(),
      },
    });
    const retryLogs = await prisma.auditLog.findMany({
      where: { action: 'retry_job', details: { path: ['originalJobId'], equals: failedJob.id } },
    });
    ...
    await prisma.$transaction([
      ...
      prisma.auditLog.create({
        data: {
          userId: null,
          action: 'soft_delete_chapter',
          details: { chapterId: ids.chapterId },
          createdAt: new Date(),
        },
      }),
    ]);
    const softDeleteLogs = await prisma.auditLog.findMany({
      where: {
        action: 'soft_delete_chapter',
        details: { path: ['chapterId'], equals: ids.chapterId },
      },
    });
    if (softDeleteLogs.length === 0)
      throw new Error('Audit log not created for soft_delete_chapter');
    ...
    await prisma.auditLog
      .deleteMany({
        where: {
          action: { in: ['approve_topic', 'cancel_job', 'retry_job', 'soft_delete_chapter'] },
        },
      })
      .catch(() => {});
  ...
  async function ensureChapter(prisma) {
    ...
    const board = await prisma.board.create({
      data: { name: 'INT_BOARD', slug: `int-board-${Date.now()}`, createdAt: new Date() },
    });
    const classLevel = await prisma.classLevel.create({
      data: { grade: 1, slug: `g1-${Date.now()}`, boardId: board.id },
    });
    const subject = await prisma.subjectDef.create({
      data: { name: 'INT_SUBJECT', slug: `s-${Date.now()}`, classId: classLevel.id },
    });
    const chapter = await prisma.chapterDef.create({
      data: { name: 'INT_CHAPTER', slug: `c-${Date.now()}`, order: 1, subjectId: subject.id },
    });
    return {
      boardId: board.id,
