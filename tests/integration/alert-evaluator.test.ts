import logger from '../../lib/logger'; /* eslint-disable */
import { prisma } from '../../lib/prisma';
import evaluateAlerts from '@/lib/alertEvaluator.js';

(async function main() {
  if (!process.env.DATABASE_URL) {
    logger.info('Skipping integration tests: DATABASE_URL not set');
    process.exit(0);
  }
  ...
  try {
    ...
    await prisma.systemAlert
      .deleteMany({ where: { type: { in: ['QUEUE_BACKLOG', 'WORKER_STALE', 'JOB_STUCK'] } } })
      .catch(() => {});
    ...
    if (!alert1 || alert1.active !== true)
      throw new Error('Test1 failed: QUEUE_BACKLOG not active');
    logger.info('Test1 passed: QUEUE_BACKLOG created and active');
    ...
    if (alertResolved && alertResolved.active !== false)
      throw new Error('Test2 failed: QUEUE_BACKLOG not resolved');
    logger.info('Test2 passed: QUEUE_BACKLOG resolved');
    ...
    for (let i = 15; i >= 2; i--) {
      baseline.push({
        key: 'jobs.failed.count',
        timestamp: new Date(now.getTime() - i * 60 * 1000),
        value: 1,
        dimensionHash: 'int-test-3',
      });
    }
    // recent minute
    baseline.push({
    ...
    if (!jobAlert || jobAlert.active !== true)
      throw new Error('Test3 failed: JOB_STUCK not active');
    logger.info('Test3 passed: JOB_STUCK created and active');

    // cleanup
    await prisma.systemAlert
      .deleteMany({ where: { type: { in: ['QUEUE_BACKLOG', 'WORKER_STALE', 'JOB_STUCK'] } } })
      .catch(() => {});
    await prisma.telemetrySample
      .deleteMany({
        where: {
          key: { in: ['queue.depth.value', 'jobs.failed.count', 'queue.oldest_age_sec.value'] },
        },
      })
      .catch(() => {});
