jest.mock('@/hydrators/hydrateSyllabus', () => ({
  enqueueSyllabusHydration: jest
    .fn()
    .mockResolvedValue({ created: true, jobId: 'hyd-1', bullJobId: 'bull-1' }),
}));
...
describe('submitJob -> syllabus hydration integration', () => {
  ...
  test('creates ExecutionJob and calls hydrateSyllabus, links hydrationJobId', async () => {
    ...
    expect(enqueueSyllabusHydration).toHaveBeenCalledWith(
      expect.objectContaining({ board: 'CBSE', grade: 6, subject: 'Mathematics' })
    );
    // ExecutionJob payload updated to include hydrationJobId
    expect(prisma.executionJob.update).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: 'exec-1' },
        data: expect.objectContaining({
          payload: expect.objectContaining({ hydrationJobId: 'hyd-1' }),
        }),
      })
    );
    ...
    expect(prisma.jobExecutionLog.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ jobId: 'exec-1', event: 'ENQUEUED' }),
      })
    );
  });
});
