...
describe('handleSyllabusJob', () => {
  ...
  test('marks job failed when subjectId missing', async () => {
    ...
    expect(prisma.hydrationJob.update).toHaveBeenCalledWith({
      where: { id: 'job-1' },
      data: { status: expect.anything(), lastError: expect.stringMatching(/^[A-Z0-9_]+::.+/) },
    });
  });
  ...
  test('happy path creates chapters and topics and marks job completed', async () => {
    ...
    const llmOutput = JSON.stringify({
      chapters: [{ title: 'Numbers', order: 1, topics: [{ title: 'Integers', order: 1 }] }],
    });
    ...
    const tx = {
      chapterDef: {
        ...
        create: jest.fn().mockResolvedValue({ id: 'chap-1' }),
      },
      topicDef: {
        findFirst: jest.fn().mockResolvedValue(null),
    ...
    expect(tx.chapterDef.create).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ name: 'Numbers' }) })
    );
    expect(tx.topicDef.create).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ name: 'Integers' }) })
    );
    expect(tx.hydrationJob.update).toHaveBeenCalledWith({
      where: { id: 'job-2' },
      data: { status: 'running', contentReady: true },
    });
  ...
  test('should inject NCERT chapter hints into prompt when CurriculumChunk rows exist', async () => {
    ...
    (prisma.curriculumChunk.findMany as jest.Mock).mockResolvedValue([
      {
        conceptIds: ['chapter:1', 'chunkIndex:0', 'source:ncert'],
        content: 'Food: Where Does It Come From? All living organisms need food.',
      },
      {
        conceptIds: ['chapter:2', 'chunkIndex:0', 'source:ncert'],
        content: 'Components of Food. Food is made up of nutrients.',
      },
    ]);
      ...
      chapters: [
        {
          ...
          order: 1,
          topics: [{ title: 'Sources of Food', order: 1 }],
        },
    ...
    expect(tx.chapterDef.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ name: 'Food: Where Does It Come From?' }),
      })
    );
