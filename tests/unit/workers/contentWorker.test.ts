...
describe('contentWorker lifecycle', () => {
  ...
  test('processContentJob creates HydrationJob and emits STARTED log', async () => {
    await jest.isolateModulesAsync(async () => {
      ...
      (prisma.executionJob.findUnique as jest.Mock).mockResolvedValue({
        id: 'exec-1',
        entityType: 'SUBJECT',
        entityId: 'sub-1',
        payload: {},
      });
      (prisma.hydrationJob.findUnique as jest.Mock)
        .mockResolvedValueOnce(null)
        .mockResolvedValueOnce({ id: 'hyd-1', subjectId: 'sub-1' });
      (prisma.hydrationJob.findFirst as jest.Mock).mockResolvedValue(null);
      (prisma.hydrationJob.create as jest.Mock).mockResolvedValue({
        id: 'hyd-1',
        subjectId: 'sub-1',
      });
      (prisma.chapterDef.findFirst as jest.Mock).mockResolvedValue({
        id: 'chap-1',
        subjectId: 'sub-1',
      });
      ...
      expect(prisma.jobExecutionLog.create).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ event: 'STARTED' }) })
      );
    });
  });
  ...
  test('startContentWorker registers completed and failed handlers that finalize ExecutionJob', async () => {
    await jest.isolateModulesAsync(async () => {
      ...
      const completedJob = {
        id: 'bull-2',
        data: { type: 'SYLLABUS', payload: { executionJobId: 'exec-2' } },
      };
      (prisma.hydrationJob.findUnique as jest.Mock).mockResolvedValue({
        id: 'hyd-2',
        subjectId: 'sub-2',
      });
      (prisma.executionJob.findUnique as jest.Mock).mockResolvedValueOnce({
        id: 'exec-2',
        payload: { hydrationJobId: 'hyd-2' },
      });
      (prisma.executionJob.findFirst as jest.Mock).mockResolvedValueOnce({ id: 'exec-2' });
      (prisma.chapterDef.findFirst as jest.Mock).mockResolvedValueOnce({
        id: 'chap-2',
        subjectId: 'sub-2',
      });
      ...
      expect(prisma.jobExecutionLog.create).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ event: 'COMPLETED' }) })
      );

      // simulate failed
      const failedJob = {
        id: 'bull-3',
        data: { type: 'SYLLABUS', payload: { executionJobId: 'exec-3' } },
      };
      ...
      expect(prisma.jobExecutionLog.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ event: 'FAILED', jobId: 'exec-3' }),
        })
      );
    });
  });
