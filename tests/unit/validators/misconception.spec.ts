import {
  MisconceptionCreateSchema,
  MisconceptionPatchSchema,
} from '../../../lib/validators/misconception';
...
describe('misconception validators', () => {
  ...
  test('create schema rejects small triggerPatterns', () => {
    const res = MisconceptionCreateSchema.safeParse({
      name: 'n',
      correction: 'c',
      triggerPatterns: ['onlyone'],
      subjectId: 's',
      conceptId: 'c',
    });
    expect(res.success).toBe(false);
