...
describe('grantConsent', () => {
  it('saves ipAddress and userAgent to the consent record (create path)', async () => {
    ...
    await grantConsent('user-1', [ConsentScope.AI_INTERACTION], {
      ipAddress: '1.2.3.4',
...
describe('withdrawConsent', () => {
  it('initiates deletion request and sets accountStatus to deletion_pending for DATA_PROCESSING', async () => {
    ...
    expect(prismaMock.deletionRequest.findUnique).toHaveBeenCalledWith(
    ...
    expect(prismaMock.user.update).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: 'user-2' },
        data: expect.objectContaining({ accountStatus: 'deletion_pending' }),
      })
    );
    expect(prismaMock.auditLog.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ action: 'ERASURE_REQUEST', ipAddress: '5.6.7.8' }),
      })
    );
