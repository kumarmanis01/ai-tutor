...
describe('buildSessionInsight', () => {
  ...
  test('should call LLM with concept name and score data', async () => {
    mockLLM.mockResolvedValueOnce({ content: 'You nailed quadratic equations today -- try polynomial division next.' } as any)
  ...
  test('should return the LLM content when call succeeds', async () => {
    mockLLM.mockResolvedValueOnce({ content: 'Strong session on fractions -- one more practice and you will have it.' } as any)
  ...
  test('should include negative mastery delta in prompt', async () => {
    mockLLM.mockResolvedValueOnce({ content: 'Review this concept once more and it will click.' } as any)
    await buildSessionInsight(makeParams({ masteryDelta: -0.05, correctAnswers: 3, totalQuestions: 10 }))
    const promptArg = mockLLM.mock.calls[0][0] as string
