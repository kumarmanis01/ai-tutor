...
describe('sendEmail()', () => {
  it('should return success when Resend resolves', async () => {
  ...
  it('should return failure when Resend throws', async () => {
  ...
  it('should pass correct subject to Resend', async () => {
    mockResendSend.mockResolvedValueOnce({ data: { id: 'msg-2' }, error: null });
    await sendEmail('STUDENT_WELCOME', 'test@example.com', { studentName: 'Aarav' });
    const call = mockResendSend.mock.calls[0][0];
    expect(call.subject).toContain('Aarav');
  });

  it('should pass html to Resend', async () => {
    mockResendSend.mockResolvedValueOnce({ data: { id: 'msg-3' }, error: null });
