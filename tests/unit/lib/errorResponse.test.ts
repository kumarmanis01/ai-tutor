...
describe('formatErrorForResponse', () => {
  ...
  test('formats string or undefined errors', () => {
    expect(formatErrorForResponse('simple')).toEqual({ name: 'Error', message: 'simple', stack: null })
    expect(formatErrorForResponse(undefined)).toEqual({ name: 'Error', message: 'Unknown error', stack: null })
  })
