...
describe('HttpOnboardingService.saveProfile', () => {
  ...
  it('throws mapped fieldErrors when server returns validation_error', async () => {
    const mockFetch = jest.fn().mockResolvedValue({ ok: false, json: async () => ({ error: 'validation_error', fieldErrors: { school_name: 'too long' }, message: 'Invalid' }) })
    ;(global as any).fetch = mockFetch
    ...
    await expect(svc.saveProfile(values as any)).rejects.toMatchObject({ fieldErrors: { school_name: 'too long' } })
  })
})
