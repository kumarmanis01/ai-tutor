...
describe('extractBadges', () => {
      ...
      userBadges: [
        {
          id: 'ub1',
          badgeKey: 'streak_7',
          badge: { id: 'b1', name: '7-Day Streak', description: '7 consecutive days', icon: 'fire' },
        },
      ],
    }
    const result = extractBadges(profile)
    expect(result).toHaveLength(1)
    expect(result[0]).toEqual({ id: 'b1', name: '7-Day Streak', description: '7 consecutive days', icon: 'fire' })
  })

  test('should extract from flat badges array', () => {
    const profile = {
      badges: [{ id: 'b1', name: 'Chapter Master', description: 'Mastered a chapter', icon: 'star' }],
    }
    const result = extractBadges(profile)
    expect(result).toHaveLength(1)
  ...
  test('should prefer userBadges over badges when both present', () => {
    const profile = {
      userBadges: [{ id: 'ub1', badge: { id: 'b1', name: 'From userBadges', description: 'A', icon: 'fire' } }],
  ...
  test('should fall back to top-level fields when badge nested object is absent', () => {
    const profile = {
      userBadges: [{ id: 'ub1', name: 'Consistency', description: 'Flat badge', icon: 'lightning' }],
    }
    const result = extractBadges(profile)
    expect(result).toHaveLength(1)
  ...
  test('should handle multiple badges correctly', () => {
    const profile = {
      userBadges: [
        ...
        { id: 'ub2', badge: { id: 'b2', name: 'Chapter Master', description: 'Mastery', icon: 'star' } },
      ],
    }
    const result = extractBadges(profile)
