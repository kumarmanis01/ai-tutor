...
describe('logger (unit)', () => {
  ...
  it('logAPI pretty prints request/response metadata when NEXT_PUBLIC_DEBUG_MODE=true', async () => {
    ...
    const fakeReq: any = {
      url: '/api/test',
      method: 'POST',
      clone() { return { text: async () => JSON.stringify({ a: 1, b: 2 }) }; },
    };
    const fakeRes: any = {
      status: 200,
      clone() { return { text: async () => JSON.stringify({ ok: true }) }; },
    };
    ...
    const first = logs.find((l) => String(l).includes('/api/test') || String(l).includes('"route"'));
