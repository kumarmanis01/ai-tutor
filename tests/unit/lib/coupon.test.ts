...
describe('coupon helpers', () => {
  test('createCoupon calls prisma.create', async () => {
    const prisma: any = {
      coupon: { create: jest.fn().mockResolvedValue({ id: 'c1', code: 'WELCOME' }) },
    };
  ...
  test('redeemCoupon for FIXED applies credit to subscription', async () => {
    ...
    const tx: any = {
      coupon: {
        findUnique: jest.fn().mockResolvedValue(couponRow),
        update: jest.fn().mockResolvedValue({ ...couponRow, uses: 1 }),
      },
      subscription: {
        findFirst: jest.fn().mockResolvedValue({ id: 'sub1' }),
        update: jest.fn().mockResolvedValue({ id: 'sub1', creditBalance: 500 }),
      },
      couponRedemption: {
