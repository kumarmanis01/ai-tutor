...
function expectOneUnsafeEvent(
  res: OutputSafetyResult,
...
describe('checkOutputSafety', () => {
  test('Clean educational response → safe, original, no events', () => {
    const input =
      'Let’s solve this step by step. What is the value of x if 2x + 3 = 11?\n[QUESTION]';
  ...
  test('Response containing self-harm reference → replacement, CRITICAL event', () => {
    ...
    expect(res.text).toBe(
      "I'm here to support your learning. If you're feeling overwhelmed, please talk to a trusted adult."
    );
  });
  ...
  test('Response containing violence term → replacement, HIGH event', () => {
    ...
    expect(res.text).toBe(
      "Let's stay focused on learning. What part of the topic would you like to explore?"
    );
  });
  ...
  test('Response containing personal advice pattern → replacement, LOW event', () => {
    ...
    expect(res.text).toBe(
      "That's outside what I can help with. I'm here for your studies -- what shall we work on?"
    );
  });
