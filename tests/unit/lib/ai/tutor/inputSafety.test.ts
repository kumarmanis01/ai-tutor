...
describe('checkInputSafety', () => {
  test('Clean input → safe, unchanged, no events', () => {
    ...
    const expected: InputSafetyResult = {
      safe: true,
      redacted: 'Hello, I need help with algebra.',
      events: [],
    };
  ...
  test('Multiple PII types → all substituted, single PII event', () => {
    const res = checkInputSafety(
      'Phone 9876543210 and email me@school.in and Aadhaar 1234 5678 9012',
      ctx
    );
    expect(res.safe).toBe(true);
