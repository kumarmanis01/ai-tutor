...
describe('copyPasteDetector', () => {
  ...
  test('flags high overlap long answer', () => {
    const chunk = 'Start by writing the equation. Then substitute values. Finally simplify to get the result.'
    const student = `Start by writing the equation. Then substitute values. Finally simplify to get the result. Extra note.`
    const res = detectCopyPaste(student, [{ chunkId: 'c2', content: chunk }], { minTokens: 5, threshold: 0.8 })
    expect(res.flagged).toBe(true)
