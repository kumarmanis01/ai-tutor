...
describe('assembleSystemPrompt', () => {
  ...
  test('7. layersTruncated populated correctly when truncation occurs', () => {
    ...
    expect(res.layersTruncated).toEqual(
      expect.arrayContaining(['CURRICULUM_CONTEXT', 'SESSION_STATE'])
    );
  });
  ...
  test('11. activeMisconception present → appears in SESSION_STATE layer', () => {
    const res = assembleSystemPrompt(
      makeCtx({ activeMisconceptionName: 'Sign error in quadratic formula' })
    );
    const s = res.system;
    const sessionIdx = s.indexOf('### SESSION_STATE');
...
describe('buildStageInstructionsLayer -- hint tiers', () => {
  function makePracticeCtx(
    hintsUsed: number,
    isHintRequest: boolean,
    stage: PromptContext['stage'] = 'GUIDED_PRACTICE'
  ): PromptContext {
    return {
      studentName: 'Priya',
      grade: 9,
