import { computeFrustrationScore, type TurnHistoryEntry, type FrustrationResult } from '@/lib/ai/tutor/signals'
...
describe('computeFrustrationScore', () => {
  ...
  test('Score >= 0.75 → FRUSTRATED', () => {
    const history = [
      sTurn({ isCorrect: false, negativeLanguageDetected: true, hintsRequestedThisTurn: 1, responseLatencyMs: 5000 }),
      sTurn({ isCorrect: false, negativeLanguageDetected: true, hintsRequestedThisTurn: 1, responseLatencyMs: 5000 }),
      sTurn({ isCorrect: false, negativeLanguageDetected: true, hintsRequestedThisTurn: 1, responseLatencyMs: 5000 }),
    ]
    const res = computeFrustrationScore(history, 1000)
