...
describe('Parent alerts mute link', () => {
  ...
  it('GET token pauses ParentStudent and creates audit', async () => {
    ...
    prismaMock.parentStudent.findUnique.mockResolvedValue({
      id: 'link-1',
      parentId: 'parent-1',
      studentId: 'student-1',
      status: 'active',
    });
    prismaMock.parentStudent.update.mockResolvedValue({
      id: 'link-1',
      isPaused: true,
      pausedUntil: new Date('2026-05-01T00:00:00.000Z'),
      pauseReason: 'muted_via_alert_link',
    });

    // Generate a valid token
    const { generateMuteToken } = await import('@/lib/parent/signedLink');
    const token = generateMuteToken({
      parentStudentId: 'link-1',
      studentId: 'student-1',
      muteDays: 7,
      tokenTTLSeconds: 60,
    });
    ...
    const req = new Request(
      'http://localhost/api/parent/alerts/mute?t=' + encodeURIComponent(token)
    );
