...
describe('getClientIp', () => {
  it('should extract IP from x-forwarded-for header', () => {
    expect(getClientIp({ 'x-forwarded-for': '203.0.113.5', 'x-real-ip': null })).toBe(
      '203.0.113.5'
    );
  });

  it('should take the first IP when x-forwarded-for has multiple', () => {
    expect(getClientIp({ 'x-forwarded-for': '203.0.113.5, 10.0.0.1', 'x-real-ip': null })).toBe(
      '203.0.113.5'
    );
  });

  it('should fall back to x-real-ip when x-forwarded-for is absent', () => {
    expect(getClientIp({ 'x-forwarded-for': null, 'x-real-ip': '198.51.100.9' })).toBe(
      '198.51.100.9'
    );
  });
