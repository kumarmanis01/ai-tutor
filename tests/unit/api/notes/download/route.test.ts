/**
 * UNIT TESTS: app/api/notes/download
 *
...
describe('POST /api/notes/download', () => {
  beforeEach(() => {
    resetPrismaMock();
  });
  ...
  it('should return success response', async () => {
    ...
    const { POST } = await import('@/app/api/notes/download/route');
    const request = new Request('http://localhost:3000/api/notes/download', {
      method: 'POST',
      body: JSON.stringify({
