...
jest.mock('@/lib/ai/learningPlan', () => ({
  generateLearningPlan: jest.fn().mockResolvedValue('plan-1'),
}));
...
describe('User profile PATCH (examDate) API', () => {
  beforeEach(() => {
    ...
    (global as any).__TEST_SESSION__ = {
      user: { id: 'student-1', role: 'user', email: 'student@example.test' },
    };
  });
  ...
  it('updates examDate and triggers learning plan regen when plans exist', async () => {
    ...
    prismaMock.user.update.mockResolvedValue({
      ...
      learningStyle: null,
      preferences: {},
      examDate: new Date('2026-05-01T00:00:00.000Z'),
    });

    const { PATCH } = await import('@/app/api/user/profile/route');
    const req = new Request('http://localhost', {
      method: 'PATCH',
      body: JSON.stringify({ examDate: '2026-05-01T00:00:00.000Z' }),
