...
describe('parent subscription verify route', () => {
  ...
  it('generates invoice and sends receipt email on successful verification', async () => {
    ...
    jest.doMock('@/lib/session', () => ({
      getServerSessionForHandlers: jest.fn(async () => ({ user: { id: 'u1', role: 'parent' } })),
    }));
    ...
    const prismaMock: any = {
      paymentOrder: {
        findUnique: jest.fn(async () => ({
          ...
          amount: 99900,
          planMonths: 1,
          providerIdempotencyKey: 'pkey',
        })),
      },
      user: {
        findUnique: jest.fn(async () => ({ name: 'Parent', email: 'p@example.test', phone: null })),
      },
    ...
    const invoiceMock = {
      createInvoiceForPayment: jest.fn(async () => ({
        invoiceNumber: 42,
        pdfBuffer: Buffer.from('pdf'),
        fileUrl: 'https://r2/invoice-42.pdf',
      })),
    };
    ...
    const req: any = {
      json: async () => ({ orderId, paymentId, signature, planId: 'standard_monthly' }),
    };
