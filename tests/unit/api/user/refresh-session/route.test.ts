/**
 * UNIT TESTS: app/api/user/refresh-session
 *
...
describe('POST /api/user/refresh-session', () => {
  beforeEach(() => {
    resetPrismaMock();
  });
  ...
  it('should return success response', async () => {
    ...
    const { POST } = await import('@/app/api/user/refresh-session/route');
    const request = new Request('http://localhost:3000/api/user/refresh-session', {
      method: 'POST',
      body: JSON.stringify({
