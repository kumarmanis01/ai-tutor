...
describe('parent subscription order - planId and family pricing', () => {
  ...
  it('persists planId and uses explicit family plan when available', async () => {
    ...
    jest.doMock('@/lib/session', () => ({
      getServerSessionForHandlers: jest.fn(async () => ({ user: { id: 'p1', role: 'parent' } })),
    }));
    ...
    const req: any = {
      json: async () => ({ planId: 'standard_monthly', childIds: ['c1', 'c2'], isFamily: true }),
    };
  ...
  it('falls back to 1.8x multiplier when explicit family plan missing', async () => {
    ...
    jest.doMock('@/lib/session', () => ({
      getServerSessionForHandlers: jest.fn(async () => ({ user: { id: 'p2', role: 'parent' } })),
    }));
    ...
    jest.doMock('razorpay', () =>
      jest.fn().mockImplementation(() => ({ orders: { create: ordersCreateMock } }))
    );
    ...
    const req: any = {
      json: async () => ({ planId: 'lite_monthly', childIds: ['c3', 'c4'], isFamily: true }),
    };
    const route = await import('@/app/api/parent/subscription/order/route');
    const res = await route.POST(req as unknown as Request);
