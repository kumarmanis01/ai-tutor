...
describe('POST /api/student/verify-parent/confirm-otp', () => {
  beforeEach(() => {
    ...
    jest.doMock('@/lib/session', () => ({
      getServerSessionForHandlers: async () => ({ user: { id: 'stu-1' } }),
    }));
  });
  ...
  it('verifies OTP, updates user and sends welcome notifications', async () => {
    ...
    const mockFindUnique = jest.fn(async (args: any) => ({
    ...
    jest.doMock('@/lib/prisma', () => ({
      prisma: { user: { update: mockUpdate, findUnique: mockFindUnique } },
    }));
    ...
    expect(mockUpdate).toHaveBeenCalledWith({
      where: { id: 'stu-1' },
      data: { parentVerifiedAt: expect.any(Date) },
    });
