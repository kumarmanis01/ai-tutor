...
describe('Diagnostic submit analytics emission', () => {
  ...
  test('enqueues diagnostic_completed when analytics queue available', async () => {
    ...
    jest.doMock('@/jobs/diagnosticBootstrap', () => ({
      enqueueDiagnosticBootstrapJob: jest.fn().mockResolvedValue(undefined),
    }));
    jest.doMock('@/lib/redis/diagnosticPartial', () => ({
      clearPartialDiagnostic: jest.fn().mockResolvedValue(undefined),
    }));
    jest.doMock('@/jobs/diagnosticAutoSubmit', () => ({
      cancelDiagnosticAutoSubmit: jest.fn().mockResolvedValue(undefined),
    }));
    jest.doMock('@/lib/diagnostics/stateStore', () => ({
      upsertSubjectDiagnosticStatus: jest.fn().mockResolvedValue(undefined),
    }));
    jest.doMock('@/lib/diagnostics/sessionStore', () => ({
      getSession: jest.fn().mockResolvedValue(null),
    }));
    jest.doMock('@/lib/diagnostics/selector', () => ({
      computeSessionTheta: jest.fn().mockResolvedValue({ theta: 0 }),
    }));
  ...
  test('falls back to DB when analytics queue unavailable on submit', async () => {
    ...
    const prismaMock = {
      question: {
        findMany: jest
          .fn()
          .mockResolvedValue([{ id: 'q1', correctAnswer: 'A', choices: ['A'], topicId: 't1' }]),
      },
      concept: { findMany: jest.fn().mockResolvedValue([{ id: 'c1', topicId: 't1' }]) },
      topicDef: { findMany: jest.fn().mockResolvedValue([{ id: 't1', chapterId: 'ch1' }]) },
      subjectDef: { findUnique: jest.fn().mockResolvedValue({ class: { id: 'grade-9' } }) },
