...
describe('POST /api/student/whiteboard/evaluate', () => {
  ...
  it('returns fallback feedback when LLM call fails', async () => {
    jest.doMock('@/lib/session', () => ({
      getServerSessionForHandlers: async () => ({ user: { id: 's1' } }),
    }));
  ...
  it('returns LLM feedback when available', async () => {
    jest.doMock('@/lib/session', () => ({
      getServerSessionForHandlers: async () => ({ user: { id: 's2' } }),
    }));
    jest.doMock('@/lib/callLLM', () => ({
      callTutorLLM: async () => ({ content: 'Nice work — try isolating the variable next.' }),
    }));
  ...
  it('stores canvas artifact when provided and returns artifactUrl', async () => {
    jest.resetModules();
    jest.doMock('@/lib/session', () => ({
      getServerSessionForHandlers: async () => ({ user: { id: 's3' } }),
    }));
  ...
  it('persists artifact record in DB when upload succeeds', async () => {
    jest.resetModules();
    jest.doMock('@/lib/session', () => ({
      getServerSessionForHandlers: async () => ({ user: { id: 's4' } }),
    }));
