...
describe('Razorpay webhook reconciliation (unit)', () => {
  ...
  it('creates payment with providerIdempotencyKey on payment.captured when header present', async () => {
    ...
    const prismaMock: any = {
      paymentOrder: { findUnique: jest.fn().mockResolvedValue({ studentId: 'student-1', status: 'pending' }) },
    ...
    jest.doMock('@/jobs/paymentDunning', () => ({ getPaymentDunningQueue: () => ({ add: jest.fn() }) }))

    const route = await import('@/app/api/razorpay/webhook/route')

    const req = new Request('http://localhost', { method: 'POST', headers: { 'x-razorpay-signature': sig, 'idempotency-key': idempotencyKey, 'content-type': 'application/json' }, body: raw })
  ...
  it('handles payment.failed: creates failed payment, bumps dunning, notifies parent and enqueues job', async () => {
    ...
    const payload = {
      ...
      payload: { payment: { entity: { id: 'pay_fail_1', order_id: 'order_fail_1', amount: 49900, error_reason: 'insufficient_funds' } } },
    }
    const raw = JSON.stringify(payload)
    const sig = crypto.createHmac('sha256', process.env.RAZORPAY_WEBHOOK_SECRET!).update(raw).digest('hex')
    ...
    const tx: any = {
      paymentOrder: { update: jest.fn().mockResolvedValue(true) },
      payment: { findFirst: jest.fn().mockResolvedValue(null), create: jest.fn().mockResolvedValue({ id: 'pf1' }), update: jest.fn() },
      subscription: { findFirst: jest.fn().mockResolvedValue({ id: 'sub1' }), update: jest.fn().mockResolvedValue(true) },
    }

    const addMock = jest.fn()
    const prismaMock: any = {
      paymentOrder: { findUnique: jest.fn().mockResolvedValue({ studentId: 'parent-1', status: 'pending', planMonths: 1 }) },
      payment: { findFirst: jest.fn() },
      subscription: { findFirst: jest.fn() },
      user: { findUnique: jest.fn().mockResolvedValue({ id: 'parent-1', email: 'p@example.test', phone: '99999', name: 'Parent' }) },
      $transaction: jest.fn(async (fn: any) => { await fn(tx); }),
    }
    ...
    jest.doMock('@/lib/mailer', () => ({ sendMailSafe: (...a: any[]) => mockSendMail(...a) }))
    jest.doMock('@/lib/sms', () => ({ sendSms: (...a: any[]) => mockSendSms(...a) }))
    jest.doMock('@/jobs/paymentDunning', () => ({ getPaymentDunningQueue: () => ({ add: addMock }) }))
