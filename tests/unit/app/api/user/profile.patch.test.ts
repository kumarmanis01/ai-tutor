...
describe('PATCH /api/user/profile', () => {
  ...
  it('merges new crunchMode into preferences when valid', async () => {
    ...
    jest.doMock('@/lib/prisma', () => ({ prisma: { user: { findUnique: mockFindUnique, update: mockUpdate } } }))
  ...
  it('returns 400 for invalid crunchMode value', async () => {
    ...
    jest.doMock('@/lib/session', () => ({ getServerSessionForHandlers: async () => session }))
    jest.doMock('@/lib/prisma', () => ({ prisma: { user: { findUnique: async () => ({ preferences: {} }) } } }))
  ...
  it('returns 400 when nothing to update', async () => {
    ...
    const { PATCH } = await import('@/app/api/user/profile/route')
    const req = new Request('http://localhost', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) })
  ...
  it('saves badgeShowcase when valid', async () => {
    ...
    const mockFindUnique = jest.fn(async () => existing)
    const mockUpdate = jest.fn(async () => ({ id: 'u4', preferences: { badgeShowcase: ['b', 'c'] } }))
    jest.doMock('@/lib/prisma', () => ({ prisma: { user: { findUnique: mockFindUnique, update: mockUpdate } } }))
