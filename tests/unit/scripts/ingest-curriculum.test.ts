...
describe('ingest-curriculum script', () => {
  ...
  it('bumps version when content changed and writes run log', async () => {
    ...
    jest.isolateModules(() => {
      ...
      prismaMock.$queryRawUnsafe = jest
        .fn()
    ...
    if (execCalls === 0) {
      ...
      throw new Error(
        `Expected $executeRawUnsafe to be called but it was not; $queryRawUnsafe calls=${qCalls}`
      );
    }
    const firstSql = (prismaMock.$executeRawUnsafe as jest.Mock).mock.calls[0][0];
    if (!firstSql.includes('version = version + 1'))
      throw new Error(`Expected SQL to bump version; got: ${firstSql}`);
    if (typeof (prismaMock.ingestRunLog.create as jest.Mock).mock !== 'object')
      throw new Error('ingestRunLog.create mock not present');
  });
  ...
  it('is idempotent when contentHash matches and embedding exists', async () => {
    ...
    if (execCalls2 > 0)
      throw new Error(`Did not expect $executeRawUnsafe to be called; calls=${execCalls2}`);
    if (typeof (prismaMock.ingestRunLog.create as jest.Mock).mock !== 'object')
      throw new Error('ingestRunLog.create mock not present');
  });
});
