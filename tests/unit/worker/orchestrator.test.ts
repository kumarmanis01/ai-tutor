...
jest.mock('../../../lib/prisma', () => ({
  prisma: {
    workerLifecycle: { findMany: jest.fn(), update: jest.fn() },
    auditLog: { create: jest.fn() },
    $queryRawUnsafe: jest.fn(),
    $transaction: jest.fn(),
  },
}));
jest.mock('../../../worker/metrics-server', () => ({
  startMetricsServer: jest.fn(),
  incJobsSpawned: jest.fn(),
}));
jest.mock('../../../worker/k8s-adapter', () => ({ createJobForWorker: jest.fn() }));
jest.mock('../../../src/jobs/analyticsJobs', () => ({
  runAnalyticsJobs: jest.fn(async () => ({ success: true, durationMs: 10 })),
}));
