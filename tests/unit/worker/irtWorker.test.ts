...
describe('processIRTUpdate', () => {
  ...
  it('should update theta, mastery, and answerEvent when answer is correct (happy path)', async () => {
    ...
    jest.doMock('@/lib/ai/tutor/irt.js', () => ({
      updateTheta: jest.fn(() => ({ newTheta: 0.5, newMastery: 0.68 })),
    }));
    jest.doMock('@/lib/push/send', () => ({ sendPushSafe: jest.fn() }));
    jest.doMock('@/lib/notifications/delivery', () => ({
      sendParentMilestoneNotification: jest.fn(),
    }));
    ...
    expect(updateMock).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { studentId_conceptId: { studentId: 's1', conceptId: 'c1' } },
      })
    );
    expect(createAnswerMock).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ studentId: 's1', sessionId: 'sess1', isCorrect: true }),
      })
    );
  });
  ...
  it('should not create a duplicate answerEvent when questionId already exists (idempotency)', async () => {
    ...
    const prismaMock: any = {
      ...
      answerEvent: {
        findFirst: jest.fn(async () => ({ id: 'existing-ae' })),
    ...
    jest.doMock('@/lib/ai/tutor/irt.js', () => ({
      updateTheta: jest.fn(() => ({ newTheta: 0.3, newMastery: 0.5 })),
    }));
    jest.doMock('@/lib/push/send', () => ({ sendPushSafe: jest.fn() }));
    jest.doMock('@/lib/notifications/delivery', () => ({
      sendParentMilestoneNotification: jest.fn(),
  ...
  it('should log error and rethrow when DB update fails', async () => {
    ...
    jest.doMock('@/lib/prisma', () => ({ prisma: prismaMock }));
    jest.doMock('@/lib/logger', () => ({
      logger: { info: jest.fn(), warn: jest.fn(), error: errorLogMock },
    }));
    jest.doMock('@/lib/ai/tutor/irt.js', () => ({
      updateTheta: jest.fn(() => ({ newTheta: 0.2, newMastery: 0.35 })),
    }));
    jest.doMock('@/lib/push/send', () => ({ sendPushSafe: jest.fn() }));
    jest.doMock('@/lib/notifications/delivery', () => ({
      sendParentMilestoneNotification: jest.fn(),
  ...
  it('sends parent notification on chapter mastery', async () => {
    ...
    const prismaMock: any = {
      ...
      concept: {
        findUnique: jest.fn(async () => ({
          topic: { chapter: { id: 'chap-1', name: 'Algebra' } },
        })),
    ...
    jest.doMock('@/lib/ai/tutor/irt.js', () => ({
      updateTheta: jest.fn(() => ({ newTheta: 1.0, newMastery: 0.9 })),
    }));
    jest.doMock('@/lib/push/send', () => ({ sendPushSafe: jest.fn() }));
    jest.doMock('@/lib/notifications/delivery', () => ({
      sendParentMilestoneNotification: sendParentMock,
    }));
    jest.doMock('@/lib/redis', () => ({
      getRedis: () => ({ get: jest.fn(async () => null), set: jest.fn(async () => 'OK') }),
    }));
