...
describe('processDiagnosticAutoSubmit', () => {
  ...
  it('queries chapterDef with lifecycle: "active" filter for partial abandon', async () => {
    ...
    const { processDiagnosticAutoSubmit } = await import('@/worker/services/diagnosticAutoSubmitWorker')
    const fakeJob: any = { id: 'job-2', data: { userId: 's1', subjectId: 'subj-1', sessionId: 'sess-1' } }
  ...
  it('uses gradeable answer count (not raw Redis answer count) to determine isPartialAbandon', async () => {
    ...
    const prismaMock: any = {
      question: {
        findMany: jest.fn(async () => [
          { id: 'q-1', correctAnswer: '0', choices: JSON.stringify(['A','B']), topicId: 'topic-1' },
          { id: 'q-2', correctAnswer: '1', choices: JSON.stringify(['A','B']), topicId: 'topic-1' },
          { id: 'no-concept', correctAnswer: '2', choices: JSON.stringify(['A','B','C']), topicId: 'topic-no-concept' },
        ]),
      },
      concept: {
        findMany: jest.fn(async () => [
    ...
    const { processDiagnosticAutoSubmit } = await import('@/worker/services/diagnosticAutoSubmitWorker')
    const fakeJob: any = { id: 'job-3', data: { userId: 's1', subjectId: 'subj-1', sessionId: 'sess-1' } }
