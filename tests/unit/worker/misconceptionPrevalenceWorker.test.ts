...
describe('misconceptionPrevalenceWorker', () => {
  ...
  it('updates prevalenceRate for each misconception', async () => {
    ...
    expect(prismaMock.misconception.update).toHaveBeenCalledWith(
      expect.objectContaining({ where: { id: 'm-1' }, data: { prevalenceRate: 5 / 50 } })
    );
  });

  it('creates a system alert when prevalence exceeds threshold', async () => {
    prismaMock.misconception.findMany = jest
      .fn()
      .mockResolvedValue([{ id: 'm-high', conceptId: 'c-high' }]);
    ...
    prismaMock.misconception.findUnique = jest
      .fn()
      .mockResolvedValue({ id: 'm-high', name: 'HighMis', conceptId: 'c-high', subjectId: 's-1' });
    prismaMock.systemAlert = {
    ...
    expect(prismaMock.systemAlert.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ type: 'MISCONCEPTION_HIGH_PREVALENCE' }),
      })
    );
  });
  ...
  it('updates existing alert when prevalence remains high', async () => {
    prismaMock.misconception.findMany = jest
      .fn()
      .mockResolvedValue([{ id: 'm-exist', conceptId: 'c-exist' }]);
    ...
    prismaMock.systemAlert = {
      findFirst: jest.fn().mockResolvedValue({ id: 'alert-1', active: true }),
      update: jest.fn(),
      create: jest.fn(),
    } as any;
  ...
  it('resolves existing active alert when prevalence drops below threshold', async () => {
    prismaMock.misconception.findMany = jest
      .fn()
      .mockResolvedValue([{ id: 'm-resolve', conceptId: 'c-res' }]);
    ...
    prismaMock.systemAlert = {
      findFirst: jest.fn().mockResolvedValue({ id: 'alert-2', active: true }),
      update: jest.fn(),
      create: jest.fn(),
    } as any;
    ...
    expect(res.updated).toBe(1);
    expect(prismaMock.systemAlert.update).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: 'alert-2' },
  ...
  it('logs alert errors but continues processing', async () => {
    prismaMock.misconception.findMany = jest
      .fn()
      .mockResolvedValue([{ id: 'm-err', conceptId: 'c-err' }]);
    ...
    prismaMock.misconception.findUnique = jest
      .fn()
      .mockResolvedValue({ id: 'm-err', name: 'ErrMis', conceptId: 'c-err', subjectId: 's-4' });
    // systemAlert.findFirst throws to trigger catch
    prismaMock.systemAlert = {
      findFirst: jest.fn().mockImplementation(() => {
        throw new Error('alert-fail');
      }),
      create: jest.fn(),
      update: jest.fn(),
    } as any;
    ...
    expect(logger.error).toHaveBeenCalledWith(
      'misconceptionPrevalence.alert.failed',
      expect.any(Object)
    );
  });

  it('returns {updated:0} and logs on top-level failure', async () => {
    prismaMock.misconception.findMany = jest.fn().mockImplementation(() => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      'misconceptionPrevalence.run.failed',
      expect.any(Object)
    );
  });
});
