...
describe('Diagnostic pipeline: auto-submit -> bootstrap', () => {
  ...
  it('should write AnswerEvents, enqueue bootstrap with covered chapters, and seed concepts for a full run (>= 10 answers)', async () => {
    ...
    jest.doMock('@/lib/logger.js', () => ({ logger: { info: jest.fn(), warn: jest.fn(), error: jest.fn() } }))
    ...
    const { processDiagnosticAutoSubmit } = await import(
      '@/worker/services/diagnosticAutoSubmitWorker'
    )
    ...
    expect(enqueueMock).toHaveBeenCalledWith(
      expect.objectContaining({ chapterIds: ['chap-1'] }),
    )
    ...
    const { processDiagnosticBootstrap } = await import(
      '@/worker/services/diagnosticBootstrapWorker'
    )
