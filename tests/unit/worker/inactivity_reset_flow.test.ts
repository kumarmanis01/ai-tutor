  ...
  it('sets suppression on send and clears it on activity', async () => {
    ...
    jest.doMock('@/lib/prisma', () => ({
      prisma: {
        ...
        learningPlanItem: {
          findFirst: jest.fn().mockResolvedValue({ id: 'planItem1', concept: { name: 'Algebra' } }),
        },
      },
    }));
    ...
    const sendMock = jest.fn().mockImplementation(async (_parentId: string, _opts: any) => {
      ...
      await redisMock.set(
        `parent:notifications:suppression:inactivity:p1:stu1`,
        '1',
        'NX',
        'EX',
        3 * 24 * 60 * 60
    ...
    jest.doMock('@/lib/prisma', () => ({
      prisma: {
        user: {
          findUnique: jest
            .fn()
            .mockResolvedValue({ lastSessionDate: null, currentStreak: 0, longestStreak: 0 }),
    ...
    console.log(
      'debug getRedis ->',
      typeof debugGetRedis,
      'returned ->',
      JSON.stringify(await debugGetRedis())
    );
    ...
    // eslint-disable-next-line no-console
    console.log(
      'debug prisma.parentStudent.findMany ->',
      JSON.stringify(await debugPrisma.parentStudent.findMany())
    );
