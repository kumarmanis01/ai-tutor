...
describe('installmentDunningWorker', () => {
  ...
  it('charges a single installment when payment method exists', async () => {
    const inst = { id: 'inst-1', subscriptionId: 'sub-1', number: 1, amount: 10000, currency: 'INR', attemptCount: 1 };
    ...
    const pm = { id: 'pm-1', userId: 'parent-1', provider: 'razorpay', providerPaymentMethodId: 'pm_1', verified: true, customer: { providerCustomerId: 'rcust_1' } };

    prismaMock.installment = { findUnique: jest.fn().mockResolvedValue(inst), update: jest.fn().mockResolvedValue(true) } as any;
    prismaMock.subscription = { findUnique: jest.fn().mockResolvedValue(sub), update: jest.fn().mockResolvedValue(sub) } as any;
    ...
    (createRazorpayTokenCharge as jest.Mock).mockResolvedValue({ id: 'pay_new', order_id: 'order_1' });
  ...
  it('increments attempt and starts grace when attempts reach 3', async () => {
    const inst = { id: 'inst-2', subscriptionId: 'sub-2', number: 2, amount: 20000, currency: 'INR', attemptCount: 2, lastAttemptAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000) };
    ...
    const pm = { id: 'pm-2', userId: 'parent-2', provider: 'razorpay', providerPaymentMethodId: 'pm_2', verified: true, customer: { providerCustomerId: 'rcust_2' } };

    prismaMock.installment = { findUnique: jest.fn().mockResolvedValue(inst), update: jest.fn().mockResolvedValue(true), findMany: jest.fn() } as any;
    prismaMock.subscription = { findUnique: jest.fn().mockResolvedValue(sub), update: jest.fn().mockResolvedValue(sub) } as any;
    prismaMock.user = { findUnique: jest.fn().mockResolvedValue(parent) } as any;
    prismaMock.paymentMethod = { findFirst: jest.fn().mockResolvedValue(pm) } as any;
