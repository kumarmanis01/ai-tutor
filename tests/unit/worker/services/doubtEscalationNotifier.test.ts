...
jest.mock('@/lib/prisma', () => ({
  prisma: {
    doubtEscalation: { findMany: (...a: any[]) => mockFindMany(...a), update: (...a: any[]) => mockEscUpdate(...a) },
...
jest.mock('@/lib/ai/tutor/doubtKb', () => ({ recordDoubt: (...a: any[]) => mockRecordDoubt(...a) }))
...
beforeAll(() => {
  ...
  processDoubtEscalationNotifications = require('@/worker/services/doubtEscalationNotifier').processDoubtEscalationNotifications
})
...
test('processes resolved escalations: caches answer and notifies student', async () => {
  ...
  expect(mockEscUpdate).toHaveBeenCalledWith({ where: { id: 'esc1' }, data: { notifiedAt: expect.any(Date) } })
})
