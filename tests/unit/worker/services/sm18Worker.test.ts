...
describe('processNightlySM18', () => {
  ...
  it('should update nextReviewAt for each due row (happy path)', async () => {
    ...
    const findManyMock = jest.fn().mockResolvedValueOnce(rows).mockResolvedValueOnce([]);
    ...
    jest.doMock('@/lib/push/notifications', () => ({
      PUSH_NOTIFICATIONS: { exam_14_days: jest.fn(() => ({})) },
    }));
  ...
  it('should use PRE_EXAM_TARGET_RETENTION for students with exams within 14 days', async () => {
    ...
    expect(updateSM18Mock).toHaveBeenCalledWith(expect.objectContaining({ targetRetention: 0.92 }));
  });
});
