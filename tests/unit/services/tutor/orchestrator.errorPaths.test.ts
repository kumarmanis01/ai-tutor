...
jest.mock('@/lib/ai/tutor/promptAssembly', () => ({
  assembleSystemPrompt: jest.fn().mockReturnValue({ system: 'sys' }),
}));
jest.mock('@/lib/ai/tutor/outputSafety', () => ({
  checkOutputSafety: jest.fn().mockImplementation((t) => ({ text: t, safe: true, events: [] })),
}));
jest.mock('@/lib/ai/tutor/rag', () => ({
  retrieveRelevantChunks: jest.fn().mockResolvedValue({ chunks: [], chunkIds: [] }),
}));
jest.mock('@/lib/ai/tutor/copyPasteDetector', () => ({
  __esModule: true,
  default: jest.fn().mockImplementation(() => ({ flagged: false })),
}));
jest.mock('@/lib/ai/tutor/explanationCache', () => ({
...
jest.mock('@/lib/ai/tutor/signals', () => ({
  computeFrustrationScore: jest
    .fn()
    .mockReturnValue({ frustrationScore: 0, emotionalState: 'calm' }),
}));
jest.mock('@/lib/ai/tutor/stateMachine', () => ({
  applyTagTransition: jest
    .fn()
    .mockReturnValue({
      stage: 'HOOK',
      stageAttemptCount: 0,
...
describe('orchestrator error paths (unit)', () => {
  ...
  it('marks turn completed and rethrows LLMError when LLM fails with LLMError', async () => {
    (callTutorLLM as jest.Mock).mockRejectedValueOnce(
      new (LLMError as any)('AI_UNAVAILABLE', 'down')
    );
