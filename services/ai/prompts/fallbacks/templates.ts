...
export const JUNIOR_TEMPLATES: Record<FallbackStrategy, FallbackTemplate> = {
  ...
  [FallbackStrategy.CONTENT_REDIRECT]: {
    ...
    emoji: '🚀',
    followUpPrompts: ['What chapter are you studying?', "Would you like to see today's lesson?"],
    showRetry: false,
    offerHumanHelp: false,
  },
...
export const MIDDLE_TEMPLATES: Record<FallbackStrategy, FallbackTemplate> = {
  ...
  [FallbackStrategy.SAFE_RESPONSE]: {
    message:
      "That's a thoughtful question, but I want to be careful to give you accurate information.",
...
export const SENIOR_TEMPLATES: Record<FallbackStrategy, FallbackTemplate> = {
  [FallbackStrategy.SIMPLIFY_AND_RETRY]: {
    message:
      'I need more clarity to provide an accurate response. I might be unable to answer this precisely right now.',
  ...
  [FallbackStrategy.SAFE_RESPONSE]: {
    ...
    suggestion:
      'I recommend cross-checking with your textbook or consulting your teacher for this specific question.',
    followUpPrompts: [
  ...
  [FallbackStrategy.CONTENT_REDIRECT]: {
    ...
    suggestion:
      'Focus on board exam syllabus topics for now. This can be explored in higher classes.',
    followUpPrompts: [
...
export function getContentTypeFallback(
  ...
  const gradeBand: GradeBand =
    typeof gradeOrBand === 'number' ? getGradeBand(gradeOrBand) : gradeOrBand;
...
export function getFailureReasonFallback(
  ...
  if (reason === FailureReason.HOMEWORK_DUMP_DETECTED) {
    const messages: Record<GradeBand, FallbackTemplate> = {
      ...
      middle: {
        ...
        suggestion:
          "To help you learn better, let's focus on one question at a time. Which concept is most confusing?",
        showRetry: false,
        offerHumanHelp: false,
      },
      senior: {
        ...
        suggestion:
          "I'm here to help you understand concepts, not complete assignments. Please ask about specific doubts you have while solving these.",
        showRetry: false,
        offerHumanHelp: false,
      },
    };
  ...
  if (reason === FailureReason.OFF_SYLLABUS_QUERY) {
    const messages: Record<GradeBand, FallbackTemplate> = {
      junior: {
        ...
        suggestion:
          "That's wonderful, but let's learn what's in your book first. We can explore more later!",
        emoji: '📚',
        showRetry: false,
        offerHumanHelp: false,
      },
      middle: {
        ...
        suggestion:
          "It's great that you're curious! For now, let's master your current syllabus first.",
        showRetry: false,
        offerHumanHelp: false,
      },
      senior: {
        ...
        suggestion:
          'I recommend focusing on exam-relevant topics. This can be explored in competitive exam preparation or higher studies.',
        showRetry: false,
        offerHumanHelp: false,
      },
    };
  ...
  if (reason === FailureReason.SAFETY_VIOLATION) {
    return {
      message:
        'I can only help with educational questions. Try asking about a different topic or a specific part of your lesson.',
...
export function getFallbackTemplate(...args: any[]): FallbackTemplate {
  ...
  if (typeof args[0] === 'string') {
    ...
    strategy = failureCategory
      ? categoryToStrategy[failureCategory] || FallbackStrategy.SAFE_RESPONSE
      : FallbackStrategy.SAFE_RESPONSE;
  } else {
    // grade-first style
  ...
  if (typeof args[0] === 'string' && args[2]) {
    ...
    if (failureCategory === FailureCategory.TIMEOUT) {
      if (contentType === 'NOTES') {
        ...
        template.message =
          'This operation is taking longer than expected. Please try again shortly.';
      }
    }
