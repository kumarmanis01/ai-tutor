...
import {
  checkInputSafety,
  type SafetyEventCreate as InputSafetyEvent,
} from '@/lib/ai/tutor/inputSafety';
...
import {
  checkOutputSafety,
  type SafetyEventCreate as OutputSafetyEvent,
} from '@/lib/ai/tutor/outputSafety';
import { parseLlmJson } from '@/lib/llm/sanitizeJson';
import {
  applyTagTransitionWithRemediation,
  type TutorTag,
  type TutorStage,
} from '@/lib/ai/tutor/stateMachine';
...
export async function getTutorSession(sessionId: string): Promise<TutorSessionState | null> {
  ...
  return {
    ...
    consecutiveWrongAnswers:
      typeof s.consecutiveWrongAnswers === 'number' ? s.consecutiveWrongAnswers : 0,
    ...
    prereqReturnStage:
      typeof s.prereqReturnStage === 'string' ? (s.prereqReturnStage as TutorStage) : null,
...
export async function runTutorOrchestrator(args: {
  ...
  const isStyleRequest =
    isExplainSimpler || isExplainHarder || isExplainExample || isExplainDiagram;
  ...
  const explainStyle: 'simpler' | 'harder' | 'real_life_example' | 'diagram' | null = isStyleRequest
    ? explainStyleFromSentinel
    : sessionExplainStyle;

  // Replace sentinels with clean phrases so safety checks never see raw values.
  const effectiveMessage = isHintRequest
  ...
  try {
    ...
    const userProfilePromise =
      prismaClient.user && typeof prismaClient.user.findUnique === 'function'
    ...
    const studentLearningProfilePromise =
      prismaClient.studentLearningProfile &&
      typeof prismaClient.studentLearningProfile.findUnique === 'function'
        ? prismaClient.studentLearningProfile.findUnique({
            where: { studentId },
    ...
    const conceptStatePromise: Promise<{ masteryScore: number | null } | null> =
      prismaClient.studentConceptState &&
      typeof prismaClient.studentConceptState.findUnique === 'function'
    ...
    const conceptDifficulty =
      typeof concept?.irt_b === 'number' && Number.isFinite(concept.irt_b) ? concept.irt_b : 0;
    ...
    if (!teachingLanguageRaw && typeof uiLanguage === 'string' && uiLanguage.length > 0)
      teachingLanguageRaw = uiLanguage;
    ...
    const safetyEvents: (InputSafetyEvent | OutputSafetyEvent)[] = [...inputSafety.events].map(
      (e) => {
    ...
    if (process.env.ENABLE_DISTRESS_DETECTION === 'true') {
      ...
      if (distressResult.detected) {
        ...
        if (distressResult.severity === 'CRITICAL' || distressResult.severity === 'HIGH') {
          ...
          await prismaClient.aITutorTurnLog
            .create({
              data: {
                sessionId,
    ...
    try {
      const cp = detectCopyPaste(
        redactedInput,
        ragContext.chunks.map((c) => ({ chunkId: c.chunkId, content: c.content }))
      );
      if (cp.flagged) {
        ...
        try {
          ...
          if (
            ...
            typeof prismaClient.message.update === 'function'
          ) {
            try {
              await prismaClient.message.update({
                where: { id: messageId },
                data: { anomalyFlags: [anomaly] },
              });
            } catch (e) {
              logger.warn('message.anomalyFlags.update.failed', {
                messageId,
          ...
          logger.warn('copyPaste.aITutorTurnLog.failed', {
            error: String((e as any)?.message ?? e),
          });
        }
        try {
    ...
    try {
      const chapterId =
        (concept as any)?.topic?.chapter?.id ?? (concept as any)?.topic?.chapterId ?? null;
      if (chapterId) {
        const bw = await prismaClient.boardChapterWeight.findUnique({
          where: { chapterId },
      ...
      logger.warn('boardChapterWeight.load.failed', {
        conceptId,
        error: String((e as any)?.message ?? e),
      });
    }
    ...
    const isDoubtTurn =
      ...
      (redactedInput.endsWith('?') ||
        /\b(what|why|how|explain|confused|don'?t understand|clarify|mean|means|help)\b/i.test(
          redactedInput
        ));
    ...
    try {
      ...
      if (modality) {
        ...
        if (cached?.content) {
          ...
          const res = await callTutorLLM(
            prompt.system,
            { callType: tutorCallType, sessionId, studentId },
            undefined
          );
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object') {
        ...
        for (const f of preferFields) {
          if (
            typeof (parsed as any)[f] === 'string' &&
            String((parsed as any)[f]).trim().length > 0
          ) {
    ...
    if (safetyEvents.length) {
      ...
      try {
        ...
        logger.warn('analyticsEvent.safety.create.failed', {
          error: String((e as any)?.message ?? e),
        });
      }
    }
    ...
    try {
      const hallCtx = {
        ...
        board: 'CBSE',
        subject: subjectName,
        originalQuestion: String(redactedInput),
      };
      const hall = checkForHallucinations(answerText, hallCtx as any);
      groundednessScore =
        typeof hall?.riskScore === 'number' ? Math.max(0, 1 - hall.riskScore) : null;
      if (hall && (hall.issues.length > 0 || hall.needsReview)) {
        try {
          ...
          logger.warn('analyticsEvent.hallucination.create.failed', {
            error: String((e as any)?.message ?? e),
          });
        }
      }
      ...
      if (hall && hall.shouldBlock) {
        try {
          const safe = getSafeResponseForIntent(
            intentClassification.primaryIntent,
            10,
            subjectName
          );
          ...
          logger.warn('analyticsEvent.hallucination_block.create.failed', {
            error: String((e as any)?.message ?? e),
          });
        }
      }
    ...
    if (
      ...
      try {
        await prismaClient.message.update({
          where: { id: messageId },
          data: { anomalyFlags: [detectionAnomaly] },
        });
      } catch (e) {
        logger.warn('message.anomalyFlags.update.failed', {
          messageId,
    ...
    if (isHintRequest) {
      try {
        ...
        if (
          prismaClient.studentConceptState &&
          typeof prismaClient.studentConceptState.upsert === 'function'
        ) {
          ...
          if (totalHints != null && totalHints >= threshold && !upsertRes?.needsConsolidation) {
            ...
            try {
              await prismaClient.studentConceptState.update({
                where: { id: upsertRes.id },
                data: { needsConsolidation: true },
              });
            } catch (e) {
              logger.warn('studentConceptState.markConsolidation.failed', {
                studentId,
                conceptId,
                error: String((e as any)?.message ?? e),
              });
            ...
            try {
              ...
              logger.warn('attentionFlag.upsert.failed', {
                studentId,
                conceptId,
                error: String((e as any)?.message ?? e),
              });
        ...
        logger.warn('studentConceptState.hintUpdate.failed', {
          studentId,
          conceptId,
          error: String((e as any)?.message ?? e),
        });
    ...
    const complete: TutorTurnComplete = {
      ...
      contrastiveExplanation: contrastiveArtifact
        ? contrastiveArtifact
        : activeMisconception
          ? {
              misconceptionId: activeMisconception.misconceptionId,
    ...
    if (logTag === 'VALIDATE') {
      await enqueueIRTUpdate({
        ...
        itemDifficulty: Number.isFinite(args.itemDifficulty)
          ? args.itemDifficulty!
          : conceptDifficulty,
